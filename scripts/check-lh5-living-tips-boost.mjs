/**
 * check-lh5-living-tips-boost.mjs
 *
 * QA-LH-5 저장 plan 기준으로 living_tips narration 보강 로직 로컬 검증
 * (유료 API 호출 없음 — 저장 JSON만 사용)
 *
 * 실행:
 *   node scripts/check-lh5-living-tips-boost.mjs
 */
...
const plan = JSON.parse(
  readFileSync(join(ROOT, "output/v2/paid_qa/gpt4o_mini_life_hacks_qa_lh5_plan.json"), "utf8")
);

const NARRATION_WARNING_LEN = 14;

// scene 10 gate 판정 regex (route.ts 6-d 와 동일)
...
// CTA 보강 판정 regex (route.ts 5-a-lt 와 동일)
const THIN_CTA_RE =
...
console.log(`=== QA-LH-5 living_tips 보강 시뮬레이션 (${plan.title}) ===\n`);
...
const results = plan.scenes.map((scene, idx) => {
  ...
  if (sceneNumber === total) {
    ...
    if (isThinCta) {
      ...
      if (!boosted) {
        // scene 6~9 (idx 5~8)에서 재료/오브젝트 추출
