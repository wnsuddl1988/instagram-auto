/**
 * upload_002_copier S4 Veo 영상 생성
 *
 * REFERENCE: kf_s4_paper_pile_button.png (continuity_pass)
 * OUTPUT:    output/v2/3d_sitcom_prod_v1/upload_002_copier/veo/s4_veo_raw.mp4
 * 방식:      Gemini 웹 VideoFX, profile 1→2→3 (4 금지)
 * 종횡비:   세로 9:16
 *
 * Owner 승인: ALLOW_VEO=true, S4 Veo 1회 (매번 명시 필요)
 * - Gemini 웹 할당량 사용, 직접 API $0
 * - 자동 재제출 0회 / TTS 금지
 *
 * 핵심 계약 (CURRENT_CONTRACT.json S4):
 *   - paper_pile: "시작부터 이미 종이 쌓여있음. 실시간 폭발·낙하 생성 금지."
 *   - veo_max_attempts: 2
 *
 * S4 reference 시작 상태 (2026-06-15 분석):
 *   - 트레이→바닥 종이 더미 이미 존재
 *   - 오른손이 제어 패널에 이미 닿아 있음 (버튼 누르는 중)
 *   - 복사기 출력 트레이에서 종이 1장 배출 중
 *   - 갈색 크로스바디 백 오른쪽 허리 ✅
 *   - 앞으로 숙인 자세 + 황당한 표정
 *
 * 전송 횟수 설계 (S3에서 검증됨):
 ...
 *
 * 사용: node scripts/_upload002-s4-veo-generate.mjs [--profile 1|2|3] [--dry-run]
 */
...
const REF_IMG = path.join(KF_DIR, "kf_s4_paper_pile_button.png");
const OUT_MP4 = path.join(VEO_DIR, "s4_veo_raw.mp4");
...
// ── S4 행동 프롬프트 ────────────────────────────────────────────────────────
// S4 reference 분석 (2026-06-15):
//   - 시작부터 트레이→바닥 종이 더미 이미 거대하게 쌓여 있음
//   - 준의 오른손이 이미 제어 패널 버튼에 닿은 상태
//   - 복사기 출력 트레이에서 종이 1장 배출 중
//   - 준은 이미 앞으로 몸을 숙이고 황당한 표정
//   따라서: "버튼 누르기 시작"이 아니라 "반복 누르는 중간 상태"에서 시작
//   절대 금지: 종이 폭발, 종이 대량 낙하, 더미 갑자기 생성
...
CRITICAL CONTRACT — paper pile:
The large pile of papers already on the floor and output tray exists from the very first frame. This pile was there before the scene starts and must NEVER appear to suddenly grow, fall, or explode. The copier ejects only one sheet at a time at a normal mechanical pace.

ACTION SEQUENCE — starting exactly from the reference image pose:
1. Scene opens with Jun already leaning forward, right hand already pressing the stop button on the control panel. One sheet is already sliding out of the output tray. The paper pile is already stacked on the floor.
2. Jun presses the stop button again — and again — with increasing urgency. Each press is a single deliberate finger tap on the panel.
3. The copier ignores the stop commands completely. It continues to eject exactly one sheet at a time, slowly and mechanically.
4. Jun's posture sinks lower. His shoulders drop. His expression shifts from alarmed urgency to exhausted disbelief.
5. By the final frames, Jun has slumped slightly, still pressing the button but with weakening energy — a portrait of helpless resignation.

Style: Cute semi-deformed 3D animated, adult office comedy. 9:16 vertical format.

STRICT RULES:
- The paper pile on the floor is present from frame one — it does NOT appear mid-scene
- No paper explosion, burst, shower, or flying sheets
- No sudden large pile growth — only one sheet added at a time via normal mechanical ejection
- No scene cuts or internal montage
- No different space, copier, or costume change
- No face closeup
- No new characters, no boss visible
- No text, subtitle, logo
- No white shirt (Jun wears light-blue shirt, red tie, navy slacks)
- Jun stays in front of the copier throughout — he does NOT walk away or give up
...
// ── 메인 ──────────────────────────────────────────────────────────────────────
async function main() {
  log(`=== S4 Veo generation (profile ${PROFILE_NUM}, port ${CDP_PORT}) ===`);
  log(`Reference: ${REF_IMG}`);
  log(`Output:    ${OUT_MP4}`);
  if (DRY) log("[DRY-RUN MODE] — 전송 없음, TOTAL=0, VEO=0");

  if (!fs.existsSync(REF_IMG)) abort("ref_missing", `S4 keyframe not found: ${REF_IMG}`);
  const refHash = fileHash(REF_IMG);
  log(`S4 ref hash: ${refHash} (${Math.round(fs.statSync(REF_IMG).size / 1024)}KB)`);

  // S4 reference 정합성 확인 (expected md5)
  const EXPECTED_HASH = "7efa35b8a681bd23d8dfe37ed32c92a5";
  if (refHash !== EXPECTED_HASH) {
    warn(`Hash mismatch: expected ${EXPECTED_HASH}, got ${refHash} — proceeding but verify`);
  } else {
    log(`S4 ref hash verified ✅ (${EXPECTED_HASH})`);
  }
  ...
  // ── 파일 직접 첨부 (트리거 없음 — S2/S3에서 검증된 패턴) ─────────────────
  ...
  else log("S4 reference image attached ✅");
  ...
  if (DRY) {
    ...
    log(`  Paper pile contract keyword: ${typed.includes("exists from the very first frame") ? "YES ✅" : "NO ⚠️"}`);
    log(`  No explosion keyword: ${typed.includes("No paper explosion") ? "YES ✅" : "NO ⚠️"}`);
  ...
  // ── 최종 전송 (정확히 1회) ────────────────────────────────────────────────
  recordSend("S4 Veo 최종 제출", true);  // TOTAL=1, VEO=1
  ...
  if (result === "saved" && savedPath) {
    ...
    console.log("Check: paper pile present from frame 1 (NOT appearing mid-scene)");
    console.log("Check: no paper explosion / burst / shower");
    console.log("Check: copier ejects one sheet at a time, mechanically");
    console.log("Check: Jun presses stop button repeatedly but copier ignores");
    console.log("Check: Jun's posture/expression transitions from alarm → exhausted resignation");
    console.log("Check: brown crossbody bag visible throughout");
    console.log("Check: no boss visible anywhere");
  } else if (result === "refusal") {
    console.log("\n-> Gemini refused. FAIL. Check prompt. Max attempts: 2.");
