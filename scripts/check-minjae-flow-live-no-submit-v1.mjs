...
check("credit fallback extracts only the numeric Google Flow credit pattern", /accountDialog \? await accountDialog\.innerText\(\) : await page\.locator\(["']body["']\)\.innerText\(\)/.test(source) && /Google Flow \(\?:크레딧\|credits\)/.test(source));
...
check("only observed read-only Flow POSTs join the telemetry allowlist", /general\\\.reportClientSideError/.test(source) && /v1:checkAppAvailability/.test(source) && /v1:fetchUserRecommendations/.test(source) && /isAllowedReadOnlyPostRequest/.test(source));
