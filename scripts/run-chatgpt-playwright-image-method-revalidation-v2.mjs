/**
 * ChatGPT+Playwright Image Method Revalidation Runner v2 (revision)
 * task: golden-sample-chatgpt-playwright-visual-only-revision-v2
 *
 * v1 대비 변경:
 *   - v2 prompts fixture(신규 3장: problem / illusion 교체 / result) 소비, 제출 하드캡 8.
 *   - Owner 지적 반영 감지/저장 속도 규칙:
 *       · 제출 후 25초까지는 passive(중단 조건만 감시), 25초부터 1.8초 간격 active poll
 *       · page-wide estuary/oaiusercontent/blob 수집 (assistant-scope 신뢰 금지, user 첨부 제외)
 *       · 신규 생성 후보가 3회 연속 stable(같은 key + 같은 크기)이면 idle 기다리지 않고 즉시 저장
 *       · 150초 미감지: diagnostic screenshot/DOM summary 저장 + current-page recover 1회
 *         (unrelated sidebar conversation scan 금지)
 *       · 180초 회수 불가: TIMEOUT_BLOCKED 기록 후 다음 prompt 진행
 *       · latency 로그 필수: submittedAt / firstCandidateAt / savedAt / detectionLatency
 *       · 가시 생성 후 저장 지연 >30초면 report에 기록
 *   - generation-latency-report.v1.json 산출.
 *
 * 재생성 정책: 품질 미달 이미지는 기존 파일을 `<이름>.rejected-rN.png`로 수동 rename 후
 * 재실행하면 해당 order만 다시 제출된다 (existing-file skip이 최종 파일명 기준이므로).
 * 자동 retry 루프 없음 — 하드캡 8은 절대 초과 불가.
 *
...
const PROMPTS_PATH = path.join(REPO_ROOT, "scripts", "fixtures", "chatgpt_playwright_image_prompts.t1_lifestyle_inflation.v2.json");
const OUT_DIR_ABS = path.join(REPO_ROOT, "output", "money-shorts", "chatgpt-playwright-image-method-revalidation-v2");

const SUBMISSION_HARD_CAP = 8;
...
function log(m) { console.log(`[${ts()}][gpt-reval-v2] ${m}`); }
function warn(m) { console.warn(`[WARN][gpt-reval-v2] ${m}`); }
...
if (promptsDoc.schemaVersion !== "chatgpt_playwright_image_prompts_v2") {
...
if (prompts.length < 1 || prompts.length > 8) {
  console.error(`ABORT: prompts 1~8개 기대 — 실제 ${prompts.length}개`);
  process.exit(1);
}
...
function writeSummary(generated, latencyEntries, fatalError = null) {
  const summary = {
    schemaVersion: "chatgpt_playwright_image_method_revalidation_summary_v2",
    taskId: "golden-sample-chatgpt-playwright-visual-only-revision-v2",
    sourcePrompts: "scripts/fixtures/chatgpt_playwright_image_prompts.t1_lifestyle_inflation.v2.json",
  ...
  fs.writeFileSync(path.join(OUT_DIR_ABS, "image-generation-summary.v2.json"), JSON.stringify(summary, null, 2));
  const latReport = {
  ...
  fs.writeFileSync(path.join(OUT_DIR_ABS, "generation-latency-report.v1.json"), JSON.stringify(latReport, null, 2));
  log(`summary + latency report 저장 (${OUT_DIR_ABS})`);
  return summary;
}

async function main() {
  log("=== v2 revision image generation ===");
...
main().catch((e) => {
  console.error("[FATAL][gpt-reval-v2]", e.message);
  process.exit(1);
});
