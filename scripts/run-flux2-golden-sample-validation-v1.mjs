/**
 * FLUX.2 [pro] Golden Sample Small Validation v1
 *
 * 목적:
 *   OpenAI recommended 3장 benchmark(golden_sample_visual_quality_benchmark.v1.json) 대비
 *   FLUX.2 [pro]가 강화 no-text/blank-object/1088x1936 native 계약을 따라오는지 소량 실측한다.
 *   자동화 구현이 아니라 소량 validation이다. render/TTS/mux/upload 없음.
 *
 * Owner 승인 (2026-07-02):
 *   FLUX2 소량 validation 승인 — FLUX.2 [pro] 최대 4장, 비용 $1 상한,
 *   BFL_API_KEY 사용 허용, render/TTS/mux/upload 없이 이미지 QA 보고 후 중단
 *
 * 안전 규칙:
 *   - FLUX.2 [pro] 단일 provider. OpenAI/ChatGPT/Gemini/Midjourney 경로 없음.
 *   - 공식 단일 endpoint https://api.bfl.ai/v1/flux-2-pro — endpoint fallback 없음.
 *   - create call hard cap = 4. size fallback 없음, 이미지 추가 생성 retry 없음.
 *   - polling call은 create call과 분리 집계 (과금 아님).
 *   - .env.local에서 BFL_API_KEY만 읽고 나머지는 파싱 즉시 버린다. 값 출력/기록 금지.
 *   - width=1088, height=1936 고정.
 *
 * 사용: node scripts/run-flux2-golden-sample-validation-v1.mjs
...
const PROMPTS_PATH = path.join(REPO_ROOT, "scripts", "fixtures", "golden_sample_flux2_validation_prompts.v1.json");
const OUT_DIR = path.join(REPO_ROOT, "output", "money-shorts", "flux2-small-validation-v1");
...
function log(m) { console.log(`[${ts()}][flux2-validation] ${m}`); }
function warn(m) { console.warn(`[WARN][flux2-validation] ${m}`); }
...
async function main() {
  ...
  log(`BFL_API_KEY 존재 확인 (값 미출력) — FLUX.2 [pro] validation 시작`);

  const promptsDoc = JSON.parse(fs.readFileSync(PROMPTS_PATH, "utf8"));
  if (promptsDoc.schemaVersion !== "money_shorts_flux2_validation_prompts_v1" || (promptsDoc.prompts || []).length !== 4) {
  ...
  log(`out-dir: ${OUT_DIR}`);
  log(`endpoint: ${FLUX_ENDPOINT} (단일, fallback 없음) size: ${SIZE.w}x${SIZE.h} (고정)`);
  ...
  for (const p of prompts) {
    const rec = { order: p.order, sceneRole: p.sceneRole, attempted: false, generated: false };
    ...
    rec.attempted = true;
    createCalls++; // create 1회/프롬프트 — 실패해도 재생성 retry 없음
    try {
      log(`scene ${p.order} (${p.sceneRole}) create call ${createCalls}/${MAX_CREATE_CALLS} — ${SIZE.w}x${SIZE.h}`);
      const resp = await fetch(FLUX_ENDPOINT, {
        method: "POST",
        headers: { "x-key": BFL_KEY, "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: p.finalPrompt, width: SIZE.w, height: SIZE.h }),
      ...
      if (!resp.ok) {
        ...
        warn(`scene ${p.order} create 거부 — retry/fallback 없이 기록: ${rec.error}`);
        results.push(rec);
        continue;
      }
      ...
      if (!sampleUrl) {
        ...
        rec.gate = "FAIL_PROVIDER_BLOCKED";
        warn(`scene ${p.order} 결과 없음 — 추가 생성 없이 기록: ${rec.error}`);
        results.push(rec);
        continue;
      }
      ...
      const file = path.join(OUT_DIR, `flux2-scene-${String(p.order).padStart(2, "0")}-${p.sceneRole}.${ext}`);
      ...
      rec.gate = "FAIL_PROVIDER_BLOCKED";
      warn(`scene ${p.order} 실패 — 추가 생성 없이 기록: ${rec.error}`);
    }
    results.push(rec);
  }
  ...
  const summary = {
    schemaVersion: "money_shorts_flux2_small_validation_summary_v1",
    taskId: "creative-v2-flux2-small-validation-v1",
    ...
    benchmarkRef: promptsDoc.benchmarkRef,
    results,
    createdAt: new Date().toISOString(),
    secretValuesLogged: false,
  };
  const outPath = path.join(OUT_DIR, "summary-flux2-small-validation.json");
...
main().catch((e) => {
  console.error("[FATAL][flux2-validation]", String(e?.message || e).slice(0, 300));
  process.exit(1);
});
