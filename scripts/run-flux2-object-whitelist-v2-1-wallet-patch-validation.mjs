/**
 * FLUX.2 [pro] Object-Whitelist v2.1 Wallet-Emboss Patch Validation
 *
 * 목적:
 *   v2 검증에서 잔존한 지갑 emboss/stamp 단일 클래스를 scene 1/5만 대상으로 패치 재검증한다.
 *   scene 1: 각인 제거 + 월급봉투 hook 전달력 유지 (지갑 soft-focus 강등).
 *   scene 5: emboss 제거 + 빈 지갑 내부 공허 정면 강조 강화 (카메라 틸트 재지시).
 *   자동화 구현이 아니라 소량 패치 검증이다. render/TTS/mux/upload 없음.
 *
 * Owner 승인 (2026-07-02):
 *   FLUX2 object-whitelist v2.1 지갑 emboss 패치 재검증 승인 — FLUX.2 [pro] 최대 2장,
 ...
 *   - scene 1/5만 생성. scene 2/3/4/6은 코드 수준에서 차단.
 *   - polling call은 create call과 분리 집계 (과금 아님).
 *   - .env.local에서 BFL_API_KEY만 읽고 나머지는 파싱 즉시 버린다. 값 출력/기록 금지.
 *   - width=1088, height=1936 고정.
 *   - prompt source of truth = v2.1 patch fixture의 finalPromptV2_1 (없으면 실행 전 abort).
 *
 * 사용: node scripts/run-flux2-object-whitelist-v2-1-wallet-patch-validation.mjs
...
const FIXTURE_PATH = path.join(REPO_ROOT, "scripts", "fixtures", "golden_sample_flux2_object_whitelist_v2_1_wallet_patch_prompts.json");
const OUT_DIR = path.join(REPO_ROOT, "output", "money-shorts", "flux2-object-whitelist-v2-1-wallet-patch-validation");
...
const TARGET_SCENES = [1, 5]; // scene 2(v1 PASS)/3(v2 PASS)/4(v2 PASS)/6(후순위) 생성 금지
const FORBIDDEN_SCENES = new Set([2, 3, 4, 6]);
const GATE = { minW: 1080, minH: 1920, aspectTarget: 0.5625, aspectTol: 0.01 };

function ts() { return new Date().toISOString().slice(11, 19); }
function log(m) { console.log(`[${ts()}][flux2-wl-v2.1] ${m}`); }
function warn(m) { console.warn(`[WARN][flux2-wl-v2.1] ${m}`); }
...
async function main() {
  ...
  log("BFL_API_KEY 존재 확인 (값 미출력) — wallet-emboss patch v2.1 validation 시작");

  const fixture = JSON.parse(fs.readFileSync(FIXTURE_PATH, "utf8"));
  if (fixture.schemaVersion !== "money_shorts_flux2_object_whitelist_v2_1_wallet_patch_prompts") {
    console.error("ABORT: v2.1 patch fixture schemaVersion 불일치");
    process.exit(1);
  }
  const scenes = TARGET_SCENES
    .map(o => (fixture.scenePatches || []).find(s => s.order === o))
    .filter(Boolean)
  ...
  for (const s of scenes) {
    ...
    if (typeof s.finalPromptV2_1 !== "string" || s.finalPromptV2_1.length < 200) {
      console.error(`ABORT: scene ${s.order} finalPromptV2_1이 없음/비정상 — patch fixture 확인 필요`);
      process.exit(1);
    }
  }
  ...
  for (const s of scenes) {
    ...
    try {
      ...
      const resp = await fetch(FLUX_ENDPOINT, {
        ...
        body: JSON.stringify({ prompt: s.finalPromptV2_1, width: SIZE.w, height: SIZE.h }),
      ...
      const file = path.join(OUT_DIR, `flux2wl21-scene-${String(s.order).padStart(2, "0")}-${s.sceneRole}.${ext}`);
  ...
  const summary = {
    schemaVersion: "money_shorts_flux2_object_whitelist_v2_1_wallet_patch_validation_summary",
    taskId: "creative-v2-flux2-object-whitelist-v2-1-wallet-emboss-validation",
    ...
    excludedScenes: { "2": "v1 PASS 보유", "3": "v2 PASS 보유", "4": "v2 PASS 보유", "6": "후순위 (최저 위험)" },
    promptSource: "scripts/fixtures/golden_sample_flux2_object_whitelist_v2_1_wallet_patch_prompts.json scenePatches[].finalPromptV2_1",
    createCalls,
  ...
  const outPath = path.join(OUT_DIR, "summary-flux2-object-whitelist-v2-1-wallet-patch-validation.json");
...
main().catch((e) => {
  console.error("[FATAL][flux2-wl-v2.1]", String(e?.message || e).slice(0, 300));
  process.exit(1);
});
