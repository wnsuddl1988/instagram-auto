...
const PROMPTS_PATH = path.join(REPO_ROOT, "scripts", "fixtures", "chatgpt_playwright_image_prompts.t1_lifestyle_inflation.v3.json");
const OUT_DIR_ABS = path.join(REPO_ROOT, "output", "money-shorts", "chatgpt-playwright-fresh-image-set-v3");

const SUBMISSION_HARD_CAP = 12;
...
function log(m) { console.log(`[${ts()}][gpt-fresh-v3] ${m}`); }
function warn(m) { console.warn(`[WARN][gpt-fresh-v3] ${m}`); }
...
if (promptsDoc.schemaVersion !== "chatgpt_playwright_image_prompts_v3") {
...
if (prompts.length < 1 || prompts.length > 12) {
  console.error(`ABORT: prompts 1~12개 기대 — 실제 ${prompts.length}개`);
  process.exit(1);
}
...
function writeSummary(generated, latencyEntries, fatalError = null) {
  const summary = {
    schemaVersion: "chatgpt_playwright_fresh_image_set_summary_v3",
    taskId: "golden-sample-chatgpt-playwright-fresh-image-set-visual-only-v3",
    sourcePrompts: "scripts/fixtures/chatgpt_playwright_image_prompts.t1_lifestyle_inflation.v3.json",
  ...
  fs.writeFileSync(path.join(OUT_DIR_ABS, "image-generation-summary.v3.json"), JSON.stringify(summary, null, 2));
  const latReport = {
  ...
  fs.writeFileSync(path.join(OUT_DIR_ABS, "generation-latency-report.v3.json"), JSON.stringify(latReport, null, 2));
  log(`summary + latency report 저장 (${OUT_DIR_ABS})`);
  return summary;
}

async function main() {
  log("=== v3 fresh image set generation ===");
...
main().catch((e) => {
  console.error("[FATAL][gpt-fresh-v3]", e.message);
  process.exit(1);
});
