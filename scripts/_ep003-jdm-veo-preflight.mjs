...
const REF_FILES = {
  ...
  S3: path.join(ROOT, "output/v2/ep003_jdm/keyframes/s3_kf_try1.png"),
...
// ── S3 Boss Rule 필수 키워드 (prompt 내 존재 확인) ──────────────────────────────
const S3_BOSS_RULE_KEYWORDS = [
  { key: "above the elbow",                            label: "팔꿈치 이상 금지" },
  { key: "reflection in the glass",                    label: "유리 반사 금지" },
  { key: "do NOT pan or move toward the inside",       label: "카메라 이동 금지" },
  { key: "No face, head, neck, shoulder, torso",       label: "Boss 신체 금지" },
  { key: "silhouette",                                 label: "실루엣 금지" },
];
