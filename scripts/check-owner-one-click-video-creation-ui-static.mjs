...
check("TTS runner supports flow-derived 4~18 scenes with legacy one-call and Minjae three-call budgets", /scenes\.length < 4/.test(ttsScriptSrc) && /scenes\.length > 18/.test(ttsScriptSrc) && /LEGACY_API_CALL_BUDGET_MAX\s*=\s*1/.test(ttsScriptSrc) && /THREE_PHASE_API_CALL_BUDGET_MAX\s*=\s*3/.test(ttsScriptSrc));
check("TTS sends continuous text or three aligned phases with character timestamps", /with-timestamps\?output_format=mp3_44100_128/.test(ttsScriptSrc) && /continuousText/.test(ttsScriptSrc) && /voicePhasePlan/.test(ttsScriptSrc) && /audio_base64/.test(ttsScriptSrc) && /character_start_times_seconds/.test(ttsScriptSrc));
...
check("real TTS readiness requires current full input hash and exact Minjae phase audit", /ttsInputContractCurrent/.test(helperCode) && /ttsInputContractSha256/.test(helperCode) && /phaseAuditReady/.test(helperCode) && /requestedTagApplied/.test(helperCode));
...
check("TTS reuses matching continuous or phase audio/alignment without a paid retry", /existsSync\(rawPath\) && existsSync\(alignmentCachePath\)/.test(ttsScriptSrc) && /reused_continuous_aligned/.test(ttsScriptSrc) && /reused_three_phase_aligned/.test(ttsScriptSrc));
