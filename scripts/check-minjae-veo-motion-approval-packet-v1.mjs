...
const retryResult = spawnSync(process.execPath, [builderPath, "--retry-v2"], { encoding: "utf8" });
const retryPacket = retryResult.status === 0 ? JSON.parse(retryResult.stdout) : null;
let passed = 0;
let failed = 0;
...
check("packet pins and verifies Gemini 3.1 Pro before the Video tool", packet?.providerUi?.reasoningModelTarget === "3.1 Pro" && packet?.providerUi?.flashAllowedForThisRetry === false && /verify selected/i.test(packet?.providerUi?.reasoningModelSelectionPolicy ?? ""));
...
check("retry v2 is isolated and requires a new explicit approval after the consumed rejected attempt", retryResult.status === 0 && retryPacket?.retryContext?.priorSubmissionConsumed === true && retryPacket?.retryContext?.newExplicitOwnerApprovalRequired === true && /pilot-retry-v2/i.test(retryPacket?.outputPlan?.root ?? "") && /APPROVE_LIVE_GEMINI_VEO_RETRY/.test(retryPacket?.liveBoundary?.requiredOwnerApprovalWording ?? ""));
