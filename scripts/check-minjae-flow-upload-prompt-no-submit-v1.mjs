...
check("only the asset picker and media upload controls are clicked", JSON.stringify(clickTargets) === JSON.stringify(["assetPickerButton", "mediaUploadButton"]));
...
check("reference attachment is bounded to one selection and one upload request", /uploadSelectionCount \+= 1/.test(runnerSource) && /uploadRequestCount === 0/.test(runnerSource) && /uploadRequestCount === 1/.test(runnerSource));
