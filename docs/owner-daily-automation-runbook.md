...
## 새 영상(future new video)의 실제 순서: dry-run → manifest → media 첨부 → preflight

새 영상 하나를 실제로 다루는 practical한 순서는 다음과 같다(이 slice는 1~2단계까지가
자동화되어 있고, 3~4단계는 별도 승인된 후속 작업, 5단계는 이미 존재하는 preflight다):
...
4. **Blob liveness evidence 첨부** (승인된 Blob upload/liveness 이후): `--blob-liveness-result
   ...
   검증한다.
5. **`--preflight --content-unit <manifest>` 실행**: 전체 readiness gate 결과를 확인한다.

이 slice에서는 1~2단계만 자동화되어 있으며, 업로드/게시/새 외부 미디어 생성은 전혀
수행하지 않는다.
