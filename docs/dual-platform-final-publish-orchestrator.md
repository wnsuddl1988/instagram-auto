...
| live | `--live` / `--arm` | armed (fail-closed) | 6단계 gate 순서로 fail-closed 평가. default content(v3_2)는 gate 4 duplicate guard가 `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`(exit 3)로 차단(credential 미접근). custom content는 gate 1~4 통과 시 credential resolution(gate 5)에 도달해 승인 6개 env key를 in-memory로 조립한 뒤, actual API 실행(gate 6)이 비활성이라 `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`(exit 4)로 fail-closed(credential key 누락 시 `CREDENTIAL_KEYS_MISSING_THIS_SLICE`, exit 4). 어느 경로든 actual API call 미도달, credential 값 미노출, API/upload/blob/ledger/video side-effect 0. |
...
### custom content의 live 실행 (credential resolution 조립까지 도달, actual API 실행은 fail-closed)

task: `dual-platform-credential-resolution-wiring-no-execute-v1`
(이전: `dual-platform-custom-content-live-credential-gate-no-execute-v1`)
...
gate 1~4를 **모두 통과하면** credential resolution(gate 5)에 **도달**하며, 이제 이 단계가 **wiring됐다**:
...
- credential이 모두 present면 `credentialValuesResolved: true`가 되고, actual API 실행(gate 6)이 이
  slice에서 비활성이라 **`ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`(exit 4)**로 fail-closed된다.
- credential key가 하나라도 비어 있으면 `credentialValuesResolved: false`가 되고
  **`CREDENTIAL_KEYS_MISSING_THIS_SLICE`(exit 4)**로 fail-closed되며, 출력에는 `missingCredentialKeyNames`
  (누락 key '이름' 배열)만 담긴다.
- 어느 경로든 actual API call(gate 6)에는 **도달하지 않고**(`actualApiCallReached: false`),
...
→ `BLOCKED_BLOB_LIVENESS_EVIDENCE`(gate 3, exit 3)). 이 경우 `credentialValuesAccessed: false`다.
실제 API 호출을 통한 새 콘텐츠 배포(gate 6)는 여전히 별도 승인 slice에서만 wiring된다 —
이 slice의 실제 publish는 비활성이다.
...
| custom non-default, gate 1~4 통과 + credential 전부 present | `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE` (gate 5 resolved) | 4 | `actual_api_call_not_enabled_this_slice` |
| custom non-default, gate 1~4 통과 + credential 일부/전부 누락 | `CREDENTIAL_KEYS_MISSING_THIS_SLICE` (gate 5, 누락 이름만) | 4 | `actual_api_call_not_enabled_this_slice` |
| custom non-default, source 미존재 | `BLOCKED_SOURCE_FILE_MISSING` (gate 2) | 3 | `actual_api_call_not_enabled_this_slice` |
| custom non-default, blob evidence 부재 | `BLOCKED_BLOB_LIVENESS_EVIDENCE` (gate 3) | 3 | `actual_api_call_not_enabled_this_slice` |

**핵심 안전 순서 불변**: gate 4 `duplicate_publish_guard`는 여전히 gate 5
`credential_presence_resolution`보다 먼저 평가된다(정적 가드가 소스 순서 검증). credential
resolver는 승인된 6개 key 외의 임의 `process.env` 값을 읽지 않고, 어떤 live client lib도
import/호출하지 않으며, actual API/upload/blob mutation을 수행하지 않는다.

**주의(task: dual-platform-content-unit-manifest-block-reason-fix-v1 → credential-resolution-wiring)**:
`liveExecutionPlan`(및 각 step)의 blocked 사유는 콘텐츠 종류에 따라 위 표대로 다르게 표시된다.
custom(비중복) 새 콘텐츠를 duplicate 사유로 표시하면 Owner가 "중복이라 막힌 것"으로 오인할 수 있어
콘텐츠 종류별로 정확히 분기했다 — `currentContentDuplicateBlocked`도 default+양쪽 already-published일
때만 `true`이고, custom content는 duplicate 여부와 무관하게 `false`다. credential resolution이 wiring되면서
custom content의 실제 최종 halt 사유는 이제 `actual_api_call_not_enabled_this_slice`다.
...
5. custom content actual API 실행 wiring — credential resolution(gate 5)은 이 slice에서 완료됨(in-memory 조립, no-execute). 남은 것은 gate 6 actual API 호출(Instagram Graph publish / YouTube upload / Blob upload) 활성화이며, 별도 승인 slice에서만 연결된다.
