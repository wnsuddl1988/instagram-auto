...
| preflight | `--preflight` | ✕ | 준비 검증. plan + 파일경로 존재여부 + gate + guard + Blob liveness evidence + arm 상태(`preflight.liveArm`) + required env key **이름** 계약(process.env 미접근). |
| credential-preflight | `--credential-preflight` | ✕ | redacted env key presence check. 승인된 runtime env key **이름**의 present boolean만 보고(값/길이/prefix/suffix/hash 미노출). `process.env`는 `Boolean(process.env[keyName])` presence 판정에만 접근하고 값은 노출하지 않는다. `.env.local`/secret 파일 read 없음, API/Blob 호출 없음. env key가 없어도 exit 0(status-style). `readyForCredentialResolution:true`는 presence 신호일 뿐 실제 publish 활성화가 아니다. |
| live | `--live` / `--arm` | armed (fail-closed) | 6단계 gate 순서로 fail-closed 평가. default content(v3_2)는 gate 4 duplicate guard가 `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`(exit 3)로 차단(credential/gate 6 미도달, `actualApiCallReached:false`). custom content는 gate 1~4 통과 시 credential resolution(gate 5)에 도달해 승인 6개 env key를 in-memory로 조립한다(값은 미노출). 6개 key 전부 present면 **gate 6 `actual_api_call` 구조에 도달**해(`actualApiCallReached:true`) value-free no-execute call plan을 구성하지만, 실제 API 실행은 비활성이라(`actualApiCallExecutionEnabledThisSlice:false`, `actualApiCallPerformed:false`) `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`(exit 4)로 fail-closed된다. credential key 누락 시에는 gate 5에서 `CREDENTIAL_KEYS_MISSING_THIS_SLICE`(exit 4)로 fail-closed되며 gate 6에는 도달하지 않는다(`actualApiCallReached:false`). 어느 경로든 credential 값 미노출, 실제 Instagram/YouTube/Blob API 호출·upload·OAuth·mutation 0, side-effect counter 0. |
...
### credential-preflight 모드 (`--credential-preflight`) — redacted presence, no values

task: `dual-platform-credential-preflight-redacted-no-live-v1`

실제 live publish 직전에 **필요한 runtime env key 이름이 present인지**를 값 없이 확인하는 no-live 진단 모드다.
`--preflight`가 env key **이름 계약**만 출력하고 `process.env`를 전혀 읽지 않는 것과 달리, 이 모드는 승인된
key 이름의 **present boolean**을 판정하기 위해 `process.env`에 접근한다. 단, 값은 절대 노출하지 않는다.

- **접근 방식**: `Boolean(process.env[keyName])` — 값을 지역 변수에 바인딩하지 않고 truthiness만 판정한다.
  미설정(undefined)/빈 문자열은 `present:false`, 비어있지 않은 값은 `present:true`. 값 자체나 길이/prefix/
  suffix/hash/sample/token type 등 값에서 파생된 어떤 정보도 계산·출력·저장하지 않는다.
- **검사 대상 key**(승인된 6개, `REQUIRED_ENV_KEY_NAMES`):
  - Instagram: `INSTAGRAM_BUSINESS_ACCOUNT_ID`, `INSTAGRAM_ACCESS_TOKEN`
  - YouTube: `YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET`, `YOUTUBE_REFRESH_TOKEN`
  - Vercel Blob: `BLOB_READ_WRITE_TOKEN`
- **출력**(JSON): `mode:"credential_preflight"`, `contentId`/`version`/`isDefaultContentUnit`,
  `credentialValuesAccessed:false`, `credentialValuesResolved:false`, `credentialValuesPrinted:false`,
  `dotEnvLocalDirectAccess:false`, `externalApiCallPerformed:false`, `requiredEnvKeyNames`,
  `platforms.{instagram,youtube,vercelBlob}.keys[]`(각 `{name, present}`)+`allPresent`,
  `allRequiredKeysPresent`, `readyForCredentialResolution`.
- **exit code**: status-style diagnostic이라 **missing key가 있어도 exit 0**. 정보는 JSON boolean으로만 표현한다.
- **safety**: `.env`/`.env.local`/secret 파일을 읽지 않고(dotenv 미사용), Instagram/YouTube/Blob API/OAuth/
  upload/HEAD, ffmpeg, deploy를 호출하지 않는다. `optional --content-unit <manifest>`는 context 필드
  (`contentId`/`version`/`isDefaultContentUnit`)에만 영향을 주고 env presence 판정 로직은 바꾸지 않는다.
- **오해 방지**: `present:true` 및 `readyForCredentialResolution:true`는 "해당 env key 이름이 이 프로세스 env에
  비어있지 않게 존재함"만을 뜻한다. credential resolution 코드 경로는 이 slice에서 wiring됐지만
  (`credentialResolutionWiredThisSlice:true`), **이 `--credential-preflight` 모드 자체는 credential 값을
  읽지 않으며**(`credentialValuesAccessedInThisMode:false`) **actual API 실행/live publish는 비활성**이다
  (`actualApiExecutionEnabledThisSlice:false`). 즉 `credentialResolutionWiredThisSlice:true`를 publish
  활성화 신호로 오인해서는 안 된다. 실제 credential 값 접근은 `--live` 실행 경로의 gate 5에서만 발생하며,
  그 경우에도 actual API call(gate 6)이 비활성이라 실제 publish는 일어나지 않는다.
- **owner 위임**: `node scripts/run-owner-daily-automation-entrypoint.mjs --credential-preflight [--content-unit <path>]`
  (`pnpm owner:credential-preflight`)가 이 모드를 `spawnSync(process.execPath, …, { shell:false })`로 위임한다.
  owner entrypoint 자체는 `process.env`를 직접 읽지 않고 orchestrator 결과의 redacted presence만 요약한다.
...
- gate 4 `duplicate_publish_guard`는 gate 5 `credential_presence_resolution`보다 **반드시 먼저**
  평가된다(소스 순서를 정적 가드가 검증). default current content는 4에서 차단되어 5/6에 도달하지 않는다.
- gate 5 `credential_presence_resolution`은 이 slice에서 **wiring됐다**
  (`credentialResolutionWiredThisSlice: true`). gate 1~4를 통과한 비중복 신규(custom) 콘텐츠는 이
  단계에 **도달**해(`credentialResolutionReached: true`) `resolveExplicitCredentialsFromRuntimeEnv()`가
  승인된 6개 runtime env key만 읽어 in-memory explicit credential 객체(Instagram/YouTube/Vercel Blob)를
  조립한다(`credentialValuesAccessed: true`). 값은 **로컬 in-memory 객체에만** 존재하며 출력/문서/
  fixture/report/error/gate trace에 **절대 노출되지 않는다**(값의 길이/prefix/suffix/hash/masked/token
  type도 계산·출력하지 않는다).
  - credential이 모두 present면 `credentialValuesResolved: true`가 되고, actual API 실행(gate 6)이
    이 slice에서 비활성이라 **`ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`(exit 4)**로 fail-closed된다.
  - credential key가 하나라도 비어 있으면 `credentialValuesResolved: false`가 되고
    **`CREDENTIAL_KEYS_MISSING_THIS_SLICE`(exit 4)**로 fail-closed되며, 출력에는 `missingCredentialKeyNames`
    (누락 key '이름' 배열)만 담긴다.
  - 어느 경로든 actual API call(gate 6)에는 **도달하지 않고**, 모든 live side-effect counter는 0으로
    유지된다(`credentialValuesAccessedCount`/`credentialValuesResolvedCount`는 이 단계 도달 시 1이 될 수
    있으나 API/upload/blob/ledger/video counter는 반드시 0). 실제 API 실행은 별도 승인 slice에서만 연결된다.
...
### custom content의 live 실행 (gate 6 no-execute plan + no-run executor + arm-ready no-run dispatcher까지 도달, actual API 실행은 fail-closed)

task: `dual-platform-executor-execution-wiring-no-run-to-arm-ready-v1`
(이전: `dual-platform-actual-api-executor-wiring-no-run-v1` → `...-actual-api-call-wiring-no-execute-...` → `...-credential-resolution-wiring-...`)
...
gate 1~4를 **모두 통과하면** credential resolution(gate 5)에 **도달**한다:
`resolveExplicitCredentialsFromRuntimeEnv()`가 승인된 6개 runtime env key만 읽어 in-memory explicit
credential 객체(Instagram/YouTube/Vercel Blob)를 조립한다(`credentialResolutionReached: true`,
`credentialValuesAccessed: true`). credential 값은 **로컬 in-memory 객체에만** 존재하며 출력/문서/
fixture/report/error/gate trace에 노출되지 않는다(값 길이/prefix/suffix/hash/masked/token type도 없음).

- credential이 모두 present면 `credentialValuesResolved: true`가 되고, **gate 6 `actual_api_call`
  구조에 도달**한다(`actualApiCallReached: true`). `buildActualApiCallPlanNoExecute()`가 value-free
  no-execute call plan(`actualApiCallPlan`)을 구성한다 — Blob upload / Instagram Graph publish /
  YouTube direct upload 3개 call spec(각 `functionRef` 문자열 + source/URL/metadata/credential
  **presence boolean** + `executionEnabled: false` + 실행 비활성 사유). 이 plan은 credential **값**을
...
- 이어서 `buildActualApiDispatcherNoRun()`이 위 executor를 소비해 **arm-ready no-run dispatcher 구조**
  (`actualApiDispatcher`)를 구성한다(`actualApiDispatcherReached: true`). dispatcher는 "실제 실행이 활성화되면
  dispatcher가 어떤 순서로, 어떤 adapter를 target으로, 무엇에 의존해 dispatch할지"의 **실행 경계(execution
  boundary)**를 명문화한다. executor와 동일한 4-step ordered 구조(`instagram_blob_upload` →
  `instagram_publish_reel`(dependsOn: blob) → `youtube_direct_upload`(독립) → `publish_ledger_record`
  (dependsOn: ig publish + yt upload))이며, 각 dispatch step은 `adapterTarget` **문자열 메타데이터**(실제
  import/참조 아님), `dependsOn`, `dependencySatisfiedInStructure`(구조 존재성 boolean), executor에서 **복사만**
  한 입력 readiness/credential presence boolean, `requiredApprovalTokens`,
  `dispatchEnabled: false`/`willDispatch: false`/`dispatched: false`/`performed: false`/disabled 사유를 담는다.
  `executorAccepted`는 executor가 기대한 4-step disabled 구조일 때만 true다. dispatcher는 credential **값**을
  받지 않고 executor(값 없음)만 받으며(값 미유입), dispatch 플래그는 모두 false다
  (`dispatchEnabledThisSlice: false`, `dispatcherWillRun: false`, `dispatcherPerformed: false`,
  `actualDispatcherRunThisRun: false`). ledger dispatch step은 string adapterTarget만 담고 실제 dispatch/
  mutation을 하지 않는다.
- credential key가 하나라도 비어 있으면 `credentialValuesResolved: false`가 되고, **gate 6에 도달하지
  ...
  `actualApiExecutorReached: false`, `actualApiDispatcherReached: false`).
- 어느 경로든 실제 Instagram/YouTube API 호출·OAuth·Blob mutation·ledger mutation·dispatch는 0이고,
...
`actualApiCallReached: false`, `actualApiExecutorReached: false`, `actualApiDispatcherReached: false`다.
실제 API **실행**(gate 6 call/executor/dispatcher 수행)은 여전히 별도 승인 slice에서만 활성화된다 — 이 slice는
call plan + no-run executor + arm-ready no-run dispatcher 구조까지만 wiring한다.

| content unit | `--live`/`--arm` 결과 | exit | gate 6 plan / executor / dispatcher |
|--------------|----------------------|------|--------------------------------------|
| default evidence (v3_2) | `BLOCKED_DUPLICATE_ALREADY_PUBLISHED` (gate 4) | 3 | 미도달 (plan/executor/dispatcher 없음) |
| custom non-default, gate 1~4 통과 + credential 전부 present | `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE` (gate 6) | 4 | plan + no-run executor + arm-ready no-run dispatcher 구성, 실행 비활성(`reached:true`, `executionEnabled/dispatchEnabled:false`) |
| custom non-default, gate 1~4 통과 + credential 일부/전부 누락 | `CREDENTIAL_KEYS_MISSING_THIS_SLICE` (gate 5, 누락 이름만) | 4 | 미도달 (plan/executor/dispatcher 없음) |
| custom non-default, source 미존재 | `BLOCKED_SOURCE_FILE_MISSING` (gate 2) | 3 | 미도달 (plan/executor/dispatcher 없음) |
| custom non-default, blob evidence 부재 | `BLOCKED_BLOB_LIVENESS_EVIDENCE` (gate 3) | 3 | 미도달 (plan/executor/dispatcher 없음) |
...
→ gate 6 `actual_api_call`(plan → executor → dispatcher) 순서가 정적 가드로 강제된다. credential resolver는
승인된 6개 key 외의 임의 `process.env` 값을 읽지 않고, gate 6 plan builder / no-run executor builder /
arm-ready no-run dispatcher builder는 credential 값 객체를 인자로 받지 않으며(plan은 credSummary presence만,
executor는 plan의 presence boolean만, dispatcher는 executor(값 없음)만) `process.env`를 읽지 않는다. 넷 다
어떤 live client lib(instagram/youtube/@vercel/blob/googleapis/publish-ledger)도 import/호출하지 않고,
actual API/upload/OAuth/Blob mutation/ledger mutation/dispatch를 수행하지 않으며,
`fetch`/`googleapis`/`youtube.videos.insert`/Blob `put·list·head·del·copy`/ledger
...
수 있어 콘텐츠 종류별로 정확히 분기했다 — `currentContentDuplicateBlocked`도 default+양쪽 already-published일
때만 `true`이고, custom content는 duplicate 여부와 무관하게 `false`다. custom content의 실제 최종 halt
사유는 `actual_api_call_not_enabled_this_slice`다.
...
5. custom content actual API 실행 활성화 — credential resolution(gate 5), gate 6 actual_api_call **no-execute plan 구조**(`buildActualApiCallPlanNoExecute`, value-free call spec), 그 plan을 소비한 **no-run executor 구조**(`buildActualApiExecutorNoRun`, Blob upload → Instagram publish → YouTube upload → ledger record의 4-step ordered/dependsOn 명문화), 그리고 executor를 소비한 **arm-ready no-run dispatcher 구조**(`buildActualApiDispatcherNoRun`, dispatch 순서/adapter target/의존/입력 readiness의 실행 경계 명문화, `executorAccepted` 수용 게이트 포함)는 이 slice에서 완료됨. 남은 것은 dispatcher step을 실제 dispatch(Instagram Graph publish / YouTube upload / Blob upload / ledger 기록)로 **실행 활성화**하는 것이며, `APPROVE_VERCEL_BLOB_OBJECT_UPLOAD_TEST` / `APPROVE_YOUTUBE_LIVE_UPLOAD_WIRING` / `APPROVE_DUAL_PLATFORM_ARM` 승인과 별도 slice에서만 연결된다.

## publish ledger 모듈 (durable 중복 방지 계약, no-live)

task: `publish-ledger-implementation-no-live-v1`

실제 dual-platform publish를 열기 전에 필요한 **durable duplicate ledger**를 `lib/publish-ledger.ts`로
구현했다. gate 6 dispatcher의 `publish_ledger_record` step이 참조하는 문자열
`lib/publish-ledger.ts#recordDualPlatformPublish`가 이제 실제 함수를 가리킨다. 이 slice는
**no-live/no-external**이다 — 로컬 filesystem JSON read/write만 하고, 외부 API/OAuth/Blob/upload/deploy/
media 부작용이 없으며, caller가 명시적으로 넘긴 `ledgerPath`에서만 동작한다(default/production 경로 없음).

**public API (guard가 lock)**:

| export | 역할 |
|--------|------|
| `PUBLISH_LEDGER_SCHEMA_VERSION` | `"publish_ledger_v1"` — read 시 정확히 일치해야 함 |
| `buildPublishLedgerKey({contentId, platform, version})` | 중복 키 `${contentId}/${platform}/${version}` 생성(orchestrator와 동일 shape) |
| `createEmptyPublishLedger()` | 빈 ledger(`{schemaVersion, records:[]}`) |
| `readPublishLedger(ledgerPath)` | 명시적 path에서 read. fail-closed |
| `writePublishLedger(ledgerPath, ledger)` | 명시적 path에만 deterministic JSON write |
| `checkPublishLedgerDuplicate(ledger, {contentId, platform, version})` | 중복 판정(`alreadyPublished`/`key`/`record`) |
| `recordPublishLedgerEntry(ledger, entry)` | 단일 record 삽입(중복이면 `duplicateBlocked`) |
| `recordDualPlatformPublish(ledger, input)` | 양 플랫폼 게시 결과를 한 번에 기록(future-facing) |

**read fail-closed 계약**:

- 파일 없음 → 빈 ledger + `ok:true` + `existed:false` (throw 없음 — 아직 아무것도 게시되지 않은 정상 상태)
- invalid JSON → `ok:false` + `reason:"invalid_json"`
- non-object / wrong `schemaVersion` / non-array `records` / invalid record shape / 중복 key →
  `ok:false` + 해당 reason (빈 ledger 반환, **쓰기 없음**)

**write 계약**: caller가 `ledgerPath`를 명시해야만 쓴다(없으면 `missing_ledger_path` 거부). 그 path의 부모
디렉토리만 필요 시 생성한다. records를 key 기준 정렬해 **deterministic** 출력. append-only partial write
없음 — 같은 디렉토리에 temp file을 먼저 쓴 뒤 `renameSync`로 **atomic replace**한다(같은 filesystem 내
rename은 원자적이라 중단 시에도 기존 파일이 온전하거나 완전히 새 내용이며, 반쯤 쓰인 파일이 남지 않는다.
실패 시 temp file 정리). import 시점 write 없음.

**dual publish all-or-nothing**: `recordDualPlatformPublish`는 Instagram key와 YouTube key를 먼저 둘 다
계산해 원본 ledger에서 중복을 판정한다. 둘 중 **하나라도** 이미 있으면 어떤 record도 추가하지 않고 원본
ledger를 그대로 반환한다(`ok:false`, `anyDuplicateBlocked:true`, 각 platform result가 자신의 duplicate
상태를 정확히 표시). 둘 다 중복이 없을 때만 두 record를 함께 추가한 새 ledger를 반환한다(`ok:true`). 따라서
caller가 실패 결과 ledger를 write해도 한쪽만 남는 partial 기록이 생기지 않는다. record의 `metadata`는 두
플랫폼 모두 보존된다(non-secret identifier/fingerprint만).

**duplicate/저장 안전**: `checkPublishLedgerDuplicate`는 key + 매칭 record(non-secret)만 반환한다.
record는 `key`/`contentId`/`platform`/`version`/`variantId`/`publishedId`(public media/video id)/
`publishedUrl`/`status`/`publishedAtIso`/`metadata`(non-secret)만 담는다. access/refresh token, client
secret, API key, Blob token, OAuth 응답, raw env 값, 값 파생(length/prefix/suffix/hash/masked)은 **절대**
