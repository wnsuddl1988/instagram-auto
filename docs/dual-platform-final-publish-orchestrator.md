...
| live | `--live` / `--arm` | armed (fail-closed) | 6단계 gate 순서로 fail-closed 평가. default content(v3_2)는 gate 4 duplicate guard가 `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`(exit 3)로 차단. custom content는 gate 1~4 통과 시 credential resolution stub(gate 5)까지 도달한 뒤 `CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`(exit 4)로 fail-closed — 어느 경로든 credential 값 미접근, actual API call 미도달, side-effect 0. |
...
- **오해 방지**: `present:true` 및 `readyForCredentialResolution:true`는 "해당 env key 이름이 이 프로세스 env에
  비어있지 않게 존재함"만을 뜻한다. 실제 credential resolution/API 실행은 이 slice에서 여전히 wiring되지
  않았으며(`credentialResolutionWiredThisSlice:false`), live publish가 활성화됐다는 신호가 아니다.
...
- gate 5는 이 slice에서 wiring되지 않은 fail-closed stub이다
  (`CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`, exit 4). gate 1~4를 통과한 비중복 신규(custom)
  콘텐츠는 이 stub까지 **도달**하지만(`credentialResolutionReached: true`) credential 값을 읽지
  않고(`credentialValuesAccessed: false`, `credentialValuesResolved: false`) 여기서 멈춘다 —
  actual API call(gate 6)에도 도달하지 않으며, 실제 credential resolution/API 실행은 별도 승인
  slice에서만 연결된다.
...
### custom content의 live 실행 (credential resolution까지 도달, 실제 publish는 fail-closed)

task: `dual-platform-custom-content-live-credential-gate-no-execute-v1`

이전 slice에서는 custom(non-default) content가 gate 4와 gate 5 사이의 무조건 halt(옛 gate 4.5,
`CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE`, exit 5)에서 멈췄다. 이 halt는 **제거됐다**.

이제 custom content unit으로 `--live`/`--arm`을 시도하면 gate 순서 평가에서
metadata(gate 1)/source(gate 2)/blob liveness(gate 3)/duplicate(gate 4) 판정을 거친다.
gate 1~4를 **모두 통과하면** credential resolution stub(gate 5)까지 **도달**한 뒤
`CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`(exit 4)로 fail-closed된다. 이때
`credentialResolutionReached: true`지만 credential 값은 읽지 않으며
(`credentialValuesAccessed: false`, `credentialValuesResolved: false`), actual API call(gate 6)에는
도달하지 않는다. 모든 live side-effect counter는 0이다.
...
→ `BLOCKED_BLOB_LIVENESS_EVIDENCE`(gate 3, exit 3)). 실제 credential resolution/API 호출을 통한
새 콘텐츠 배포는 여전히 별도 승인 slice에서만 wiring된다 — 이 slice의 실제 publish는 비활성이다.
...
| custom non-default, gate 1~4 통과 | `CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE` (gate 5 도달) | 4 | `credential_resolution_not_wired_this_slice` |
| custom non-default, source 미존재 | `BLOCKED_SOURCE_FILE_MISSING` (gate 2) | 3 | `credential_resolution_not_wired_this_slice` |
| custom non-default, blob evidence 부재 | `BLOCKED_BLOB_LIVENESS_EVIDENCE` (gate 3) | 3 | `credential_resolution_not_wired_this_slice` |
...
resolution stub은 `process.env`/secret/credential 값을 읽지 않고 어떤 live client lib도
import/호출하지 않는다.
...
콘텐츠를 duplicate 사유로 표시하면 Owner가 "중복이라 막힌 것"으로 오인할 수 있어 콘텐츠 종류별로
정확히 분기했다 — `currentContentDuplicateBlocked`도 default+양쪽 already-published일 때만
`true`이고, custom content는 duplicate 여부와 무관하게 `false`다. custom content의 실제 최종
halt 사유는 이제 `credential_resolution_not_wired_this_slice`다.
