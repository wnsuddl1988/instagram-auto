...
- **캐릭터 음성은 '좋은 목소리'보다 역할 연기와 감정 전달이 우선이다.** Jun은 상황별로 당황·안도·짜증·체념이 억양에 드러나야 하며, 단일톤 낭독은 FAIL. 상사/단역은 캐릭터 기능에 맞는 톤이어야 한다(예: 상사는 매끈한 고막남친 톤보다 중후하고, 느리고, 웃는 듯하지만 은근히 눈치 주는 부장님 톤).
- **대사는 화면 행동과 의미·감정이 붙어야 한다.** 기술적으로 offset이 맞아도 행동과 말의 의도/감정이 분리되어 보이면 FAIL. 숫자 싱크 QA와 Owner 체감 싱크 QA를 분리한다.
- **무대사 시각 코미디는 영상 자체가 충분히 역동적일 때만 허용한다.** 움직임·리듬·표정 변화가 약한 5초 이상 무대사 구간은 이탈 위험으로 보고, 코믹 BGM/SFX/기계음/버튼음/출력 리듬 등 사운드 레이어를 설계한다.
- **펀치라인은 대본만으로 통과하지 않는다.** 직전 리액션, 대사 억양, 간격, SFX/BGM ducking까지 포함해 웃음이 전달되어야 PASS.
