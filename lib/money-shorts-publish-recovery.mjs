...
function buildPlan({
  ...
  const stable = {
    ...
    ownerResolution: {
      present: ownerResolution?.present === true,
      valid: ownerResolution?.valid === true,
      applied: ownerResolution?.applied === true,
      sha256:
        typeof ownerResolution?.sha256 === "string" &&
        SHA256_RE.test(ownerResolution.sha256)
          ? ownerResolution.sha256
          : null,
      resolutionFingerprint:
        typeof ownerResolution?.resolutionFingerprint ===
          "string" &&
        SHA256_RE.test(
      ...
      sourceRecoveryFingerprint:
        typeof ownerResolution?.sourceRecoveryFingerprint ===
          "string" &&
        SHA256_RE.test(
          ownerResolution.sourceRecoveryFingerprint,
        )
          ? ownerResolution.sourceRecoveryFingerprint
          : null,
      instagramPermalink:
        typeof ownerResolution?.instagramPermalink === "string"
          ? ownerResolution.instagramPermalink
          : null,
      youtubeChannelId:
        typeof ownerResolution?.youtubeChannelId === "string"
          ? ownerResolution.youtubeChannelId
          : null,
      checkedAtIso:
        typeof ownerResolution?.checkedAtIso === "string"
          ? ownerResolution.checkedAtIso
          : null,
    },
 ...
 * @param {{resultFile: any, attemptFile?: any, attemptEvidence?: any, ownerResolutionFile?: any, currentBinding: any, ledgerEvidence: any}} input
 */
export function classifyMoneyShortsPublishRecovery(input) {
  ...
  const attemptClaimSha256 =
    typeof attemptFile?.sha256 === "string"
      ? attemptFile.sha256
      : null;
  const ownerResolutionExists =
    ownerResolutionFile?.exists === true;
  const ownerResolutionSha256 =
    typeof ownerResolutionFile?.sha256 === "string"
      ? ownerResolutionFile.sha256
      : null;
  const absentOwnerResolution = {
    present: false,
    valid: false,
    applied: false,
    sha256: null,
  };
  const invalidOwnerResolution = {
    present: ownerResolutionExists,
    valid: false,
    applied: false,
    sha256: ownerResolutionSha256,
  };
  const plan = (input) =>
    buildPlan({
      ...input,
      ...
      attemptClaimSha256,
      ownerResolution:
        input.ownerResolution ?? absentOwnerResolution,
    });
  ...
  if (!resultExists) {
    if (ownerResolutionExists) {
      return plan({
        state: "invalid_evidence",
        reason:
          "owner_reconciliation_present_publish_result_missing",
        resultSha256: null,
        currentBinding: current,
        ledgerEvidence,
        ownerResolution: invalidOwnerResolution,
      });
    }
    if (attemptExists) {
      return plan({
  ...
  if (
    ...
    return plan({
      state: ownerResolutionExists
        ? "invalid_evidence"
        : "ambiguous",
      reason: ownerResolutionExists
        ? "owner_reconciliation_publish_result_unreadable"
        : "armed_result_unreadable_or_unhashed",
      resultSha256,
      currentBinding: current,
      ledgerEvidence,
      ownerResolution: ownerResolutionExists
        ? invalidOwnerResolution
        : absentOwnerResolution,
    });
  }
  ...
  if (ownerResolutionExists) {
    const unresolvedPlan = plan({
      ...
      reason: youtubeOutcomeUnknown
        ...
        : instagramOutcomeUnknown
          ? "instagram_publish_outcome_unknown"
          : "armed_result_does_not_prove_safe_recovery_state",
      resultSha256,
      currentBinding: current,
      instagramMediaId,
      youtubeVideoId,
      ledgerEvidence,
      ownerResolution: absentOwnerResolution,
    });
    if (
      ownerResolutionFile?.parseOk !== true ||
      !isPlainObject(ownerResolutionFile?.evidence) ||
      !SHA256_RE.test(
        String(ownerResolutionSha256 ?? ""),
      ) ||
      youtubeOutcomeUnknown !== true ||
      instagram.status !== "published" ||
      instagram.outcome !== "confirmed_published" ||
      !instagramMediaId ||
      youtubeVideoId !== null ||
      attemptExists !== true ||
      attemptEvidence?.present !== true ||
      attemptEvidence?.journalValid !== true ||
      attemptEvidence?.claimSha256 !==
        attemptClaimSha256 ||
      ledgerEvidence.youtubeAlreadyPublished === true
    ) {
      return plan({
        state: "invalid_evidence",
        reason:
          "owner_reconciliation_not_applicable_to_current_publish_state",
        resultSha256,
        currentBinding: current,
        instagramMediaId,
        youtubeVideoId,
    ...
    const resolutionValidation =
      validateMoneyShortsPublishOwnerReconciliationEvidence({
        evidence: ownerResolutionFile.evidence,
        fileSha256: ownerResolutionSha256,
        currentBinding: current,
        resultSha256,
        attemptEvidence,
        instagramMediaId,
        expectedSourceRecoveryFingerprint:
          unresolvedPlan.recoveryFingerprint,
      });
    if (resolutionValidation.valid !== true) {
      return plan({
        state: "invalid_evidence",
        reason:
          "owner_reconciliation_schema_or_binding_invalid",
        resultSha256,
        currentBinding: current,
        instagramMediaId,
        youtubeVideoId,
    ...
    return plan({
      ...
      ownerResolution: {
        ...
        sha256: ownerResolutionSha256,
        resolutionFingerprint:
          resolutionValidation.resolutionFingerprint,
        sourceRecoveryFingerprint:
          resolutionValidation.sourceRecoveryFingerprint,
        instagramPermalink:
          resolutionValidation.instagramPermalink,
        youtubeChannelId:
          resolutionValidation.youtubeChannelId,
        checkedAtIso: resolutionValidation.checkedAtIso,
      },
    });
  }
