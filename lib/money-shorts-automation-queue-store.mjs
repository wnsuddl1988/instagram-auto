...
function compareQueueOrder(left, right) {
  const leftOrder = Number.isInteger(left?.queueOrder) && left.queueOrder > 0 ? left.queueOrder : Number.MAX_SAFE_INTEGER;
  const rightOrder = Number.isInteger(right?.queueOrder) && right.queueOrder > 0 ? right.queueOrder : Number.MAX_SAFE_INTEGER;
...
function normalizeQueue(queue) {
  ...
  return {
    ...
    jobs: jobs
      .map((job, index) => ({
        ...job,
        queueOrder: Number.isInteger(job?.queueOrder) && job.queueOrder > 0 ? job.queueOrder : index + 1,
      }))
...
function nextQueueOrder(queue) {
  return queue.jobs.reduce((maximum, job) => Math.max(maximum, Number.isInteger(job.queueOrder) ? job.queueOrder : 0), 0) + 1;
}

function withReindexedQueueOrder(jobs, timestamp, movedTopicId = null) {
  return jobs
    .map((job, index) => ({
      ...job,
      queueOrder: index + 1,
      updatedAt: job.topicId === movedTopicId ? timestamp : job.updatedAt,
    }));
}
...
export function enqueueMoneyShortsAutomationJob({
  ...
  return withQueueMutation({ rootDir, now, mutationId }, (queue) => {
    ...
    const job = {
      ...
      updatedAt: timestamp,
      queueOrder: prior?.queueOrder ?? nextQueueOrder(queue),
...
/**
 * Moves one queued topic by a single explicit Owner priority step.
 * It only changes local queue order and writes no execution receipt.
 * @param {{topicId: string, direction: "earlier" | "later", rootDir?: string, now?: () => string, mutationId?: string}} input
 */
export function moveMoneyShortsAutomationJobPriority({
  topicId,
  direction,
  rootDir = MONEY_SHORTS_AUTOMATION_QUEUE_ROOT,
  now = () => new Date().toISOString(),
  ...
  if (direction !== "earlier" && direction !== "later") {
    throw new Error("automation_queue_priority_direction_invalid");
  }
  const timestamp = now();
  return withQueueMutation({ rootDir, now, mutationId }, (queue) => {
    const ordered = [...queue.jobs].sort(compareQueueOrder);
    const currentIndex = ordered.findIndex((job) => job.topicId === safeId);
    if (currentIndex < 0) throw new Error("automation_queue_job_not_found");
    const targetIndex = direction === "earlier" ? currentIndex - 1 : currentIndex + 1;
    if (targetIndex < 0 || targetIndex >= ordered.length) {
      throw new Error("automation_queue_priority_boundary");
    }
    const [moved] = ordered.splice(currentIndex, 1);
    ordered.splice(targetIndex, 0, moved);
    const jobs = withReindexedQueueOrder(ordered, timestamp, safeId);
    return {
      ...queue,
      updatedAt: timestamp,
      jobs,
      history: appendHistory(queue, {
        at: timestamp,
        kind: direction === "earlier" ? "priority_moved_earlier" : "priority_moved_later",
        jobId: moved.jobId,
        topicId: moved.topicId,
        title: moved.title,
        queueOrder: targetIndex + 1,
        actionCount: 0,
        lastAdvance: moved.lastAdvance,
      }),
    };
  });
}
