...
const SAFE_AUTO_ADVANCE_ACTIONS = new Set([
  "realTtsPreflight",
  "flowMotionPrepare",
  "finalVideoCreate",
  "wizardPreflight",
...
function nextForStage(stageId, input) {
  ...
  if (stageId === "quality_gate") {
    return {
      ...
      reason: "최종 MP4가 미디어 품질 게이트를 통과하지 못해 Owner 확인이 필요합니다.",
    };
  }
...
export function buildMoneyShortsResumablePlan(input) {
  ...
  const imagesGenerated = input.expectedImageCount > 0 && input.generatedImageCount >= input.expectedImageCount;
