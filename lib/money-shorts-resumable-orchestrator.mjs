...
function nextForStage(stageId, input) {
  ...
  if (input.publishRecoveryRequired === true) {
    return {
      action: null,
      gate: "manual_recovery",
      reason:
        "게시 결과 증거가 이미 존재하거나 부분 성공 상태입니다. 일반 양쪽 업로드를 재실행하지 말고 Owner가 편별 복구 증거를 확인해야 합니다.",
    };
  }
  return {
    action: "actualUpload",
...
export function buildMoneyShortsResumablePlan(input) {
  ...
  // 게시 result/ledger 증거가 이미 존재하면 오래되거나 사라진 로컬 미디어보다
  // 외부 상태 확인을 우선한다. 이때 이전 제작 단계를 다시 실행하도록 안내하면
  // 이미 게시된 편의 artifacts를 바꾸거나 generic upload를 재시도할 수 있다.
  const currentStageId =
    input.publishRecoveryRequired === true
      ? "publication"
      : MONEY_SHORTS_RESUMABLE_STAGE_ORDER.find(
          (stageId) => ready[stageId] !== true,
        ) ?? null;
