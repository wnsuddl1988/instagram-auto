...
const STATE_CONCLUSION = {
  ...
  instagram_only: "Instagram 게시와 YouTube 미게시 Owner 대조 증거가 확인됐습니다. YouTube-only 복구 후보일 뿐 자동 동작은 하지 않습니다.",
...
export function buildMoneyShortsPublishReconciliationPacket({
  ...
  if (
    ...
    confirmedFacts.push(
      fact(
        ...
        "Owner 수동 플랫폼 대조 증거가 현재 result·산출물·시도 기록에 결속되어 있습니다.",
        text(ownerResolution?.resolutionFingerprint) || null,
      ),
    );
    confirmedFacts.push(
      fact(
        "owner_reconciliation_instagram_permalink",
        "Owner가 확인한 Instagram 공개 주소",
        text(ownerResolution?.instagramPermalink) || null,
      ),
    );
    confirmedFacts.push(
      fact(
        "owner_reconciliation_youtube_not_published",
        "Owner가 YouTube Studio에서 해당 채널의 미게시 상태를 확인했습니다.",
        text(ownerResolution?.youtubeChannelId) || null,
      ),
    );
  }
  ...
  } else if (state === "instagram_only") {
    ownerReview.push(
      review(
        "keep_instagram_republish_blocked",
        "Instagram은 이미 게시됐으므로 일반 양쪽 업로드와 Instagram 재게시를 계속 차단하세요.",
      ),
    );
    ownerReview.push(
      review(
        "youtube_only_requires_separate_owner_approval",
        "YouTube-only 복구는 전용 실행 경로와 별도 Owner 승인 전까지 실행하지 마세요.",
      ),
    );
  } else {
    uncertainFacts.push(
