...
export function serializePublishLedgerRuntime(ledger) {
  if (!isValidLedgerShape(ledger)) return null;
  const sortedRecords = [...ledger.records].sort((a, b) =>
...
export function writePublishLedgerRuntime(
  ...
  const bytes = serializePublishLedgerRuntime(ledger);
  if (!bytes) {
    return { ok: false, reason: "invalid_ledger" };
  }
  const serialized = JSON.parse(bytes.toString("utf8"));
