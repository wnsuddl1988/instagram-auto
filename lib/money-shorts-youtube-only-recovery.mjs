...
export const MONEY_SHORTS_YOUTUBE_ONLY_RECOVERY_EVENT_TRANSITIONS =
  Object.freeze([
...
export function validateMoneyShortsYoutubeOnlyRecoveryClaimEvidence(
  evidence,
) {
  const valid =
    validateRecoveryClaim(evidence) &&
    ...
    hasExactKeys(evidence?.currentBinding, [
      "contentId",
      "version",
      "productionPartId",
      "contentUnitManifestPath",
    ...
    hasExactKeys(evidence?.instagram, [
      "outcome",
      "mediaId",
      "permalink",
      "publishedAtIso",
    ...
    hasExactKeys(evidence?.youtubeBinding, [
      "sourceSha256",
      "metadataSha256",
      "variantId",
      "channelId",
    ]) &&
    hasExactKeys(evidence?.safety, [
      "automaticRetryAllowed",
      "instagramPublishAllowed",
      "blobMutationAllowed",
      "part2PublishAllowed",
    ]);
  return {
    valid,
    reason: valid
      ? "youtube_only_recovery_claim_valid"
      : "youtube_only_recovery_claim_invalid",
  };
}

export function validateMoneyShortsYoutubeOnlyRecoveryPreflightForClaim({
  evidence,
  claim,
}) {
  const valid =
    ...
    evidence.status === "PREFLIGHT_ONLY_OK" &&
    evidence.armed === false &&
    evidence.planFingerprint === claim.planFingerprint &&
    evidence.recoveryFingerprint ===
      claim.recoveryFingerprint &&
    evidence.ownerResolutionSha256 ===
      claim.ownerResolutionSha256 &&
  ...
  return {
    valid,
    reason: valid
      ? "youtube_only_recovery_preflight_valid"
      : "youtube_only_recovery_preflight_invalid",
  };
}

export function validateMoneyShortsYoutubeOnlyRecoveryEventEvidence({
  evidence,
  claim,
  previousEvidenceSha256,
  previousTransition,
  ...
  if (!isPlainObject(evidence)) {
    return {
      valid: false,
      reason: "youtube_only_recovery_event_invalid",
    };
  }
  const rebuilt =
  ...
  return {
    valid,
    reason: valid
      ? "youtube_only_recovery_event_valid"
      : "youtube_only_recovery_event_invalid",
  };
}

export function validateMoneyShortsYoutubeOnlyRecoveryResultEvidence({
  evidence,
  claim,
}) {
  if (!isPlainObject(evidence)) {
    return {
      valid: false,
      reason: "youtube_only_recovery_result_invalid",
    };
  }
  const rebuilt =
  ...
  return {
    valid,
    reason: valid
      ? "youtube_only_recovery_result_valid"
      : "youtube_only_recovery_result_invalid",
  };
}
