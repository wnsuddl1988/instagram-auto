...
/**
 * Aggregates an existing policy preview for Owner capacity awareness only.
 * It does not infer a schedule or grant execution authority.
 */
export function summarizeMoneyShortsAutomationQueueCapacity({ batchPolicy } = {}) {
  if (batchPolicy?.schemaVersion !== MONEY_SHORTS_AUTOMATION_QUEUE_PLANNER_VERSION || batchPolicy?.mode !== "no_submit_batch_policy_preview" || !Array.isArray(batchPolicy.entries)) {
    throw new Error("automation_queue_capacity_summary_invalid");
  }
  const count = (kind) => batchPolicy.entries.filter((item) => item.kind === kind).length;
  return {
    schemaVersion: MONEY_SHORTS_AUTOMATION_QUEUE_PLANNER_VERSION,
    mode: "no_submit_capacity_summary",
    queueItemCount: batchPolicy.itemCount,
    localSafeReadyCount: count("local_safe_next"),
    localSafeWaitingCount: count("local_safe_waiting"),
    paidGenerationApprovalCount: count("paid_generation_approval"),
    ownerQualityCheckCount: count("owner_qa"),
    publicationApprovalCount: count("publication_approval"),
    ownerDecisionCount: count("topic_selection") + count("owner_approval"),
    pausedCount: count("paused"),
    recoveryOrBlockedCount: count("manual_recovery") + count("execution_blocked"),
