...
- HEAD: `fe6437f` — `feat(source-facts): add caption system v1 safe-zone qa` ← **현재 HEAD**
- Commit: `361de7b` — `docs(state): clean route smoke checkpoint state`
- Commit: `2b1db95` — `docs(state): record package preview route smoke pass`
- Commit: `b2dadee` — `feat(source-facts): include QA report in generated copy payload`
- Commit: `eac702c` — `feat(package-preview): add scene package QA report panel`
- Branch: `codex/source-first-blueprint-clean` (ahead 72)
- Uncommitted: `money-shorts-os-scene-label-safe-zone-contract-v1` (7 files modified, static guard 64/64 PASS)
- Push: 미실행
...
- HEAD는 `2b1db95`, ahead 70, clean working tree (untracked `piq_diag_out.txt` 제외).
