...
## v3.2 Owner QA actual pass + upload approval packet 준비 (`golden-sample-v3-2-owner-qa-pass-upload-approval-packet-prep-v1` — 2026-07-06, Opus 4.8/high)

**Owner 명시 QA 통과 `OWNER_QA_ACTUAL_PASS: t1_lifestyle_inflation — latest mux mp4 watched/listened directly by Owner, visual/caption/story/audio accepted, proceed to prepare upload approval packet only; no upload execution yet` 하에 no-live 준비 slice 수행. Owner 직접 시청/청취 QA 통과(approvalSequenceRule 4단계)를 기록하고, upload 실행(5단계)은 future-use-only packet으로만 준비. upload hard block 계속 active, uploadReady/productionReady false 불변, upload/API/env/secret/render/mux/TTS/image/browser 실행 0.**

- **핵심 안전 판단**: Owner QA actual pass는 실제 통과로 기록(`ownerQaPassed:true`, `ownerViewingListeningActualStatus:PASS_DIRECT_OWNER_REVIEW_COMPLETED`)했으나, 이것이 upload 승인으로 오해되지 않도록 QA fixture·packet·markdown·guard 4계층 모두에서 "선행 조건일 뿐 upload 승인 아님"을 명시. upload는 별도 명시 `APPROVE_UPLOAD` + 별도 upload slice에서만 발생.
- 신규 `scripts/fixtures/golden_sample_v3_2_owner_qa_actual_pass.t1_lifestyle_inflation.v1.json`: Owner 문구 문자 단위 + passSource OWNER_DIRECT_VIEWING_LISTENING(자동/기술 pass 아님) + 수용 4차원(visual/caption/story/audio) + qaTarget(latest mux mp4 경로, render/mux plan, checkpoint adbcb4b, audit JSON, 자동 audit verdict PASS_CANDIDATE_PENDING_VISION_QA) + uploadApprovedNow/uploadExecutionApprovedNow/uploadReady/productionReady false + uploadHardBlockActive true + prerequisiteRole(isUploadApproval false, isPrerequisiteForUploadApproval true, stage 4 충족) + readiness STANDARDIZED_NO_LIVE_READY 불변.
- 신규 `scripts/fixtures/golden_sample_v3_2_upload_approval_packet.t1_lifestyle_inflation.v1.json`: future-use-only(uploadApprovalGrantedNow false) + currentApprovalFlags 6개 false(upload execution/endpoint unblock/platform credential/env secret/db status 포함) + prohibitedReadinessFlags 4개 false(uploadReady/productionReady/automationExpansionReady/uploadHardBlockLifted) + ownerQaPrerequisite(QA pass 충족 + upload 별도 승인 요구) + uploadTarget(latest mux mp4 지목만) + upload hard block active + futureUploadApprovalRequirement(requiresOwnerQaActualPassFirst/requiresSeparateUploadSlice/requiresExplicitApproveUploadWording true, capsEffectiveNow false) + requiredOwnerApprovalWording(APPROVE_UPLOAD 템플릿) + futureUseUploadApprovalSnippet(FUTURE_USE_ONLY_NOT_APPROVED_NOW 라벨) + stopConditions 7종.
- 신규 `_ai/GOLDEN_SAMPLE_V3_2_UPLOAD_APPROVAL_PACKET.md`: 상단 "⚠️ NOT UPLOAD APPROVAL / 준비 문서" 경고 + 현재 상태 테이블(QA pass ✅ / upload 미승인 ❌ / hard block 🔒) + Owner QA 문구 + latest mux 경로 + upload 차단(403) 명시 + future-use-only APPROVE_UPLOAD snippet + 미래 요건 8항 + 참조 8개 + 절대 금지.
- 신규 `scripts/check-golden-sample-v3-2-upload-approval-packet-static.mjs`: dependency-free guard (node:fs/path/url only). **ALL PASS 59/59** — QA 문구 정확/직접 확인 근거/ownerQaPassed true, upload 미승격(4 flag false + hard block active + 선행 역할), packet future-use-only(6+4 flag false), hard block active + policy 실재, latest mux 참조=render/mux plan mux path+audit JSON, snippet future-use-only+APPROVE_UPLOAD+QA 선행+별도 slice+cap effectiveNow false, no-regeneration/no-render·mux·TTS·image·browser 경계(요건/문구/snippet 3중), stop 완비, prior 계약 4종 불변, markdown 정합 6종 + mutant 21종(QA text drift/누락, uploadApproved/uploadReady/productionReady/hardBlock false, packet grant/ready/credential/block/snippet 현재승인화/라벨제거/QA선행제거/slice제거/caps현재예산화/mux참조변조·누락/no-regeneration 경계 제거 2종/stop 제거).
- Preflight/regression 전부 PASS: 신규 upload packet guard **59/59**, upload hard block **ALL PASS**, live render/mux run plan **58/58**, live action approval packet **51/51**, future execution plan gate **44/44**, integrated readiness **97/97**. `node --check` guard PASS, 신규 fixture 2개 JSON parse PASS.
- prior 계약 무수정: live action approval packet/future gate/integrated readiness/upload hard block policy 전부 미변경(guard가 불변 확인) — 신규 QA-pass+upload-packet 아티팩트가 이들을 historical prerequisite로 참조만.
- 금지 작업 미수행: upload 실행///api/upload POST/upload queue/Instagram·YouTube client/n8n 0, env/secret/.env.local read 0, external API/network 0, image/TTS/render/mux/browser/CDP 실행·재생성 0, audio/video/image binary 분석 0(post_render audit JSON은 read-only metadata만), dependency/lockfile/font/DB/deploy 변경 0, commit/push 0.
- 제외 파일 보존: CODEX_REVIEW.md·NEXT_ACTION.md·PROJECT_STATE.md·CONTEXT_TRANSFER_CODEX.md·piq_diag_out.txt·render-golden-sample-visual-only-v1.mjs·salary_3days manifest·unrelated output/·C:\tmp — 무접촉·미스테이지. HANDOFF_NOW.md의 M은 Codex 작성분(이번 세션 무수정, status 갱신 불요).
- deviations/risks: 범위 이탈 없음. Owner QA actual pass는 실제 통과로 기록됐지만 upload는 여전히 차단 — upload hard block active, uploadReady:false, productionReady:false 유지, readiness STANDARDIZED_NO_LIVE_READY 불변. 다음 단계는 Owner의 명시 `APPROVE_UPLOAD` + 별도 upload slice가 있어야만 upload 승인 후보가 됨.
- checkpoint recommendation: repo 변경 = 신규 fixture 2 + markdown 1 + guard 1 + report = 5파일(신규 4 + report append) — Owner QA pass + upload packet 준비 증거로 **checkpoint commit 권장**. branch ahead 184, 이번 slice uncommitted.

## v3.2 live Instagram upload 준비 + live 체인 harness 차단 (`golden-sample-v3-2-live-instagram-upload-with-buildgongjakso-public-url-v1` — 2026-07-06, Fable 5/ULTRAMODE)

**결과 요약: upload 실행 여부 = BLOCKED (upload 0회, Instagram API 호출 0회, credential/env read 0회, 외부 네트워크 요청 0회). blocker = `BLOCKED_AUTO_MODE_PERMISSION_LIVE_CHAIN` — Claude Code auto-mode 권한 분류기가 public URL liveness 사전 점검(curl HEAD)을 거부했고, 거부 사유는 "공개 표면 생성 + live Instagram upload 체인은 현재 세션 대화에서 Owner의 직접 승인 없이 auto mode 실행 불가, URL/upload 단계는 auto mode 밖에서 직접 검토 하에 실행하라"였다. 이 거부는 우회하지 않았고 live 체인 전체(URL 검증 → mp4 public/ 복사 → credential read → upload)를 중단했다. 승인 범위 중 no-live 준비물(run plan fixture + fail-closed one-shot runner + static guard)은 전부 완성·검증 완료.**

- **변경 파일 (신규 3 + report append)**:
  - `scripts/fixtures/golden_sample_v3_2_live_instagram_upload_run_plan.t1_lifestyle_inflation.v1.json` — Owner 승인 3건 문자 단위 기록(APPROVE_UPLOAD / APPROVE_UPLOAD_PUBLIC_URL / "자꾸 물어보지말고" clarification), platform 정확히 Instagram·cap 정확히 1, deterministic URL(`https://www.buildgongjakso.com/golden-sample/v3-2/t1_lifestyle_inflation/...mp4`) + repo static path + rejectHtml + video/* 요구, credential allowlist 2-key(INSTAGRAM_ACCESS_TOKEN/INSTAGRAM_BUSINESS_ACCOUNT_ID — lib/instagram.ts 실제 naming 확인), 8게이트 순서, one-shot ledger(container 호출 전 attempted 선기록), blocker 코드 13종, final pre-upload audit 5요건, httpEndpointStillBlocked:true(HTTP hard block 불변), executionPreconditionNotes에 auto-mode 차단 사실 기록
  - `scripts/run-golden-sample-v3-2-instagram-upload-once.mjs` — fail-closed one-shot runner: GATE_1 plan → GATE_2 ledger 0/1 → GATE_3 source mp4(20294549 bytes) → GATE_4 owner QA pass → GATE_5 hard-block reconcile → GATE_6 public URL(2xx+video/*, text/html 거부, buildgongjakso host만) → (--arm 전용) GATE_7 allowlist 2-key targeted 추출(broad .env.local parse 없음, presence boolean만) → GATE_8 업로드 1회(ledger 선기록 → container → poll 24×5s → FINISHED일 때만 publish, 재시도 없음). --arm 없으면 GATE_6까지 preflight-only. secret 값/account ID 원문은 로그·result JSON 미기록, result JSON은 승인된 C:\tmp 경로에만
  - `scripts/check-golden-sample-v3-2-live-instagram-upload-run-static.mjs` — dependency-free 정적 가드 **ALL PASS 31/31**: plan 불변조건 + runner 소스 검사(import allowlist, YouTube/googleapis/dotenv/subprocess/브라우저 금지, env 접근이 allowlist 루프 유일, 게이트 순서, ledger 선기록, FINISHED 전 publish 차단, graph.facebook.com 외 https 리터럴 금지, repo 내 write 금지) + HANDOFF 요구 mutant 11종 전부 fail-closed 확인(+allowlist 확장/ledger 선기록 해제 2종 추가)
- **public URL materialization 결과**: 미수행. deterministic URL 사전 점검(curl)이 auto-mode 분류기에서 거부되어 liveness 미확인. mp4의 `public/golden-sample/...` 복사도 같은 "공개 표면 생성" 체인으로 지목되어 보류 — `public/` 무변경. deploy/push 미승인 상태에서는 배포 없이 URL이 live일 수 없으므로 GATE_6에서 BLOCKED_PUBLIC_URL_NOT_DEPLOYED로 멈추는 것이 예정 동작
- **credential/env 접근 범위**: 0 — .env.local 미접근(파싱/read 일절 없음), 어떤 env key도 읽지 않음. YouTube client/import/credential 0
- **checks/results**: `git status -sb` ✓ / `node --check` guard·runner ✓ / plan fixture JSON parse ✓ / 신규 guard **31/31** / regression: upload approval packet **59/59**, upload hard block **ALL PASS**, live render/mux run plan **58/58**
- **금지 작업 미수행**: Owner 재질문 0(URL 질문이 아닌 권한 차단 보고), Instagram 외 플랫폼 0, upload 시도 0, YouTube 0, broad .env.local parse 0, secret logging 0, image/TTS/render/mux/browser/CDP 0, DB/Supabase/deploy/Vercel/push 0, dependency/lockfile/font 0, commit/push 0
- **보존 확인**: CODEX_REVIEW/NEXT_ACTION/PROJECT_STATE/CONTEXT_TRANSFER_CODEX/piq_diag_out.txt/render-golden-sample-visual-only-v1.mjs/salary_3days manifest/unrelated output·C:\tmp 무접촉. HANDOFF_NOW.md 무수정(Codex 작성분 유지)
- **Owner 재개 경로 (live 체인 실행 시)**: ① mp4를 `public/golden-sample/v3-2/t1_lifestyle_inflation/`에 복사(또는 auto mode 밖에서 Claude에게 해당 단계 직접 승인) ② 사이트 배포(별도 승인 필요 — 배포 없이는 URL 미라이브) ③ `node scripts/run-golden-sample-v3-2-instagram-upload-once.mjs` (preflight — URL/게이트 확인, credential 미접근) ④ 통과 시 `node scripts/run-golden-sample-v3-2-instagram-upload-once.mjs --arm` (1회 업로드). runner는 어느 단계 실패에도 fail-closed blocked result JSON을 남김
- deviations/risks: 범위 이탈 없음. blocked result JSON은 runner 미실행이라 C:\tmp에 미생성 — blocked 기록은 이 report + plan fixture executionPreconditionNotes로 대체(runner ledger 경로는 실제 run 전용 보존). harness 차단은 HANDOFF 실패 분류상 권한/sandbox 제한이며 scope 확장 없이 보고로 종료
- checkpoint recommendation: 신규 3파일 + report append — live upload 준비물 완성 증거로 **checkpoint commit 권장**(Owner 승인 필요). branch ahead 185, 이번 slice uncommitted

## 지속 public media URL 계층 설계 고정 (`stable-public-media-url-layer-v1` — 2026-07-06, Opus 4.8/high)

**결과 요약: 지속(sustained) 사용에 안정적인 public mp4 URL 경로를 설계로 고정. 기본 전략 = `media.buildgongjakso.com` 전용 media origin + object storage/CDN, fallback = repo `public/` static mp4 + deploy(one-off/manual만). no-live 준비 slice — live upload 0, credential/env read 0, public URL network/liveness check 0, storage provisioning/object upload 0, DNS/deploy 0, DB/Supabase/YouTube/render/mux/TTS/image/browser 0, dependency/lockfile 0, commit/push 0.**

- **변경 파일 (신규 3 + report append)**:
  - `docs/public-media-url-strategy.md` — 지속 default(media origin + object storage/CDN) vs fallback(repo public/ static, one-off만) 비교표, 결정론적 key scheme(`media/<schemaGroup>/<version>/<topicId>/<artifact>.mp4`), provider-neutral 요건(Vercel Blob/R2/S3-compat), Instagram 업로드 전 verifier gate 7종(HTTPS/allowed host/2xx|206/video content-type/reject text-html/unauth/reject login·HTML redirect), 기존 runner 연결, 외부 Owner 승인 항목 6종(DNS/storage provisioning/env·secret/deploy/live liveness check/`--arm` upload), 금지 목록, 이후 통합 순서
  - `scripts/fixtures/stable_public_media_url_strategy.v1.json` — 기계판독 계약: sustainedDefault(mediaOriginHost=media.buildgongjakso.com, backing=object_storage_cdn, commitGeneratedMp4ToGit:false) / fallback(repo_public_static_plus_deploy, role=manual_one_off_only, isSustainedDefault:false) / deterministicKeyScheme / providerNeutral(vendorLock:false) / uploadVerifierGates(7 gate + uploadOnlyWithVerifiedUrl) / runnerContract(consumesVerifiedUrlOnly, preservesFailClosedGates) / externalOwnerApprovalRequired 6종 / forbiddenBehavior 14종 / prohibitedSustainedDefaultDrift
  - `scripts/check-stable-public-media-url-strategy-static.mjs` — dependency-free 정적 가드(node:fs/path/url only) **ALL PASS 24/24**: fixture 불변조건(default host=media.buildgongjakso.com, fallback≠sustained, key scheme, provider-neutral, verifier gate, runner 계약, 외부 승인, forbidden) + docs 명문화 5종 + self-scan(live/env/write 토큰 없음, import 제한) + mutant 10종 전부 fail-closed(default host drift media→www, fallback을 sustained 승격, text/html 허용, non-HTTPS 허용, 검증 없는 URL upload, broad .env.local 금지 제거, YouTube/Supabase/deploy 금지 제거, secret logging 금지 제거, 외부 승인 요건 제거, mp4 git 커밋을 default로)
- **chosen sustained path**: `https://media.buildgongjakso.com/...` (전용 media origin + object storage/CDN, git/deploy 무결합)
- **fallback policy**: repo `public/` static mp4 + site deploy → `https://www.buildgongjakso.com/...`, 긴급/수동 샘플 복구 one-off만, 지속 default 아님
- **checks/results**: `git status -sb` ✓ / `node --check scripts/check-stable-public-media-url-strategy-static.mjs` ✓ / 신규 fixture JSON parse ✓ / 신규 guard **24/24 ALL PASS** / regression `node scripts/check-golden-sample-v3-2-live-instagram-upload-run-static.mjs` **31/31 ALL PASS** (기존 live runner fail-closed 계약 무손상)
- **live side effects: none** — upload 0, Instagram API 0, credential/env read 0, .env.local 미접근, public URL network/liveness check 0, storage provisioning/object upload 0, DNS/custom domain 0, deploy/Vercel 0, DB/Supabase 0, YouTube 0, render/mux/TTS/image/browser 0, dependency/lockfile/font 0, secret logging 0, commit/push 0
- **보존 확인**: CODEX_REVIEW/NEXT_ACTION/PROJECT_STATE/CONTEXT_TRANSFER_CODEX/piq_diag_out.txt/render-golden-sample-visual-only-v1.mjs/salary_3days manifest/unrelated output·C:\tmp 무접촉. 기존 live runner/plan/guard 무수정(reference patch 불필요 — fixture runnerContract가 텍스트 참조만). HANDOFF_NOW.md는 이번 slice 승인 범위 문서로 무수정
- deviations/risks: 범위 이탈 없음. runner/guard 최소 reference patch는 "필요할 때만" 옵션이었고, fixture가 runner를 참조·명문화하는 것으로 충분해 runner 소스 무변경(regression 31/31 유지가 이를 확인). storage provider 실제 선택·프로비저닝·DNS·deploy·live check는 전부 외부 Owner 승인 항목으로 명시, 이 slice 밖
- checkpoint recommendation: 신규 3파일 + report append — 지속 media URL 설계 고정 증거로 **checkpoint commit 권장**(Owner 승인 필요). branch ahead 186, 이번 slice uncommitted

## media provider 선택 결정 패킷 (`media-provider-discovery-decision-packet-v1` — 2026-07-06, Opus 4.8/high)

**결과 요약: sustained media origin(media.buildgongjakso.com)의 최적 provider를 no-secret/no-provisioning 조사로 결정. primary=Vercel Blob, fallback=Cloudflare R2, tertiary=S3-compatible. no-live discovery slice — provisioning 0, DNS 0, env/secret write 0, secret 값 read/print 0, deploy 0, object upload 0, public URL liveness 0, Instagram upload/--arm 0, dependency/lockfile 0, commit/push 0.**

- **변경 파일 (신규 3 + report append)**:
  - `docs/media-provider-decision-packet.md` (신규) — Vercel Blob(primary)/Cloudflare R2(fallback)/S3-compatible(3순위) 비교표, 로컬 read-only evidence, 공식문서 근거(env 이름만), 8단계 승인 시퀀스, 리스크 R1~R3 + 롤백, 금지 목록
  - `scripts/fixtures/media_provider_decision_packet.v1.json` (신규) — 기계판독 결정 패킷: sustainedDefaultHost 보존, candidates 3종(requiredEnvNames 이름만), chosen(primary=vercel_blob/fallback=cloudflare_r2), localReadOnlyEvidence(secretValuesRead:false), officialDocEvidence, futureSequence 8게이트(전부 ownerApprovalRequired:true), approvalGates(dns/env/provisioning/deploy/upload/live/arm), verifierGatesInheritedFromStrategy, riskAndRollback, forbiddenBehavior 14종
  - `scripts/check-media-provider-decision-packet-static.mjs` (신규) — dependency-free 정적 가드(node:fs/path/url only) **ALL PASS 28/28**: fixture 불변조건 + docs 명문화 6종 + self-scan + mutant 13종 fail-closed(sustained host drift, candidate 누락, chosen 누락, DNS/env 게이트 누락, provisioning/deploy/upload/live/arm 허용, secret read 허용, dependency 금지 제거, env name에 secret 값 섞임)
- **provider recommendation**: primary=**Vercel Blob**, fallback=**Cloudflare R2** (tertiary=S3-compatible)
- **근거 (official docs + local/account read-only evidence)**:
  - 로컬: `.vercel/project.json`(projectName=instagram-auto, org team 링크됨), `vercel.json`(nextjs/pnpm), `next.config.ts`(outputFileTracingExcludes), `.gitignore` line47 `*.mp4`(→ repo public/ 커밋 fallback은 git add -f 필요+bloat, object storage default 근거)
  - account read-only: `vercel whoami`(로그인됨, 프로비저닝/토큰 프롬프트 없음), `vercel project ls`(non-mutating listing) → **buildgongjakso-home → https://www.buildgongjakso.com 존재**(도메인이 같은 Vercel org 관리 → media.buildgongjakso.com 서브도메인 추가가 동일 대시보드 가능) + instagram-auto-tau.vercel.app 배포 실재. **secret 값은 읽지 않음**
  - 공식문서: Vercel Blob(public direct URL, 1st-party), R2(S3-compat, egress 무료, custom domain), S3(표준 API). env는 이름만 기록(BLOB_READ_WRITE_TOKEN / R2_* / AWS_*·S3_*)
- **future sequence (승인 게이트)**: ① provider 선택 ② provisioning ③ DNS(media.buildgongjakso.com) ④ env/secret ⑤ code integration slice ⑥ deploy ⑦ live URL liveness ⑧ Instagram --arm — 각 단계 별도 Owner 명시 승인 필요, 이 slice는 어느 것도 미실행
- **checks/results**: `git status -sb` ✓ / `node --check` guard ✓ / 신규 fixture JSON parse ✓ / 신규 guard **28/28 ALL PASS** / regression `node scripts/check-stable-public-media-url-strategy-static.mjs` **24/24 ALL PASS**
- **live side effects: none** — provisioning/bucket/token/account creation 0, DNS 0, env/secret write 0, secret 값 read/print/write 0(env 이름만), deploy 0, object upload 0, public URL liveness 0, Instagram upload/--arm 0, Instagram credential/env read 0, broad .env.local parse 0, YouTube/Supabase/DB/render/mux/TTS/image/browser 0, dependency/lockfile/font 0, commit/push 0. read-only CLI(whoami/project ls)는 mutation/로그인 프롬프트 없이 완료
- **보존 확인**: CODEX_REVIEW/NEXT_ACTION/PROJECT_STATE/CONTEXT_TRANSFER_CODEX/piq_diag_out.txt/render-golden-sample-visual-only-v1.mjs/salary_3days manifest/unrelated output·C:\tmp 무접촉. 기존 upload runner/guard + stable strategy 3파일 무수정(regression 24/24가 확인)
- deviations/risks: 범위 이탈 없음. Vercel Blob 권장은 로컬 링크 상태 기반 실용 판단이며 provider-neutral 계약 유지 → 비용/한도 시 R2 무손실 전환. 실제 provider 선택·프로비저닝·DNS·env·deploy·upload·liveness·--arm은 전부 이후 별도 Owner 승인 게이트로 명시, 이 slice 밖
- checkpoint recommendation: 신규 3파일 + report append — provider 결정 패킷 완성 증거로 **checkpoint commit 권장**(Owner 승인 필요). branch ahead 187, 이번 slice uncommitted

## media provider 순서 수정 — R2 primary (`media-provider-decision-packet-provider-order-fix-v1` — 2026-07-06, Opus 4.8/high)

**결과 요약: Codex review finding 반영 — strict custom media origin(media.buildgongjakso.com) 기준에서 custom domain 직접 연결 근거가 공식문서에 있는 Cloudflare R2를 primary로, Vercel Blob을 fallback(별도 도메인 전략 검증 필요), S3-compatible을 tertiary로 순서 정정. 문서/fixture/guard 3파일을 일관되게 수정하고 guard가 R2 primary를 강제 + primary가 vercel_blob으로 회귀하면 fail하는 mutant 추가. no-live boundary 유지 — provisioning/DNS/env write/deploy/upload/liveness/Instagram --arm 0, secret read/print/write 0, dependency/lockfile 0, commit/push 0.**

- **변경 파일 (기존 3 수정 + report append)**:
  - `docs/media-provider-decision-packet.md` — §1 권장 테이블 primary=Cloudflare R2 / fallback=Vercel Blob로 교체 + 결정 기준(strict custom origin) 명시, §2 공식문서 근거를 R2(custom domain 직접 연결 근거 존재, ref developers.cloudflare.com/r2/buckets/public-buckets) vs Vercel Blob(blob custom domain 직접 연결 근거 미확인 → redirect/proxy/도메인 전략 검증 필요, ref vercel.com/docs/vercel-blob)로 재작성, §3 비교표 custom domain 행을 최상단으로 올려 R2 ✅ / Blob ⚠️ 대비, §4 시퀀스 step1~3 R2 기준 detail, §5 리스크 R1(R2 zone/DNS)+R4(Blob custom domain 미확인) 추가
  - `scripts/fixtures/media_provider_decision_packet.v1.json` — candidates roles 교체(cloudflare_r2=primary/vercel_blob=fallback) + customDomainSupported 필드 추가(R2 true/Blob false/S3 true), chosen.primary=cloudflare_r2·fallback=vercel_blob·rationale 재작성, officialDocEvidence R2/Blob custom domain 근거 대비 + ref URL, futureSequence step1~3 R2 detail, riskAndRollback R1(zone/DNS)+R4(Blob custom domain 미확인) 4항으로, nature 갱신
  - `scripts/check-media-provider-decision-packet-static.mjs` — primary 검증을 vercel_blob→**cloudflare_r2**로 정정 + fallback=vercel_blob 강제, candidate role 일관성(R2 primary/Blob fallback) + customDomainSupported 근거 검증(R2 true/Blob false) 추가, docs 검증에 primary=R2/fallback=Blob 순서 + custom domain 근거 대비 명문화 2종 추가, riskAndRollback 4개 이상으로, mutant 14(primary가 vercel_blob 회귀 → fail)·15(role 뒤집힘+Blob custom domain 근거 위조 → fail) 추가, 헤더 주석 갱신
- **provider recommendation after fix**: primary=**Cloudflare R2**, fallback=**Vercel Blob**, tertiary=S3-compatible
- **official-doc basis**: R2 public buckets 문서 — bucket contents를 내가 제어하는 custom domain으로 노출 가능 + production custom domain 권장 → media.buildgongjakso.com 직접 연결 근거 존재 (https://developers.cloudflare.com/r2/buckets/public-buckets/). Vercel Blob 문서 — public direct blob URL(anyone-with-URL)은 제공하나 blob 자체 custom domain 직접 연결 근거 미확인 → strict media origin엔 별도 redirect/proxy/도메인 전략 필요 (https://vercel.com/docs/vercel-blob)
- **checks/results**: `git status -sb` ✓ / `node --check` guard ✓ / fixture JSON parse ✓ / 신규 guard **32/32 ALL PASS**(provider order fix mutant 14/15 포함) / regression `node scripts/check-stable-public-media-url-strategy-static.mjs` **24/24 ALL PASS**
- **live side effects: none** — provisioning/bucket/token/account 0, DNS 0, env/secret write 0, secret 값 read/print/write 0, deploy 0, object upload 0, public URL liveness 0, Instagram upload/--arm 0, credential/env read 0, broad .env.local 0, YouTube/Supabase/DB/render/mux/TTS/image/browser 0, dependency/lockfile/font 0, commit/push 0. 이번 slice는 문서/fixture/guard 텍스트 수정만 — 새 CLI/network 호출 없음(이전 discovery의 vercel whoami/project ls read-only evidence 재사용)
- **보존 확인**: CODEX_REVIEW/NEXT_ACTION/PROJECT_STATE/CONTEXT_TRANSFER_CODEX/piq_diag_out.txt/render-golden-sample-visual-only-v1.mjs/salary_3days manifest/unrelated output·C:\tmp 무접촉. stable strategy 3파일 + upload runner/guard 무수정(regression 24/24 확인). HANDOFF_NOW.md는 이전 discovery slice 문서로 무수정(status update 불요)
- deviations/risks: 범위 이탈 없음. sustainedDefaultHost/approvalGates/verifier/forbidden 등 no-live 계약은 불변, provider 순서·근거·리스크만 정정. R2 primary는 custom domain 근거 우위 판단이며 provider-neutral 계약상 host/경로만 교체 가능. 실제 zone/DNS/env/deploy/upload/liveness/--arm은 이후 별도 Owner 승인 게이트

## owner-local-env-no-log-command-wrapper-v1 (2026-07-08)

- **목적**: Owner가 로컬에서 `.env.local`의 승인된 6개 key만 child process env로 no-log 주입해 `credential-preflight`를 `present:true`로 확인하는 wrapper/launcher + guard. Claude/Codex는 실제 `.env.local` 미read, fake/dummy env로만 검증.
- **changed files**(4): `scripts/run-owner-command-with-local-env-no-log.mjs`(신규 wrapper), `scripts/check-owner-local-env-no-log-wrapper-static.mjs`(신규 guard), `package.json`(scripts 1줄 `owner:credential-preflight:local` — deps/devDeps/lockfile 불변), `docs/owner-daily-automation-runbook.md`(로컬 env no-log wrapper 섹션 추가).
- **added command/script**: `pnpm owner:credential-preflight:local` → `node scripts/run-owner-command-with-local-env-no-log.mjs credential-preflight`.
- **no-log wrapper behavior**: allowlist 6 key(`INSTAGRAM_BUSINESS_ACCOUNT_ID`/`INSTAGRAM_ACCESS_TOKEN`/`YOUTUBE_CLIENT_ID`/`YOUTUBE_CLIENT_SECRET`/`YOUTUBE_REFRESH_TOKEN`/`BLOB_READ_WRITE_TOKEN`)만 `.env` 파서로 로드(그 외 라인 값 미read). `KEY=value`/quote/comment/`export ` 처리, 변수확장·명령실행 없음. 값은 `injected` 객체에만 존재 → 출력/파일/길이/prefix/suffix/hash/mask 미노출(key 이름 + present boolean만). child env는 화이트리스트 non-secret OS 변수 개별 상속(broad spread 금지) + 승인 key만; `spawnSync(process.execPath, [...], { shell:false, env })`로 entrypoint `--credential-preflight` 위임. child exit 후 로컬 secret 참조 delete. flag는 `--env-path`(node 예약 `--env-file`과 충돌 회피), 기본값은 Owner runtime에서만 `.env.local`.
- **fake-env test result**: guard가 OS temp에 dummy env(승인6 + 무시될 non-approved secret) 생성 → wrapper 실행 시 **6/6 present:true**, `allRequiredKeysPresent:true`, `readyForCredentialResolution:true`, `credentialResolutionWiredThisSlice:false`. dummy 값·non-approved secret 값 **출력에 미노출**. missing-keys → 1/6 present + allRequiredKeysPresent false, nonexistent env → 0/6 present, unsupported command → non-zero fail-closed. 모두 dummy 값 미노출.
- **checks/results**: [1] git status-sb OK, [2] node --check wrapper OK, [3] node --check guard OK, [4] wrapper guard **47 PASS/0 FAIL**, [5a] owner entrypoint guard regression **222 PASS/0 FAIL**, [5b] orchestrator guard regression **405 PASS/0 FAIL**, [6] package.json JSON parse OK · deps/devDeps 불변 · pnpm-lock.yaml 미변경.
- **side effects confirmation**: Instagram/YouTube/Blob/OpenAI/ElevenLabs/Pexels/Supabase API 0, OAuth 0, upload/HEAD/mutation 0, ffmpeg/ffprobe 0, 새 media/TTS/image/browser 0, deploy/DNS 0, dependency/lockfile 0, commit/push 0. guard의 fake env는 mkdtemp OS temp에 생성 후 rmSync 정리(repo 잔존 0).
- **env/secret handling confirmation**: Claude는 실제 `.env.local` 미read(구현·테스트 전부 fake/dummy env). wrapper 실행 코드에 broad `{ ...process.env }` spread 0, dotenv/`vercel env pull` 0, value 파생(length/slice/hash/prefix/suffix/mask) 0 — 관련 언급은 전부 문서 주석. guard가 dummy 값 미노출·value-derived 필드 부재·secret-shaped value 부재·shell:false·allowlist-only·`.env.local` 미read를 자동 검증.
- **deviations/risks**: 범위 이탈 없음(신규 2파일 + package.json scripts 1줄 + runbook 1섹션 = 허용 범위). entrypoint 미수정(standalone wrapper로 충분). flag를 `--env-file`→`--env-path`로 정한 이유는 node 예약 옵션과 충돌 회피(런타임 검증으로 확인). 남은 위험 없음 — 실제 present:true 확인은 Owner가 로컬에서 `.env.local`로 실행할 때만 발생하며, live publish는 여전히 비활성.
- checkpoint recommendation: 신규 wrapper+guard는 env/secret-adjacent이나 no-log·값 미노출·fake-env 다층 검증 완료. Codex 검토 후 checkpoint commit 권장(신규 파일이라 정확한 stage 목록 필요).
- checkpoint recommendation: 기존 3파일 수정 + report append — provider 순서 정정 완료. stable strategy checkpoint(44a60da) 이후 media provider packet(discovery + 이번 fix)이 누적됐으므로 **discovery+fix 묶어 checkpoint commit 권장**(Owner 승인 필요). branch ahead 187, 이번 slice uncommitted

## R2 provisioning 승인 패킷 (`r2-primary-provisioning-approval-packet-v1` — 2026-07-06, Opus 4.8/high)

**결과 요약: Cloudflare R2를 media.buildgongjakso.com sustained provider primary로 실제 provisioning하기 전 Owner 승인 패킷을 고정. bucket(buildgongjakso-media)/custom domain/object key policy/env 이름 6종/10단계 setup/unknowns U1~U5/rollback 6단계/future approval snippets 7종을 no-secret/no-provisioning으로 문서화. live side effects 0 — Cloudflare login/query 0, bucket/token 0, DNS 0, env/secret write 0, secret read/print 0, deploy 0, object upload 0, liveness 0, Instagram --arm 0, dependency/lockfile 0, commit/push 0.**

- **변경 파일 (신규 3 + report append)**:
  - `docs/r2-provisioning-approval-packet.md` (신규) — 확정 파라미터표(provider/host/bucket/custom domain/object key), env 이름 6종(값 금지) + 역할, R2 setup 10단계, unknowns U1~U5(Cloudflare zone 여부/partial CNAME/account ID/bucket name/DNS provider), rollback 6단계, future approval snippets 7종(APPROVE_R2_*), 금지, 공식문서 근거(R2 public buckets + API tokens)
  - `scripts/fixtures/r2_provisioning_approval_packet.v1.json` (신규) — 기계판독 패킷: provider=cloudflare_r2, sustainedHost 보존, proposedBucketName=buildgongjakso-media, customDomain, objectKeyPolicy(deterministic + objectKeyEqualsUrlPath + 예시 URL/key 일치), requiredEnvNames 6종 + envNamesOnlyNoValues, publicReadViaCustomDomainOnly, r2DevEndpointForProductionForbidden, setupSequence 10, approvalGates 8종, unknowns 5, rollback 6, futureApprovalSnippets 7, verifierGatesInheritedFromStrategy, officialDocRefs, forbiddenBehavior 16종
  - `scripts/check-r2-provisioning-approval-packet-static.mjs` (신규) — dependency-free guard(node:fs/path/url only) **ALL PASS 31/31**: fixture 불변조건(object key = URL path 정합 포함) + docs 명문화 7종 + self-scan + mutant 15종 fail-closed(provider/host drift, bucket/domain 누락, env 누락, env 값 섞임, provisioning/DNS/env/upload/liveness/arm 게이트 누락, rollback 누락, secret logging 제거, bucket/DNS/upload 금지 제거)
- **proposed R2 bucket/custom domain/env names**: bucket=`buildgongjakso-media`, custom domain=`media.buildgongjakso.com`, env(이름만)=`R2_ACCOUNT_ID`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_BUCKET`/`R2_ENDPOINT`/`PUBLIC_MEDIA_BASE_URL` (R2_ENDPOINT는 업로드 전용, public read는 PUBLIC_MEDIA_BASE_URL custom domain만, r2.dev 비-production 금지)
- **future approval snippets**: APPROVE_R2_PROVISIONING / APPROVE_R2_DNS_CUSTOM_DOMAIN / APPROVE_R2_ENV_WRITE / APPROVE_R2_CODE_INTEGRATION / APPROVE_R2_DEPLOY / APPROVE_R2_UPLOAD_TEST_AND_LIVENESS / APPROVE_INSTAGRAM_ARM_WITH_R2_URL (각 게이트 명시 승인 시에만 진행, 이 slice 미실행)
- **rollback plan**: ① disable domain access ② remove custom domain connection ③ revoke token ④ remove env values ⑤ stop uploader path ⑥ one-off repo public/ fallback(명시 승인 시에만). 결정론적 key 불변, host/경로만 교체
- **checks/results**: `git status -sb` ✓ / `node --check` guard ✓ / fixture JSON parse ✓ / 신규 guard **31/31 ALL PASS** / regression `check-media-provider-decision-packet-static.mjs` **32/32** + `check-stable-public-media-url-strategy-static.mjs` **24/24** ALL PASS
- **live side effects: none** — Cloudflare login/account query/mutation 0, R2 bucket/token creation 0, DNS/custom domain 0, env/secret write 0, secret 값 read/print/write 0(env 이름만), deploy 0, object upload 0, public URL liveness 0, Instagram upload/--arm 0, credential/env read 0, broad .env.local 0, YouTube/Supabase/DB/render/mux/TTS/image/browser 0, dependency/lockfile/font 0, commit/push 0. 이번 slice는 문서/fixture/guard 텍스트 작성만 — 새 CLI/network 호출 없음(공식문서 근거는 HANDOFF에 Codex가 이미 검증·기록한 R2 public buckets/tokens 포인트 재사용)
- **보존 확인**: CODEX_REVIEW/NEXT_ACTION/PROJECT_STATE/CONTEXT_TRANSFER_CODEX/piq_diag_out.txt/render-golden-sample-visual-only-v1.mjs/salary_3days manifest/unrelated output·C:\tmp 무접촉. stable strategy + provider decision + upload runner/guard 무수정(regression 32/32·24/24 확인)
...
## dual-platform-content-unit-manifest-parameterization-no-live-v1 (2026-07-07)

- **목적**: 새 영상이 생겼을 때 하드코딩 `t1_lifestyle_inflation/v3_2` 대신 외부 content unit manifest를 받아 dual-platform publish dry-run/preflight plan을 생성. no-live/no-execute. custom content live는 fail-closed.
- **변경 파일**: orchestrator runner(`run-dual-platform-final-publish-orchestrator.mjs`)/guard/fixture(`dual_platform_final_publish_orchestrator.v1.json`), owner runner(`run-owner-daily-automation-entrypoint.mjs`)/guard, docs 2종(`dual-platform-final-publish-orchestrator.md`/`owner-daily-automation-runbook.md`), 신규 fixture(`dual_platform_content_unit.sample.v1.json`), 본 report append. lib 무수정.
- **manifest 계약**(`schemaVersion: dual_platform_content_unit_v1`): 필수 `contentId`/`version`/`instagramSourcePath`/`youtubeSourcePath`, 선택 `instagramMetadata`/`youtubeMetadata`/`blobPublicUrlLivenessEvidence`/`existingPublishedKeys`. `loadContentUnitFromManifest`(JSON.parse만, 네트워크/env 미접근)가 schema/필수필드 검증 후 unit 반환(throw 없이 ok/reasons).
- **default 동작 불변(하위 호환)**: `--content-unit` 없으면 default evidence content로 동작. default `--dry-run`/`--preflight` 출력 필드 유지(신규 필드 부가만), default `--live` 여전히 gate 4 duplicate block `BLOCKED_DUPLICATE_ALREADY_PUBLISHED` exit 3, side effect 0. 실측: default preflight `preflightOk:true`/`duplicateGuardUsesV3_2:true`, default live exit 3.
- **custom 동작**: `--content-unit <path>`로 custom manifest 전달 시 Instagram/YouTube job 생성 + metadata optimization gate 적용 + source 존재 boolean + blob evidence shape 검증(네트워크 재검증 없음, 부재/부정확 시 Instagram readiness fail-closed). custom `--live`/`--arm`은 gate 1~4 판정 뒤 gate 4.5(credential resolution 이전)에서 `CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE`(exit 5)로 fail-closed. 실측: custom preflight `isDefaultContentUnit:false`/`preflightOk:false`(sample source 미존재)/`customContentLiveHaltError` 정확; custom live probe(존재 source 사용) gate 1~4 통과 후 exit 5, credentialResolutionReached/actualApiCallReached false, side effect 0.
- **owner entrypoint 확장**: `--preflight`/`--duplicate-guard-check`에 `--content-unit` 전달. `--status`에 `futureNewVideoContentUnit` 안내(schema/sample). `--duplicate-guard-check`는 custom(non-default) manifest면 `--live`를 **아예 호출하지 않고** fail-closed(`liveInvoked:false`, `confirmed`가 `isDefault &&` 요구). 실측: custom owner preflight exit 1(not ready), custom owner duplicate-guard-check exit 1(liveInvoked:false).
- **checks/results**: node --check 4파일 ✓, fixture JSON parse 2종 ✓. orchestrator guard **347/347**(316→347, content-unit 31항목 추가: 로더/판정/gate4.5 소스+순서, sample fixture, custom preflight 실행, default 불변, custom live gate4.5 exit5 런타임). owner guard **101/101**(81→101, content-unit 20항목: --content-unit 지원, status 안내, custom --live 미호출, custom preflight/dgc smoke). regression: local-pipeline **61/61**, render-manifest **42/42**. smoke: default orch preflight exit0, default owner dgc exit0(safe block), custom orch preflight exit0(preflightOk:false JSON), custom owner preflight exit1(fail-closed not-ready).
- **side effects 0**: Instagram/YouTube API 0, OAuth/upload 0, Vercel Blob upload/list/delete/copy/head 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, 새 미디어/render/ffmpeg 0, dependency/lockfile/deploy 0, commit/push 0. guard의 임시 probe fixture(gate4.5 검증용)는 생성 후 즉시 unlink(잔여 파일 없음 확인). protected/excluded 9종 미접촉.
- **deviations/risks**: 없음. HANDOFF 권장 경로/필드/gate 순서 그대로. custom `--live`의 gate 3(blob liveness)도 default/custom 분기 적용(default는 전역 evidence 상수, custom은 manifest evidence) — 이로써 custom live가 gate 4.5까지 도달 가능. sample fixture는 source 미존재 템플릿이므로 preflightOk:false가 정상(실제 새 영상 렌더 후 ready로 전환).
- checkpoint recommendation: 6개 tracked 변경 + 1개 신규 fixture(+830/-129 중 이번 slice 관련은 orchestrator/owner/docs/fixture). 계약 변경(gate 순서/live 안전)이라 Codex 검토 후 Owner 승인 시 checkpoint commit 권장.

## dual-platform-content-unit-manifest-block-reason-fix-v1 (2026-07-07)

- **Codex review finding**: custom manifest preflight의 실제 live 차단 사유는 `CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE`인데 `liveExecutionPlan.steps[*].willExecuteBlockedReason`이 항상 `duplicate_publish_guard_blocks_current_content`로 하드코딩되어 Owner가 비중복 새 콘텐츠를 "중복 차단"으로 오인할 수 있었음.
- **변경 파일**: `run-dual-platform-final-publish-orchestrator.mjs`(`buildLiveExecutionPlan` 분기), `check-dual-platform-final-publish-orchestrator-static.mjs`(+10 checks), `dual_platform_final_publish_orchestrator.v1.json`(contentUnitManifest.liveExecutionPlanBlockedReasonByContentKind + drift case 1건), docs 2종, 본 report. lib/owner entrypoint 무수정.
- **fix summary**: `buildLiveExecutionPlan`에서 `isDefault = isDefaultContentUnit(unit)`, `bothAlreadyPublished`, `currentContentDuplicateBlocked = isDefault && bothAlreadyPublished`를 계산해 `willExecuteBlockedReason`을 분기(`duplicate_publish_guard_blocks_current_content` vs `custom_content_live_not_enabled_this_slice`). 반환 객체에 `isDefaultContentUnit`/`customContentLiveEnabledThisSlice`/`customContentLiveHaltError`/`willExecuteBlockedReason` 필드 추가, `note`도 콘텐츠 종류별 분기.
- **default 동작**: 실측 `currentContentDuplicateBlocked:true`, 전 step `willExecuteBlockedReason:"duplicate_publish_guard_blocks_current_content"` — 기존과 완전 동일(무변경 확인).
- **custom 동작**: 실측 `currentContentDuplicateBlocked:false`, 전 step `willExecuteBlockedReason:"custom_content_live_not_enabled_this_slice"`, `customContentLiveHaltError:"CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE"`. custom `--live`(존재 source probe로 gate4.5 재검증) 여전히 exit 5, credential/API 미도달, side effect 0.
- **checks/results**: node --check 2파일 ✓, fixture JSON parse ✓. main guard **357/357**(347→357, +10: default blocked-reason 불변 검증 3항목 + custom blocked-reason 정정 검증 5항목 + 소스 하드코딩 금지 검증 2항목). regression: owner guard **101/101** 무변경. smoke: default preflight exit0, custom preflight exit0, HANDOFF 지정 명령(`--live --content-unit sample.json`) exit3(`BLOCKED_SOURCE_FILE_MISSING`, gate2 — sample fixture가 source 미존재 템플릿이라 gate4.5 이전에 차단, credential/API 미도달·side effect 0 확인), gate4.5(exit5) 자체는 기존 존재-source probe로 재검증 통과.
- **side effects 0**: Instagram/YouTube API/OAuth/upload 0, Vercel Blob 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, 새 미디어/ffmpeg 0, dependency/lockfile/deploy 0, commit/push 0. protected/excluded 9종 미접촉.
- **deviations/risks**: 없음. HANDOFF의 smoke 명령 4c는 sample fixture 특성상 exit 5가 아닌 exit 3으로 나오나(gate 2가 gate 4.5보다 먼저 걸림), credential/API 미도달·side effect 0인 안전한 fail-closed 결과이며 gate 4.5 자체는 별도 probe로 재검증 완료.
- checkpoint recommendation: 좁은 보정(3개 파일 diff, blocked-reason 필드/문서/guard만) — Codex 검토 후 Owner 승인 시 이전 checkpoint와 함께 묶어 commit 가능.

## local-pipeline-content-unit-manifest-bridge-no-live-v1 (2026-07-07)

- **목적**: local dry-run pipeline 산출물에서 `dual_platform_content_unit_v1` manifest를 손으로 쓰지 않고 자동 생성. no-live/no-execute(신규 미디어/API/네트워크 호출 없음).
- **신규 파일**: `scripts/build-dual-platform-content-unit-from-local-summary.mjs`(builder, `buildContentUnitFromLocalSummary` export), `scripts/check-dual-platform-content-unit-from-local-summary-static.mjs`(신규 guard), sample fixture 3종(`dual_platform_content_unit_from_local_summary.sample.v1.json` + `...owner_approved_upload_ready_packet.json` + `...blob_liveness_result.json`).
- **변경 파일**: `scripts/run-owner-daily-automation-entrypoint.mjs`(builder import + `--build-content-unit` mode 추가 + `--dry-run`이 생성 summary path surface + `--status` 안내 갱신), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(+14 checks), `docs/owner-daily-automation-runbook.md`(dry-run→manifest→media 첨부→preflight 실제 순서 섹션 추가). `run-local-money-shorts-from-render-manifest.mjs`/`run-local-money-shorts-pipeline-dry-run.mjs`/`check-local-pipeline-runner-static.mjs`/`check-render-manifest-local-runner-static.mjs`는 허용 범위였으나 읽기 전용 참조만 필요해 무수정.
- **bridge command(s)**: `node scripts/build-dual-platform-content-unit-from-local-summary.mjs --summary <path>|--pipeline-summary <path> --out-dir <repo 밖> [--content-id][--version][--youtube-source][--blob-liveness-result]`, 동일 옵션의 owner entrypoint `--build-content-unit` mode.
- **manifest contract summary**: `artifacts.uploadReadyPacket`(두 summary schema 공통)을 읽어 `sourceVideoPath→instagramSourcePath`, `platformPayloads[instagram_reels/youtube_shorts]→instagramMetadata/youtubeMetadata` 변환. `captionFirstLineHook`/`callToAction`은 로컬 pipeline에 별도 필드가 없어 값을 지어내지 않고 빈 문자열+reasons로 fail-closed 보고. `youtubeSourcePath`는 `--youtube-source` 없으면 deterministic placeholder(`youtubeSourceReady:false`). `blobPublicUrlLivenessEvidence`는 `--blob-liveness-result` 명시 시에만 shape-valid 포함(네트워크 검증 없음). readiness booleans 5종(`instagramSourceReady`/`youtubeSourceReady`/`metadataReady`/`blobLivenessEvidenceReady`/`contentUnitPreflightExpectedReady`) 모두 산출.
- **smoke/preflight 결과**: 신규 checked-in sample(source 미존재/hashtag 4개 템플릿)로 builder 실행 → `instagramSourceReady:false`(sourceVideoPath 의도적 미존재)/`youtubeSourceReady:false`/`metadataReady:false`(hashtag 4<8+hook/CTA 없음)/`blobLivenessEvidenceReady:false`. `--blob-liveness-result` 추가 시 `blobLivenessEvidenceReady:true`로 정확히 전환 확인. 생성 manifest를 `run-dual-platform-final-publish-orchestrator.mjs --preflight --content-unit <생성 manifest>`에 그대로 연결 → `isDefaultContentUnit:false`/`preflightOk:false`(예상된 fail-closed)/`sourceFilesReady:false`. 실제 gitignored `render-manifest-local-runner-v1` 산출물(기존에 이미 존재, 신규 미디어 생성 없음)로도 동일 파이프라인 재현: instagramSourceReady:true(실제 mp4 존재)/나머지 fail-closed. owner entrypoint `--build-content-unit`→`--preflight --content-unit`도 동일 결과.
- **checks/results**: node --check 4파일(builder/신규 guard/owner runner/owner guard) ✓, 신규 fixture 3종 JSON parse ✓. 신규 bridge guard **45 PASS / 0 FAIL**. owner guard **115/115**(101→115, +14). regression 무변경: local-pipeline guard **61/61**, render-manifest guard **42/42**, dual-platform orchestrator guard **357/357**.
- **side effects 0**: Instagram/YouTube API/OAuth/upload 0, Vercel Blob upload/list/delete/copy/head 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0(builder에 child_process/spawnSync 자체가 없음 — 순수 fs read/write만), ffmpeg/media generation 0(신규 미디어 0, 기존 gitignored 산출물만 read), dependency/lockfile/deploy 0, commit/push 0. protected/excluded 9종 미접촉. smoke용 레포 밖 temp 산출물은 전부 정리 완료.
- **deviations/risks**: 없음. 다만 로컬 pipeline의 upload metadata는 애초에 `captionFirstLineHook`/`callToAction`을 별도 필드로 갖지 않으므로, 이 bridge만으로는 Instagram metadata gate를 자동으로 통과시킬 수 없다 — Owner/editorial 검토가 별도로 hook/CTA를 채워야 `metadataReady:true`가 된다(값을 지어내지 않고 정확히 fail-closed 보고하는 쪽을 선택).
- checkpoint recommendation: 신규 파일 6개 + 기존 3개 파일 보강, 순수 부가 기능(no-live bridge)이라 기존 계약 변경 없음. Codex 검토 후 Owner 승인 시 checkpoint commit 가능.

## local-pipeline-content-unit-preflight-readiness-fix-v1 (2026-07-07)

- **Codex review finding**: `build-dual-platform-content-unit-from-local-summary.mjs`의 `contentUnitPreflightExpectedReady`가 `instagramSourceReady && youtubeSourceReady && metadataReady`만 보고 `blobLivenessEvidenceReady`를 빠뜨려, Blob evidence 없이도 "preflight 통과 기대"로 오인될 수 있었음.
- **변경 파일**: `scripts/build-dual-platform-content-unit-from-local-summary.mjs`(계산식 1줄 수정), `scripts/check-dual-platform-content-unit-from-local-summary-static.mjs`(+2 checks). docs/CLAUDE_REPORT 외 추가 변경 없음.
- **fix summary**: `contentUnitPreflightExpectedReady = instagramSourceReady && youtubeSourceReady && metadataReady && blobLivenessEvidenceReady`로 수정(AND 조건에 blob evidence 추가).
- **검증**: `--blob-liveness-result` 지정(evidence 있음, `blobLivenessEvidenceReady:true`)해도 sample source/metadata 미완성이면 `contentUnitPreflightExpectedReady:false` 유지 확인. `--blob-liveness-result` 미지정 시(`blobLivenessEvidenceReady:false`)도 여전히 false 확인.
- **checks/results**: node --check 2파일 ✓. 신규 bridge guard **47 PASS / 0 FAIL**(45→47, +2: 계산식 소스 검증 1건 + "blob evidence 단독으로 전체 readiness true 안 됨" 실행 검증 1건). regression 무변경: owner guard **115/115**, dual-platform orchestrator guard **357/357**.
- **side effects 0**: Instagram/YouTube API/OAuth/upload 0, Vercel Blob 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, ffmpeg/media generation 0, dependency/lockfile/deploy 0, commit/push 0. protected/excluded 파일 미접촉. smoke temp 산출물 정리 완료.
...
## owner-youtube-letterbox-source-plan-bridge-no-live-v1 (2026-07-07)

- **목적**: content unit manifest의 `instagramSourcePath`에서 deterministic YouTube Shorts letterbox 출력 경로/render profile 계획을 만드는 no-live planner를 Owner 일일 workflow에 연결. ffmpeg 실행/새 mp4 생성/업로드/API/env 접근 전부 금지.
- **신규 파일**: `scripts/plan-youtube-letterbox-source-from-content-unit.mjs`(planner, `planYoutubeLetterboxSourceFromContentUnit` export), `scripts/check-youtube-letterbox-source-plan-bridge-static.mjs`(신규 guard), `scripts/fixtures/youtube_letterbox_source_plan_from_content_unit.sample.v1.json`(sample content unit manifest, instagramSourcePath 의도적 미존재).
- **변경 파일**: `scripts/run-owner-daily-automation-entrypoint.mjs`(planner import + `--plan-youtube-letterbox` mode 추가 + `--status` ownerNextSteps/futureNewVideoContentUnit 안내 보강), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(+14 checks), `docs/owner-daily-automation-runbook.md`(practical 순서를 3→7단계로 확장, letterbox 계획 단계 삽입).
- **planner command(s)**: `node scripts/plan-youtube-letterbox-source-from-content-unit.mjs --content-unit <dual_platform_content_unit_v1 manifest> --out-dir <repo 밖> [--version-suffix <suffix>]`, 동일 옵션의 owner entrypoint `--plan-youtube-letterbox` mode.
- **generated plan contract summary**: manifest의 `instagramSourcePath`/`contentId`/`version`을 읽어 `scripts/create-youtube-shorts-letterbox-variant.mjs`의 `deterministicYoutubePlaceholderPath`와 동일한 명명 관례로 `plannedYoutubeSourcePath` 산출. `renderProfile`은 실제 renderer의 `PROFILE` 상수(1080x1920 canvas, black background, 864x1536 contentBox centered, 192/108px margin, h264/yuv420p/faststart/aac)와 숫자 값이 정확히 일치하도록 별도 상수(`RENDER_PROFILE`)로 유지(소스 레벨로 drift 검증). `ffmpegPlanCommandString`/`recommendedNextCommand`는 실제 renderer를 `--dry-run`으로 가리키며 `--run`은 포함하지 않음. `inputExists`(fs.existsSync만, 파일 내용 읽지 않음), `outputDirOutsideRepo:true`, `willExecuteFfmpeg:false`, `sideEffectCounters` 5종(ffmpeg/media/api/env/manifest-mutation) 전부 0. content unit manifest는 읽기 전용 — 어떤 필드도 mutate하지 않음(guard가 실행 전후 바이트 동일성으로 확인).
- **sample/owner entrypoint smoke result**: 체크인 sample(instagramSourcePath 의도적 미존재)로 planner 실행 → `inputExists:false`, `plannedYoutubeSourcePath`가 contentId/version 포함한 deterministic mp4 경로, `willExecuteFfmpeg:false`, counters 전부 0. owner entrypoint `--plan-youtube-letterbox --content-unit <sample> --out-dir <repo 밖>` 실행 → exit 0, 동일 plan JSON 생성, stdout에 secret 값 형태 없음. out-dir을 레포 내부로 지정하면 두 스크립트 모두 exit != 0(fail-closed)로 중단하고 디렉터리를 만들지 않음 확인.
- **checks/results**: node --check 4파일(planner/신규 guard/owner runner/owner guard) ✓, 신규 fixture JSON parse ✓. 신규 guard **39 PASS / 0 FAIL**. owner guard **129/129**(115→129, +14). regression 무변경: dual-platform content-unit-from-local-summary guard **67/67**, youtube-shorts-letterbox-variant-renderer guard **36/36**.
- **side effects 0**: Instagram/YouTube API/OAuth/upload 0, Vercel Blob 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, ffmpeg/ffprobe 실행 0(child_process/spawnSync 자체가 planner 소스에 없음 — 순수 fs read/write만), 신규 미디어 생성 0, content unit manifest mutation 0, dependency/lockfile/deploy 0, commit/push 0. protected/excluded 9종 미접촉. smoke용 레포 밖 temp 산출물 전부 정리 완료.
- **deviations/risks**: 없음. 다만 이 slice는 계획만 생성하며, 실제 letterbox mp4 생성은 별도 승인된 후속 slice(`scripts/create-youtube-shorts-letterbox-variant.mjs --run` 활성화)가 필요함 — HANDOFF에서 명시적으로 요구한 범위와 정확히 일치.
- checkpoint recommendation: 신규 파일 3개 + 기존 3개 파일 보강, 순수 부가 기능(no-live planning bridge)이라 기존 계약 변경 없음. Codex 검토 후 Owner 승인 시 checkpoint commit 가능.

## youtube-letterbox-render-execution-wiring-no-execute-v1 (2026-07-07)

- **목적**: `youtube-letterbox-source-plan.json`에서 실행 직전 YouTube letterbox render request JSON을 만드는 no-execute preparer를 Owner 일일 workflow에 연결. ffmpeg 실행/새 mp4 생성/업로드/API/env 접근 전부 금지. `--run`은 이 slice에서 항상 fail-closed.
- **신규 파일**: `scripts/prepare-youtube-letterbox-render-from-plan.mjs`(preparer, `prepareYoutubeLetterboxRenderFromPlan`/`RUN_DISABLED_STATUS` export), `scripts/check-youtube-letterbox-render-execution-wiring-static.mjs`(신규 guard), `scripts/fixtures/youtube_letterbox_render_request_from_plan.sample.v1.json`(sample plan JSON, instagramSourcePath 의도적 미존재).
- **변경 파일**: `scripts/run-owner-daily-automation-entrypoint.mjs`(preparer import + `--prepare-youtube-letterbox-render` mode 추가 + `--status` ownerNextSteps/futureNewVideoContentUnit 안내 보강), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(+16 checks), `docs/owner-daily-automation-runbook.md`(practical 순서를 7→8단계로 확장, render 요청 준비 단계 삽입).
- **render request command(s)**: `node scripts/prepare-youtube-letterbox-render-from-plan.mjs --plan <youtube-letterbox-source-plan.json> --out-dir <repo 밖> [--dry-run | --run]`, 동일 옵션의 owner entrypoint `--prepare-youtube-letterbox-render` mode.
- **generated request contract summary**: plan JSON의 `schemaVersion === youtube_letterbox_source_plan_v1` + `willExecuteFfmpeg === false`를 검증하고, `plannedYoutubeSourcePath`가 레포 밖 + `.mp4` 확장자인지 확인(불일치 시 fail-closed abort). `instagramSourcePath`/`plannedYoutubeSourcePath`/`renderProfile`을 그대로 전달하고 `futureApprovedCommand`(실제 renderer `--dry-run` 형태, `--run` 미포함)를 산출. `runStatus:"YOUTUBE_LETTERBOX_RENDER_RUN_DISABLED_THIS_SLICE"`, `executed:false`, `willExecuteFfmpeg:false`, `sideEffectCounters` 5종(ffmpeg/media/api/env/plan-mutation) 전부 0. `--run` 플래그는 CLI 진입점에서 request 생성 함수 호출 전에 즉시 abort(exit 1) — preparer/owner entrypoint 양쪽 모두 소스 레벨로 순서 검증. plan JSON은 읽기 전용 — mutate하지 않음(guard가 실행 전후 바이트 동일성으로 확인).
- **sample/owner entrypoint smoke result**: 체크인 sample(instagramSourcePath 의도적 미존재)로 preparer 실행 → `inputExists:false`, `futureApprovedCommand`가 정확히 기대값과 일치, `willExecuteFfmpeg:false`, counters 전부 0, exit 0. `--run` 추가 시 preparer/owner entrypoint 모두 exit 1 + request JSON 미생성 확인. schemaVersion 불일치 plan(guard 내부 temp fixture)도 exit != 0 fail-closed 확인. owner entrypoint `--prepare-youtube-letterbox-render --plan <sample> --out-dir <repo 밖>` 실행 → exit 0, 동일 request JSON 생성, stdout에 secret 값 형태 없음.
- **checks/results**: node --check 4파일(preparer/신규 guard/owner runner/owner guard) ✓, 신규 fixture JSON parse ✓. 신규 guard **51 PASS / 0 FAIL**. owner guard **145/145**(129→145, +16). regression 무변경: youtube-letterbox-source-plan-bridge guard **39/39**, youtube-shorts-letterbox-variant-renderer guard **36/36**(--run fail-closed 유지 확인).
- **side effects 0**: Instagram/YouTube API/OAuth/upload 0, Vercel Blob 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, ffmpeg/ffprobe 실행 0(child_process/spawnSync 자체가 preparer 소스에 없음 — 순수 fs read/write만), 신규 미디어 생성 0, plan JSON mutation 0, 기존 renderer(`create-youtube-shorts-letterbox-variant.mjs`)는 수정하지 않음(--run fail-closed 그대로 유지), dependency/lockfile/deploy 0, commit/push 0. protected/excluded 9종 미접촉. smoke용 레포 밖 temp 산출물 전부 정리 완료.
- **deviations/risks**: 없음. 이 slice는 request 준비만 하며, 실제 letterbox mp4 생성은 별도 승인된 후속 slice(renderer `--run` 활성화)가 필요함 — HANDOFF에서 명시적으로 요구한 범위와 정확히 일치.
- checkpoint recommendation: 신규 파일 3개 + 기존 3개 파일 보강, 순수 부가 기능(no-execute render-request wiring)이라 기존 계약 변경 없음. Codex 검토 후 Owner 승인 시 checkpoint commit 가능.

## youtube-letterbox-local-render-execution-once-v1 (2026-07-07)

- **목적**: Owner 승인(`APPROVE_YOUTUBE_LETTERBOX_LOCAL_RENDER_EXECUTION_ONCE`) 하에 기존 승인 source mp4 1개에서 YouTube Shorts letterbox mp4를 로컬 ffmpeg로 **정확히 1회** 생성/검증. 첫 승인된 로컬 media execution slice(ffmpeg 1회 + read-only ffprobe만, API/upload/env/deploy side-effect 0).
- **신규 파일**: `scripts/run-youtube-letterbox-render-from-request-once.mjs`(approval-gated one-shot runner), `scripts/check-youtube-letterbox-local-render-execution-static.mjs`(result/static guard), `scripts/fixtures/youtube_letterbox_local_render_execution_once.v1.json`(result 계약 fixture, 경로 sanitized).
- **변경 파일**: `scripts/run-owner-daily-automation-entrypoint.mjs`(runner import + `--render-youtube-letterbox-once` mode + approval gate + `--status` usage), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(+7 checks), `docs/owner-daily-automation-runbook.md`(5단계를 승인된 실렌더 명령으로 구체화 + 명령표/명령목록 추가).
- **exact runner command used**: `node scripts/run-youtube-letterbox-render-from-request-once.mjs --approval APPROVE_YOUTUBE_LETTERBOX_LOCAL_RENDER_EXECUTION_ONCE` (source/output은 승인 기본값 사용).
- **source/output paths and sizes**: source `C:\tmp\money-shorts-os\golden-sample-chatgpt-playwright-v3-2-script-voice-mux-audit\golden_sample_t1_lifestyle_inflation_tts_mux_v3_2.mp4` (20294549 bytes, 승인값 정확 일치), output `C:\tmp\money-shorts-os\youtube-letterbox-local-render-execution-once-v1\golden_sample_t1_lifestyle_inflation_youtube_letterbox_v3_2_once.mp4` (7349425 bytes).
- **ffmpeg conversion count + ffprobe verification**: `ffmpegConversionCount:1`, `ffprobeReadOnlyCount:2`. 검증 전부 PASS — output 1080x1920, h264, yuv420p, duration Δ0s(source 53.966667s = output), source audio(aac) → output audio(aac) 보존. `allVerificationsPass:true`, exit 0. ffmpeg 인자에 `-y`(overwrite) 없음 — output 미존재를 사전 검증 후 렌더.
- **result JSON path**: `C:\tmp\money-shorts-os\youtube-letterbox-local-render-execution-once-v1\youtube-letterbox-render-result.json` (+ `source-ffprobe.json`, `output-ffprobe.json` 동일 폴더).
- **fail-closed 검증(ffmpeg 0회)**: approval 없음/잘못된 approval → `MISSING_OR_INVALID_APPROVAL_TOKEN`; 잘못된 source → `SOURCE_PATH_NOT_APPROVED`; repo-내부 output → `OUTPUT_INSIDE_REPO`; **재실행 시 output 존재 → `BLOCKED_OUTPUT_ALREADY_EXISTS_NO_RENDER`(output 미변경, 7349425 bytes 유지)**. owner entrypoint도 approval 없으면 하위 runner 위임 전 abort.
- **checks/results**: node --check 4파일 PASS, 신규 fixture JSON parse PASS. 신규 result guard **32 PASS / 0 FAIL**. owner guard **152/152**(145→152, +7). regression 무변경: render-execution-wiring guard **51/51**, source-plan-bridge guard **39/39**, letterbox-variant-renderer guard **36/36**.
- **side effects confirmation**: ffmpeg 실행 정확히 1회(재실행 시도는 전부 fail-closed 0회), ffprobe read-only 2회. Instagram/YouTube API·OAuth·upload 0, Vercel Blob 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, 신규 원본/TTS/image 생성 0, dependency/lockfile/deploy 0, output overwrite/delete 0, commit/push 0. protected/excluded 9종 미접촉. result JSON side-effect counters(api/upload/env/deploy/mutation) 전부 0.
- **deviations/risks**: 없음. 렌더 프로필은 기존 `create-youtube-shorts-letterbox-variant.mjs` PROFILE(scale+pad+black, 1080x1920/864x1536)과 정합. 기존 renderer는 수정하지 않음. 생성된 mp4는 향후 content unit의 `youtubeSourcePath`로 첨부 가능(result JSON `nextStep`에 명시).
- checkpoint recommendation: 신규 파일 3개(runner/guard/fixture) + 기존 3개 파일 보강. 실제 media 1개 생성했으나 레포 밖(gitignored 경로)이라 커밋 대상 아님. Codex 검토 후 Owner 승인 시 코드/문서 checkpoint commit 가능.

## content-unit-youtube-render-result-attach-preflight-no-live-v1 (2026-07-07)

- **목적**: 성공한 YouTube letterbox local render result JSON을 content unit manifest 생성 흐름에 연결 — Owner가 mp4 경로를 손으로 복사하지 않고 `--youtube-render-result <youtube-letterbox-render-result.json>`만 주면 `youtubeSourcePath`가 검증된 `outputPath`로 자동 채워지도록 함.
- **변경 파일**: `scripts/build-dual-platform-content-unit-from-local-summary.mjs`(`readAndValidateYoutubeRenderResult` fail-closed 검증 함수 + `--youtube-render-result` CLI 인자 + mismatch 판정 + `youtubeSourceDerivedFromRenderResult` 필드), `scripts/check-dual-platform-content-unit-from-local-summary-static.mjs`(+22 checks: 소스 정적 검증, 정상/mismatch/같은경로/잘못된검증값 실행 케이스, owner entrypoint forwarding 확인), `scripts/run-owner-daily-automation-entrypoint.mjs`(`--youtube-render-result` forwarding + usage/status 문서), `docs/owner-daily-automation-runbook.md`(6단계를 render-result attach 우선 방식으로 갱신).
- **신규 파일**: `scripts/fixtures/youtube_letterbox_render_result_attach.sample.v1.json`(실제 존재하는 승인 output mp4를 가리키는 sample render result, guard 실행 케이스용).
- **new/updated command**: `node scripts/build-dual-platform-content-unit-from-local-summary.mjs --summary <summary.json> --youtube-render-result <youtube-letterbox-render-result.json> --out-dir <레포 밖>` (owner entrypoint 동일 옵션 forwarding: `--build-content-unit ... --youtube-render-result <path>`).
- **render-result attach contract summary**: fail-closed 검증 항목 — JSON parse, `schemaVersion==="youtube_letterbox_render_result_v1"`, `executed===true`, `allVerificationsPass===true`, `ffmpegConversionCount===1`, `sideEffectCounters.{apiCallCount,uploadCount,envSecretReadCount,deployCount}===0`, `outputPath` non-empty `.mp4` + 레포 밖 + 실제 존재, `outputSizeBytes>0`. 검증 통과 시 `outputPath`를 `youtubeSourcePath`로 채택하고 `youtubeSourceDerivedFromRenderResult:true` 기록. `--youtube-source`와 동시 지정 시 resolve 경로가 다르면 `youtube_source_render_result_mismatch`로 abort, 같으면 통과. `contentUnitPreflightExpectedReady`는 여전히 instagramSourceReady && youtubeSourceReady && metadataReady && blobLivenessEvidenceReady AND 조건 그대로(YouTube source 하나만 준비돼도 전체 true 안 됨).
- **smoke/preflight 결과**: 실제 render result JSON(`C:\tmp\money-shorts-os\youtube-letterbox-local-render-execution-once-v1\youtube-letterbox-render-result.json`)로 builder/owner entrypoint 직접 실행 — `youtubeSourceReady:true`, `youtubeSourceDerivedFromRenderResult:true`, manifest `youtubeSourcePath`가 실제 render result `outputPath`와 정확히 일치 확인. 생성된 manifest를 `--preflight --content-unit`에 연결 → `preflightOk:false`(Instagram source/Blob evidence 미완성으로 예상된 fail-closed 유지), `sourceFilesReady:false`. mismatch(`--youtube-source` 다른 경로 + `--youtube-render-result`) → exit 1 `youtube_source_render_result_mismatch` 확인. 허위 검증값(`allVerificationsPass:false`) render result → exit 1 `youtube_render_result_allVerificationsPass_not_true` 확인.
- **checks/results**: node --check 4파일(builder/guard/owner-entrypoint/fixture) PASS, 신규 fixture JSON parse PASS. 메인 bridge guard **90 PASS / 0 FAIL**(기존 68 + 신규 22). regression 무변경: owner entrypoint guard **152/152**, letterbox local render guard **32/32**, dual-platform final publish orchestrator guard **357/357**.
- **side effects confirmation**: ffmpeg/ffprobe 실행 0(read-only JSON/fs.existsSync/statSync만), Instagram/YouTube API·OAuth·upload 0, Vercel Blob 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, 신규 원본/TTS/image/mux 생성 0, dependency/lockfile/deploy 0, output overwrite/delete 0, commit/push 0. protected/excluded 9종 미접촉.
- **deviations/risks**: 없음. 기존 renderer(`run-youtube-letterbox-render-from-request-once.mjs`)는 수정하지 않음, ffmpeg 재실행 없음. guard 작성 중 기존 `ffmpeg/spawnSync/child_process` 정규식이 `result?.ffmpegConversionCount`(필드 접근, media 생성 아님) 문자열까지 오탐하는 것을 발견해 함수 호출 형태(`spawnSync(`, `ffmpeg(`)만 감지하도록 좁혀 수정(진짜 media 생성 호출은 여전히 차단).
- checkpoint recommendation: 신규 파일 1개(fixture) + 기존 4개 파일 보강. no-live/no-media-generation 순수 부가 기능. Codex 검토 후 Owner 승인 시 checkpoint commit 가능.

## instagram-blob-upload-plan-from-content-unit-no-upload-v1 (2026-07-07)

- **목적**: `dual_platform_content_unit_v1` manifest의 `instagramSourcePath`(full-frame mp4)를 read-only로 읽어 SHA-256을 계산하고, 기존 `lib/instagram-blob-media.ts` no-upload 계약(deterministic pathname/put option plan)을 그대로 미러링해 Instagram Vercel Blob upload request JSON을 생성. 실제 업로드는 하지 않음 — 나중에 승인된 upload 단계가 그대로 쓸 수 있는 deterministic 입력만 만든다.
- **신규 파일**: `scripts/plan-instagram-blob-upload-from-content-unit.mjs`(planner — content unit 검증 + SHA-256 read-only 계산 + deterministic pathname/put option plan 빌더), `scripts/check-instagram-blob-upload-plan-from-content-unit-static.mjs`(static guard), `scripts/fixtures/instagram_blob_upload_plan_from_content_unit.sample.v1.json`(실제 존재하는 로컬 mp4로 생성한 sample request, 경로/사이즈/sha256 실값).
- **변경 파일**: `scripts/run-owner-daily-automation-entrypoint.mjs`(planner import + `--plan-instagram-blob-upload` mode + usage/ownerNextSteps/futureNewVideoContentUnit 문서), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(기존 `no @vercel/blob import`/`no Blob put(/list(/del(/head(` 두 check가 usage 문자열 안의 "@vercel/blob"/"put()" 언급을 오탐하던 것을 발견 — 블록주석+문자열 리터럴을 제거한 코드에서만 검사하도록 수정, 그 외 체크는 무변경), `docs/owner-daily-automation-runbook.md`(명령 목록/명령표에 `--plan-instagram-blob-upload` 추가, 새 영상 순서 절차를 7~9단계로 재구성해 Blob upload plan을 7단계로 삽입). `lib/instagram-blob-media.ts`는 수정하지 않음(기존 계약만 참조).
- **new/updated command**: `node scripts/plan-instagram-blob-upload-from-content-unit.mjs --content-unit <manifest.json> --out-dir <레포 밖>` (owner entrypoint 동일: `--plan-instagram-blob-upload --content-unit <manifest.json> --out-dir <레포 밖>`).
- **upload-plan request contract summary**: 검증 순서 — JSON parse, `schemaVersion==="dual_platform_content_unit_v1"`, `contentId`/`version` non-empty, `instagramSourcePath` non-empty `.mp4` + 실제 존재, size>0 및 ≤35MB(`INSTAGRAM_BLOB_SIZE_CAP_BYTES = 35*1024*1024`). 통과 시 mp4를 read-only로 읽어 SHA-256 계산 → deterministic pathname `instagram/reels/{contentId}/instagram_reels_full_frame_1080x1920/{version}/{sha256_12}.mp4` + put option plan(`access:"public"`, `addRandomSuffix:false`, `allowOverwrite:false`, `multipart:true`, `contentType:"video/mp4"`) 생성. request JSON에 `uploadPerformed:false`, `willUpload:false`, `requiresApprovalToken:"APPROVE_VERCEL_BLOB_OBJECT_UPLOAD_TEST"`, side-effect counters(blob upload/list/head/delete/copy, api, env, deploy, media generation) 전부 0 포함. `--out-dir`은 레포 밖 강제, `.money-shorts-local` 접근 금지, manifest는 mutate하지 않음(읽기 전용).
- **smoke 결과**: 실제 존재하는 로컬 mp4(`C:\tmp\money-shorts-os\golden-sample-chatgpt-playwright-v3-2-script-voice-mux-audit\golden_sample_t1_lifestyle_inflation_tts_mux_v3_2.mp4`, 20294549 bytes)를 가리키는 content unit manifest로 planner/owner entrypoint 양쪽 직접 실행 — `sha256_12:54957450ac10`, `pathname:instagram/reels/t1_lifestyle_inflation/instagram_reels_full_frame_1080x1920/v3_2/54957450ac10.mp4`, `uploadPerformed:false` 확인. fail-closed 케이스(manifest 없음/instagramSourcePath 없음/repo-내부 out-dir/잘못된 schemaVersion) 전부 exit != 0 확인.
- **checks/results**: node --check 3파일(planner/guard/owner-entrypoint) PASS, 신규 fixture JSON parse PASS. 신규 guard **72 PASS / 0 FAIL**. regression 무변경: vercel-blob dependency guard **45/45**, content-unit-from-local-summary guard **90/90**, owner entrypoint guard **152/152**(문자열 오탐 수정 후), dual-platform final publish orchestrator guard **357/357**.
- **side effects confirmation**: `@vercel/blob` import/호출 0, put()/list()/head()/del() 호출 0, ffmpeg/ffprobe 실행 0, Instagram/YouTube API·OAuth·upload 0, Vercel Blob 실제 업로드/liveness 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, dependency/lockfile/deploy 0, content unit manifest mutate 0, commit/push 0. protected/excluded 9종 미접촉. source mp4는 read-only로만 열어 해시 계산(수정/이동 없음).
- **deviations/risks**: 없음. `lib/instagram-blob-media.ts`는 수정하지 않고 상수/pathname 규칙만 참조해 순수 JS로 미러링(TypeScript 빌드 의존성 없이 스크립트 단독 실행 가능하게 하기 위함). guard 작성 중 owner entrypoint guard의 기존 `no @vercel/blob import`/`no Blob put(...)` 두 check가 새로 추가한 usage/문서 문자열(`"...put() option plan..."`, `"...@vercel/blob call..."`)을 실제 import/호출로 오탐하는 것을 발견 — 블록주석+문자열 리터럴 제거 헬퍼를 추가해 실제 코드만 검사하도록 좁혀 수정(진짜 import/호출은 여전히 차단).
- checkpoint recommendation: 신규 파일 3개(planner/guard/fixture) + 기존 3개 파일 보강. no-upload/no-network 순수 부가 기능. Codex 검토 후 Owner 승인 시 checkpoint commit 가능.

## instagram-blob-upload-from-request-executor-no-execute-v1 (2026-07-07)

- **목적**: 이전 slice의 `instagram-blob-upload-request.json`을 소비해 실제 업로드 없이 upload readiness를 증명하는 재사용 가능한 no-execute executor/preflight 계층 추가. source mp4가 여전히 request의 size + SHA-256 + deterministic pathname + put option plan과 일치하는지 read-only로 재검증하고, `--run`은 이 slice에서 fail-closed로 막아 향후 승인 slice가 redesign 없이 열 수 있게 함.
- **신규 파일**: `scripts/prepare-instagram-blob-upload-from-request.mjs`(executor — request 계약 검증 + 현재 source size/SHA-256 재계산 대조 + no-execute preflight JSON 생성, planner에서 계약 상수 import), `scripts/check-instagram-blob-upload-from-request-executor-static.mjs`(static guard), `scripts/fixtures/instagram_blob_upload_from_request_executor.sample.v1.json`(실제 mp4 재검증으로 생성한 sample preflight).
- **변경 파일**: `scripts/run-owner-daily-automation-entrypoint.mjs`(executor import + `--prepare-instagram-blob-upload` mode + `--run` fail-closed + usage/ownerNextSteps/futureNewVideoContentUnit 문서), `docs/owner-daily-automation-runbook.md`(명령 목록/명령표에 `--prepare-instagram-blob-upload` 추가, 새 영상 순서를 8단계=no-execute upload preflight로 삽입 후 9·10단계 재번호). `lib/instagram-blob-media.ts`/`package.json`/`pnpm-lock.yaml` 미수정.
- **new/updated command**: `node scripts/prepare-instagram-blob-upload-from-request.mjs --request <instagram-blob-upload-request.json> --out-dir <레포 밖> [--dry-run | --run]` (owner entrypoint 동일: `--prepare-instagram-blob-upload --request <path> --out-dir <레포 밖> [--dry-run | --run]`).
- **upload executor/readiness contract summary**: fail-closed 검증 — JSON parse, `schemaVersion==="instagram_blob_upload_request_v1"`, `requiresApprovalToken==="APPROVE_VERCEL_BLOB_OBJECT_UPLOAD_TEST"`, `uploadPerformed===false`, `willUpload===false`, `platform==="instagram"`, `variantId==="instagram_reels_full_frame_1080x1920"`, `contentType==="video/mp4"`, contentId/version non-empty, putOptions가 승인 계약(access public/addRandomSuffix false/allowOverwrite false/multipart true/contentType video/mp4)과 정확히 일치, sha256 hex 64자 + sha256_12 일치, pathname이 deterministic 계약과 정확히 일치. 그다음 source mp4 존재+`.mp4` 확인, **현재** size가 `sourceSizeBytes`와 일치, **현재** SHA-256을 read-only로 재계산해 request `sha256`/`sha256_12`와 일치 확인. 전부 통과 시 `instagram-blob-upload-preflight.json`(schemaVersion `instagram_blob_upload_preflight_v1`)에 `readyForFutureApprovedUpload:true`, `runStatus:"INSTAGRAM_BLOB_UPLOAD_RUN_DISABLED_THIS_SLICE"`, `executed:false`/`blobUploadPerformed:false`/`willUpload:false`, side-effect counters 11종(blob put/list/head/delete/copy, publicUrlLiveness, api, env, deploy, media, requestMutation) 전부 0, future approval token + future command template 기록. `--run`은 request 검증/출력 이전에 abort(부작용 0, out-dir 미생성).
- **smoke 결과**: 실제 mp4(20294549 bytes)를 가리키는 fresh request로 executor/owner entrypoint 양쪽 실행 — `sourceSizeMatches:true`, `sha256Matches:true`, `putOptionsMatchApprovedContract:true`, `readyForFutureApprovedUpload:true`, exit 0 확인. `--run`(직접+owner entrypoint 양쪽) → abort exit 1 + out-dir 미생성(부작용 0) 확인. fail-closed 케이스(request 없음/잘못된 token/size mismatch/hash mismatch/putOptions mismatch/pathname mismatch/repo-내부 out-dir) 전부 exit != 0 확인.
- **checks/results**: node --check 2파일(executor/owner-entrypoint) PASS, 신규 fixture JSON parse PASS. 신규 guard **82 PASS / 0 FAIL**(실행 케이스 21종 포함). regression 무변경: plan-from-content-unit guard **72/72**, owner entrypoint guard **152/152**, dual-platform final publish orchestrator guard **357/357**.
- **side effects confirmation**: `@vercel/blob` import/호출 0, put()/list()/head()/del()/copy() 호출 0, public-URL liveness/HEAD 0, ffmpeg/ffprobe 실행 0, child_process/spawnSync 0(executor는 subprocess 없음), Instagram/YouTube API·OAuth·upload 0, OpenAI/ElevenLabs/Pexels/Supabase/browser 0, `.env.local`/`process.env` 접근 0, dependency/lockfile/deploy 0, request JSON/source mp4 mutate 0, commit/push 0. protected/excluded 9종 미접촉. source mp4는 read-only로만 열어 해시 재계산(수정/이동 없음).
- **deviations/risks**: 없음. HANDOFF 문구는 "variant"/"schema/version field"를 개념적으로 언급했으나, planner가 실제 출력하는 request 계약 필드명은 `variantId`/`schemaVersion`이므로 executor는 실제 계약(source of truth)인 `variantId`/`schemaVersion`을 검증(executor 상단 주석에 명시). executor는 planner에서 계약 상수(schema version/platform/variantId/path prefix/content type/approval token)를 import해 단일 진실 공급원 유지 — 계약이 어긋나면 executor가 컴파일/검증 단계에서 드러남.
- checkpoint recommendation: 신규 파일 3개(executor/guard/fixture) + 기존 2개 파일 보강. no-execute/no-upload/no-network 순수 부가 기능, `--run` fail-closed. Codex 검토 후 Owner 승인 시 checkpoint commit 가능.

## instagram-blob-upload-from-request-executor-outdir-root-fix-v1 (2026-07-07)

- **Codex finding**: `prepare-instagram-blob-upload-from-request.mjs --out-dir .`(repo root 자체)가 차단되지 않고 repo root에 `instagram-blob-upload-preflight.json`을 생성. 원인은 `outDirAbs.startsWith(REPO_ROOT + "\\"/"/")`가 repo 하위 경로만 막고 `outDirAbs === REPO_ROOT`(정확히 같은 경로)는 놓치는 것.
- **fix**: `scripts/prepare-instagram-blob-upload-from-request.mjs`에 공통 helper `isRepoRootOrInside(absPath, repoRoot)` 추가(`absPath === repoRoot || startsWith(repoRoot+sep)` 명시적 판정) → 직접 executor의 out-dir 체크가 이 helper 사용. `scripts/run-owner-daily-automation-entrypoint.mjs`는 같은 helper를 import해 `runPrepareInstagramBlobUpload`의 out-dir 체크를 교체하고, 누락돼 있던 `.money-shorts-local` 차단도 이 함수에 추가(직접 executor와 정합화).
- **범위 제한**: owner entrypoint에는 동일한 `startsWith(REPO_ROOT+sep)`-only 패턴이 5곳 더 있음(`--dry-run`/`--build-content-unit`/`--plan-youtube-letterbox`/`--prepare-youtube-letterbox-render`/`--render-youtube-letterbox-once` 관련 out-dir 체크, 각각 line 355/413/477/529/621 부근) — 이번 승인 범위는 `--prepare-instagram-blob-upload` 경로로 한정되어 나머지는 수정하지 않음. 동일 버그 패턴이 잠재적으로 존재하므로 Codex 재검토 시 별도 task로 확장 고려 필요.
- **direct/owner root out-dir repro 결과**: fix 전 재현 없이 바로 fix 적용 후 확인 — 직접 executor `--out-dir .` → exit 1(`ABORT: --out-dir must be outside repo root`), repo root에 `instagram-blob-upload-preflight.json` 미생성. owner entrypoint `--prepare-instagram-blob-upload --out-dir .` → exit 1(동일 메시지), repo root 파일 미생성. 둘 다 확인 완료.
- **checks/results**: node --check 3파일(executor/guard/owner-entrypoint) PASS. 신규 guard(regression 4종 추가: helper 소스 검증 2개 + `--out-dir .` 직접/owner 재현 2쌍) **90 PASS / 0 FAIL**(기존 82 + 신규 8). regression 무변경: plan-from-content-unit guard **72/72**, owner entrypoint guard **152/152**, dual-platform final publish orchestrator guard **357/357**.
- **side effects confirmation**: `@vercel/blob`/put/list/head/del/copy 호출 0, ffmpeg/ffprobe 0, 외부 API/업로드/env 0, request/source mutate 0, commit/push 0. repo root에 확인용 파일 남지 않음(`Test-Path instagram-blob-upload-preflight.json` → False). protected 9종 미접촉.
- **deviations/risks**: 없음. 위 "범위 제한"에 기록한 5개 미수정 위치는 별도 승인 필요.
- checkpoint recommendation: 기존 3개 파일 보강(신규 파일 없음), 좁은 안전 버그 fix. Codex 검토 후 Owner 승인 시 checkpoint commit 가능.

## owner-entrypoint-repo-root-outdir-guard-commonize-v1 (2026-07-07)

- **목적**: `--prepare-instagram-blob-upload`에서 발견/수정된 repo-root out-dir 버그(`startsWith(REPO_ROOT+sep)`가 `outDirAbs===REPO_ROOT` 자체를 놓침)를 owner entrypoint의 나머지 5개 모드에도 공통화.
- **변경 파일**: `scripts/run-owner-daily-automation-entrypoint.mjs`(line 355/413/477/529/621의 raw `startsWith(REPO_ROOT+sep)`를 기존 `isRepoRootOrInside(absPath, repoRoot)` helper 호출로 교체 — 신규 helper 추가 없이 이미 import된 것을 재사용, 총 6개 호출 지점), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(소스 검증 3개 + 5개 모드의 `--out-dir .`/`--out-root .` repo-root-자체 실행 케이스 11개 추가).
- **modes covered**: `--dry-run`(`--out-root`), `--build-content-unit`, `--plan-youtube-letterbox`, `--prepare-youtube-letterbox-render`, `--plan-instagram-blob-upload` — 5개 전부 `.`(repo root 자체) → exit != 0, 예상 output artifact가 repo root에 미생성 확인. `--prepare-instagram-blob-upload`는 이전 slice에서 이미 수정된 상태 유지(변경 없음). `--render-youtube-letterbox-once`는 owner entrypoint 자체에 out-dir 체크가 없고 하위 runner(`run-youtube-letterbox-render-from-request-once.mjs`)로 값만 전달 — 그 하위 runner는 `--out-dir`을 파일명과 결합해 `outputPath`(`.mp4`로 끝나는 파일 경로)를 만들므로 `outDirArg===REPO_ROOT`라도 최종 `outputAbs`는 `REPO_ROOT\<filename>.mp4`가 되어 기존 `outputAbs.startsWith(REPO_ROOT+sep)` 체크가 정상 작동함(파일 클래스 취약점 아님, 허용 파일 목록에도 없어 미수정).
- **direct root-outdir 결과**: `--dry-run --out-root .` → exit 1, local pipeline spawn 전에 실패(러너 관련 stdout 없음), repo root에 summary 미생성. `--build-content-unit --out-dir .` → exit 1, manifest 미생성. `--plan-youtube-letterbox --out-dir .` → exit 1, plan 미생성. `--prepare-youtube-letterbox-render --out-dir .` → exit 1, request 미생성. `--plan-instagram-blob-upload --out-dir .` → exit 1, request 미생성. 5개 전부 repo root 오염 없음 확인.
- **checks/results**: node --check 2파일 PASS. 신규 guard **166 PASS / 0 FAIL**(기존 152 + 신규 14: 소스 체크 3개 + 실행 케이스 11개). regression 무변경: instagram-blob-upload-from-request-executor guard **90/90**, plan-from-content-unit guard **72/72**, dual-platform final publish orchestrator guard **357/357**.
- **side effects confirmation**: `@vercel/blob`/put/list/head/del/copy 0, ffmpeg/ffprobe 0, 외부 API/업로드/env 0, 새 영상 생성 0, commit/push 0. repo root에 6종 예상 output artifact 전부 미생성 확인(`render-manifest-local-run-summary.local-mock.json`/`dual_platform_content_unit.generated.json`/`youtube-letterbox-source-plan.json`/`youtube-letterbox-render-request.json`/`instagram-blob-upload-request.json`/`instagram-blob-upload-preflight.json` 전부 `Test-Path` → False). protected 9종 미접촉.
...
## instagram-existing-blob-liveness-and-content-unit-attach-no-arm-v1 (2026-07-07)

- **목적**: 기존 Vercel Blob 객체(`.../v3_2/54957450ac10.mp4`)의 public URL을 SDK/token 없이 결정론적으로 도출하고, 무인증 `HEAD` 1회로 liveness를 재확인한 뒤 content-unit builder → orchestrator preflight까지 no-arm으로 연결.
- **신규 파일**: `scripts/check-instagram-existing-blob-liveness-no-arm.mjs`(liveness runner — `@vercel/blob` 미사용, `PUBLIC_BLOB_BASE_URL`(이 store의 non-secret public host, orchestrator 코드/이전 완료된 `instagram-blob-url-liveness-no-arm-v1` result.json에 이미 기록된 값)+pathname 문자열 결합으로 URL 도출 → `fetch(url,{method:"HEAD"})` 정확 1회), `scripts/check-instagram-existing-blob-liveness-attach-static.mjs`(static guard, 44 PASS — 오프라인 스모크만 실행하고 자체 네트워크 HEAD는 수행하지 않음).
- **liveness 실행 결과(실제 네트워크 HEAD 1회)**: url `https://7iq7vppwlaha2vuo.public.blob.vercel-storage.com/instagram/reels/t1_lifestyle_inflation/instagram_reels_full_frame_1080x1920/v3_2/54957450ac10.mp4`, headStatus **200**, contentType **video/mp4**, contentLength **20294549**(expected와 일치), urlPathMatchesExpectedPath true → status `LIVE_PUBLIC_URL_OK`, livenessOk true. 결과: `C:\tmp\money-shorts-os\instagram-existing-blob-liveness-no-arm-v1\instagram-existing-blob-liveness-result.json`.
- **content unit attach**: `scripts/build-dual-platform-content-unit-from-local-summary.mjs --summary scripts/fixtures/dual_platform_content_unit_from_local_summary.sample.v1.json --content-id t1_lifestyle_inflation --version v3_2 --blob-liveness-result <위 liveness result>` → `blobLivenessEvidenceReady:true`, manifest에 `blobPublicUrlLivenessEvidence`(url/headStatus 200/contentType/contentLength) 정확히 attach 확인. 생성 manifest: `C:\tmp\money-shorts-os\instagram-existing-blob-liveness-and-content-unit-attach-no-arm-v1\dual_platform_content_unit.generated.json`(checked-in static sample fixture 기반이라 `instagramSourceReady`/`youtubeSourceReady`는 false — golden sample 전용 upload-ready-packet이 아직 없어 의도된 결과).
- **orchestrator preflight**: `--preflight --content-unit <위 manifest>` → `contentUnit.blobLivenessEvidenceOk:true`(gate accepted), `preflightOk:false`(정확한 사유: `sourceFilesReady:false`), `anyStepWillExecute:false`/`anySideEffectPerformed:false`(t1_lifestyle_inflation/v3_2는 duplicate guard로도 이중 차단). publish/upload 0.
- **checks/results**: node --check 2개 신규 파일 PASS, liveness/manifest JSON parse PASS. 신규 guard **44 PASS / 0 FAIL**. regression 무변경: content-unit-from-local-summary guard **90/90**, final publish orchestrator guard **357/357**, blob upload once guard **64/64**.
- **환경 이슈(기능 영향 없음)**: liveness runner를 CLI로 직접 실행하면 result.json이 정확히 write된 후 Node v24.14.1(Windows)에서 `Assertion failed: !(handle->flags & UV_HANDLE_CLOSING)` libuv crash로 exit code가 오염됨(exit -1073740791) — undici fetch keep-alive 소켓 정리 타이밍 문제로 보이는 런타임 버그. `connection:close`/`AbortController`/`setImmediate` 지연 종료를 모두 시도했으나 재현되어 원복. **result.json 내용 자체는 매 실행 정확함**(모든 필드 검증 완료)이므로 guard는 CLI exit code 대신 result.json 파일 내용을 검증한다.
- **side effects confirmation**: Blob SDK/token 사용 0, list/head/put/del/copy 0, HEAD 요청 정확히 1회(GET/download 0), Instagram/YouTube API 0, ffmpeg/ffprobe 0, 새 미디어 0, dependency 변경 0, commit/push 0.
- **env/secret handling**: process.env 접근 0, `.env`/`.env.local` 읽기 0, token 값 read/print 0(애초에 SDK를 안 써서 token 자체가 불필요).
- **deviations/risks**: golden sample(t1_lifestyle_inflation/v3_2) 전용 upload-ready-packet이 프로젝트에 없어 checked-in static sample fixture(`local_sample-bridge-fixture`, 의도적으로 존재하지 않는 source mp4 경로)로 builder를 실행함 — HANDOFF 목표인 "Blob evidence gate accepted + 나머지 실패가 정확히 보고되는지"는 완전히 검증되었으나, `sourceFilesReady:true`까지 도달하는 완전한 golden-sample content unit은 별도 upload-ready-packet 생성 작업이 필요함. Node/libuv exit-code crash는 별도 환경 이슈로 별도 보고.
- checkpoint recommendation: 신규 2 + report. 실제 외부 mutation 0(read-only HEAD 1회만), Blob/Instagram/YouTube publish 전부 0이므로 low-risk. Codex 검토 후 checkpoint commit 가능.

## instagram-existing-blob-liveness-attach-env-guard-fix-v1 (2026-07-07)

- **목적**: Codex review finding 1건 보정 — static guard가 smoke 자식 프로세스에 부모 env를 broad하게 넘기던 부분을 제거해 "process.env 접근 0 / env secret access 0" 계약과 정합화.
- **변경 파일**: `scripts/check-instagram-existing-blob-liveness-attach-static.mjs`, `_ai/CLAUDE_REPORT.md`(append). liveness runner 본체(`check-instagram-existing-blob-liveness-no-arm.mjs`)는 미변경(원래부터 `process.env` 접근 0).
- **env/process.env fix 요약**: `runRunner()`의 `execFileSync(...)` 옵션에서 `env: { ...process.env }`를 **완전히 제거**(옵션 키 자체 삭제). smoke는 전부 offline fail-closed 케이스(missing args, repo-root out-dir, `.money-shorts-local`)라 자식이 어떤 env 값도 소비하지 않으며, `env` 미지정 시 자식은 Node 기본 동작으로 부모 환경을 상속(guard 소스가 env를 직접 spread하지 않음). `env: {}`(빈 객체)는 Windows에서 SystemRoot 부재로 node 실행이 깨질 수 있어 사용하지 않고 **옵션 생략** 방식을 택함.
- **self-regression 체크 3건 추가**(guard가 자기 소스를 검사): ①`env: { ...process.env }` broad spread 부재 ②executable code(정규식 리터럴/문자열/주석 제거 후) 내 `process.env` 직접 참조 부재 ③`execFileSync` 옵션에 `env:` 키 부재. 정규식 리터럴 오탐 방지를 위해 `codeLines`→`stripBlockCommentsAndStrings`→정규식 리터럴 제거 순으로 전처리. 격리 negative test로 회귀 소스는 3건 모두 FAIL, clean 소스는 3건 모두 PASS 검증 완료(원본 파일 미변경, scratchpad 임시 스크립트 실행 후 삭제).
- **기존 runner 안전 체크 유지**: "no process.env access"(runner 본체 대상) 체크는 그대로 유지됨.
- **checks/results**: node --check PASS. guard 실행 **47 PASS / 0 FAIL**(기존 44 + self-regression 3). regression 무변경: content-unit-from-local-summary **90/90**, final publish orchestrator **357/357**, blob upload once **64/64**.
- **confirmation**: public HEAD 재호출 0(기존 result JSON을 read-only로만 확인), env/secret 접근 0, Instagram/YouTube API·Blob mutation·upload 0, commit/push 0.
...
## dual-platform-custom-content-live-credential-gate-no-execute-v1 (2026-07-07)

- **목적**: custom(non-default) content가 gate 1~4(metadata/source/blob/duplicate) readiness를 통과하면 더 이상 옛 gate 4.5 `CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE`(exit 5)에서 무조건 멈추지 않고, credential resolution stub(gate 5)까지 도달한 뒤 `CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`(exit 4)로 fail-closed되게 함. default golden sample duplicate block(exit 3)은 완전 불변. no-execute — 실제 credential/API/publish 없음.
- **변경 파일**: `scripts/run-dual-platform-final-publish-orchestrator.mjs`(gate 4.5 무조건 halt 블록 삭제 → gate 5 credential stub로 자연 진행, gate 5 반환에 custom content 컨텍스트 필드 추가, preflight의 expectedLiveStatus/exitCode·willExecuteBlockedReason·customContentLiveHaltError·note를 credential-gate 의미로 갱신, 상수/헤더 주석 갱신), `scripts/fixtures/dual_platform_final_publish_orchestrator.v1.json`(customContentExpected/liveExecutionPlanBlockedReasonByContentKind/customContentLiveHaltError/prohibitedOrchestratorDrift를 새 계약으로 갱신), `scripts/check-dual-platform-final-publish-orchestrator-static.mjs`(옛 gate 4.5/exit 5 정적 계약 → duplicate-guard-before-credential 순서 + credential stub secret-free 검증으로 교체, 7e probe 블록을 OS temp 기반 ready-probe[exit 4]/missing-blob probe[exit 3]로 재작성), `docs/dual-platform-final-publish-orchestrator.md`(custom content live 섹션·요약 테이블·gate 5 설명 갱신).
- **범위 이탈 1파일**: `scripts/check-owner-daily-automation-entrypoint-static.mjs` 1개 assertion 수정(line 397: `customContentLiveHaltError === CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE` → `... === CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`). 이유: orchestrator 계약 변경으로 owner guard의 직접적 check가 break됨(HANDOFF의 "direct import/check break proves necessary" 예외). owner entrypoint 본체는 이 값을 orchestrator 출력에서 그대로 surface(하드코딩 없음)하므로 미수정.
- **behavior summary — default**: `--live` → exit 3, status `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`, gate 4 duplicate blocked, gate 5 미도달, `credentialResolutionReached:false`, `actualApiCallReached:false`, side-effect counter 11개 전부 0. **불변 확인.**
- **behavior summary — custom missing readiness**: source 미존재 → gate 2 `BLOCKED_SOURCE_FILE_MISSING`(exit 3); blob evidence 부재 → gate 3 `BLOCKED_BLOB_LIVENESS_EVIDENCE`(exit 3). 둘 다 credential 이전 fail-closed, gate 5 미도달, counter 0.
- **behavior summary — custom ready-probe**: gate 1~4 통과 → exit 4, status `CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`, `credentialResolutionReached:true`, `credentialValuesAccessed:false`, `credentialValuesResolved:false`, `actualApiCallReached:false`, counter 전부 0, gate trace 1→2→3→4(미차단)→5(도달, wiredThisSlice:false)→6(미도달, blockedBy:credential_resolution_not_wired).
- **checks/results**: git status-sb OK, node --check(orchestrator/guard) PASS, fixture JSON parse OK. orchestrator guard **374 PASS / 0 FAIL**(신규 credential-gate ready-probe/missing-blob probe 런타임 검증 포함). regression: content-unit-final-readiness **64/64 (exit 0)**, owner-daily-automation-entrypoint **assertion 1건 갱신 후 통과(exit 0)**.
- **side effects confirmation**: Instagram/YouTube API 0, Blob SDK/mutation/HEAD 0, ffmpeg/ffprobe 0, 새 미디어 0, deploy 0, dependency/lockfile 0, commit/push 0. probe manifest는 OS temp(mkdtempSync)에 생성 후 rmSync로 정리 — repo 안 잔존 0.
- **env/secret handling confirmation**: orchestrator에 process.env 접근 0(gate 5 stub이 credentialValues* false 반환, guard가 소스 텍스트로 재검증), `.env`/`.env.local` 읽기 0, credential 값 read/print/hash/copy/log 0, lib live client import/호출 0.
- **deviations/risks**: 위 범위 이탈 1파일(owner guard assertion) 외 없음. 핵심 안전 순서(duplicate guard gate 4가 credential gate 5보다 먼저) 불변이며 guard가 소스 순서로 재검증. `_ai/HANDOFF_NOW.md`는 Codex 갱신분(미수정).
- checkpoint recommendation: 변경 5파일(orchestrator/fixture/orch-guard/docs/owner-guard) + report append, auth-adjacent이나 no-execute·default 불변·gate 순서 불변·counter 0 검증 완료. Codex 검토 후 checkpoint commit 권장.

## dual-platform-credential-preflight-redacted-no-live-v1 (2026-07-08)

- **목적**: Owner가 실제 live publish 직전에 필요한 runtime env key **이름**의 present 여부를 값 없이 확인하는 redacted `--credential-preflight` 명령 추가. no-live·no-secret-value: credential 값/길이/prefix/suffix/hash 미노출, `.env.local`/secret 파일 미read, dotenv 미사용, API/Blob/ffmpeg/deploy 미호출.
- **변경 파일**(8): `scripts/run-dual-platform-final-publish-orchestrator.mjs`(resolveMode에 credential_preflight 판정, isEnvKeyPresentRedacted/buildPlatformKeyPresence/buildCredentialPreflight helper, main에 mode 분기, 헤더 주석), `scripts/run-owner-daily-automation-entrypoint.mjs`(MODES/printUsage/runCredentialPreflight handler/dispatch/--status 안내 — orchestrator에 spawnSync shell:false 위임, 자체 process.env 미접근), `scripts/check-dual-platform-final-publish-orchestrator-static.mjs`(섹션8 credential-preflight 검증 + 기존 "process.env 부재" 2건을 "Boolean(process.env[...]) presence만 허용"으로 정밀화), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(owner credential-preflight 검증 블록), `scripts/fixtures/dual_platform_final_publish_orchestrator.v1.json`(modes.credentialPreflight + prohibitedOrchestratorDrift 6항목), `docs/dual-platform-final-publish-orchestrator.md`(mode 테이블 + 전용 섹션), `docs/owner-daily-automation-runbook.md`(명령 목록 + mode 테이블 행), `package.json`(scripts에 owner:credential-preflight 1줄만 — deps/devDeps 불변).
- **added command/script**: orchestrator `--credential-preflight`, owner entrypoint `--credential-preflight`, `pnpm owner:credential-preflight`.
- **credential preflight behavior**: 접근 방식 `Boolean(process.env[keyName])`(값 미바인딩, non-empty presence만). 출력 = key 이름 + `present` boolean + platform/전체 `allPresent` + `readyForCredentialResolution` + 상수 안전 assertion(credentialValuesAccessed/Resolved/Printed:false, dotEnvLocalDirectAccess:false, externalApiCallPerformed:false). exit 0(status-style; missing key여도 0). present:true/readyForCredentialResolution:true는 presence 신호일 뿐 실제 publish 활성화 아님(credentialResolutionWiredThisSlice:false). 런타임 검증: env 미설정→전부 present:false; 6개 더미값 주입→전부 present:true+allRequiredKeysPresent:true지만 **더미 값 문자열이 출력에 절대 미노출**; custom --content-unit→isDefaultContentUnit:false, env presence 로직 불변.
- **live behavior unchanged**: default `--live` exit 3(BLOCKED_DUPLICATE_ALREADY_PUBLISHED), custom ready-probe exit 4(CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE), default --preflight/--dry-run exit 0 — 모두 불변 확인. orchestrator guard의 default live/custom ready-probe 회귀 통과.
- **checks/results**: [1] git status-sb OK, [2-5] node --check 4개 PASS, [6] package.json parse OK, [7] fixture parse OK, [8] orchestrator guard **399 PASS / 0 FAIL**(credential-preflight present-probe 값 미노출 검증 포함), [9] owner guard **214 PASS / 0 FAIL**(owner credential-preflight present-probe 포함), [10] regression content-unit-final-readiness **64 PASS / 0 FAIL**.
- **side effects confirmation**: Instagram/YouTube API 0, OAuth 0, Blob SDK/mutation/HEAD 0, ffmpeg/ffprobe 0, 새 미디어 0, deploy 0, dependency/lockfile 0, commit/push 0. guard present-probe는 execFileSync env 옵션으로 자식 프로세스에만 더미값 전달(부모/`.env` 미변경).
- **env/secret handling confirmation**: `.env`/`.env.local`/secret 파일 read 0(dotenv/vercel env pull 미사용 — 모든 `.env.local` 언급은 주석/문서 note에만). orchestrator process.env 접근은 승인된 `Boolean(process.env[keyName])` presence 판정에만(guard가 `stripCommentsAndStrings` 기준으로 "승인 패턴 외 process.env 참조 0" + "값 파생 length/slice/substring/crypto/hash 0" 재검증 통과). owner entrypoint 자체 process.env 접근 0(orchestrator 위임). credential 값/길이/prefix/suffix/hash/sample/masked value 출력·저장·log 0(더미값 미노출 런타임 검증 통과).
- **deviations/risks**: 범위 이탈 없음(허용 파일 8개 내). package.json은 scripts 1줄만 변경(deps/devDeps 불변, git diff로 확인). owner entrypoint의 status 안내 문자열은 guard의 `.env.local` executable-string 금지 계약 준수를 위해 "local secret files"로 표기(주석은 정확 명칭 유지). 남은 위험: 없음 — presence:true는 실제 값 유효성이 아니라 key 이름 존재만 의미하며 이를 출력/문서/fixture에 명시.
- checkpoint recommendation: 변경 8파일 + report append(+약 400/-30 규모), auth-adjacent이나 no-execute·값 미노출·default 불변·gate 순서 불변·env 파일 미read 다층 검증 완료. diff가 8파일에 걸쳐 phase-boundary이므로 Codex 검토 후 checkpoint commit 권장.

## dual-platform-credential-preflight-review-fix-v1 (2026-07-08)

- **목적**: Codex review finding 2건 수정(credential-preflight diff 유지, checkpoint 전 정확 수정만).
- **변경 파일**(3): `docs/owner-daily-automation-runbook.md`, `scripts/check-owner-daily-automation-entrypoint-static.mjs`, `scripts/check-dual-platform-final-publish-orchestrator-static.mjs`.
- **Finding A (runbook stale custom live 계약)**: runbook의 옛 `CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE`/`custom_content_live_not_enabled_this_slice`/exit 5 active 서술을 현재 계약으로 교체 — default v3_2 → duplicate guard `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`(exit 3); custom gate 1~4 통과 → credential stub(gate 5) 도달 후 `CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`(exit 4); source missing → gate 2, blob missing → gate 3 먼저 fail-closed; credential 값 접근/API/side effect 0; `--duplicate-guard-check --content-unit`는 custom manifest에서 `--live` 미호출 계약 유지. 옛 halt 상수는 "제거됐다"는 historical 문맥으로만 1회 언급. owner guard의 runbook check도 옛 token 대신 `CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`/`credential_resolution_not_wired_this_slice`를 요구하도록 수정 + 옛 소문자 block reason 부재 검증.
- **Finding B (guard env broad spread 제거)**: 두 guard의 `const cpEnv = { ...process.env }` / `const ownerCpEnv = { ...process.env }`를 제거하고 sanitized `buildSanitizedProbeEnv(dummyKeys, dummyValue)` helper로 교체. helper는 parent env 전체를 복사하지 않고 화이트리스트(`SAFE_CHILD_OS_ENV_KEYS`: SystemRoot/windir/SystemDrive/PATH/Path/PATHEXT/COMSPEC/TEMP/TMP/NUMBER_OF_PROCESSORS/PROCESSOR_ARCHITECTURE — Windows child node 실행용 non-secret OS 변수)만 개별 상속한 뒤 승인된 credential dummy key 6개만 추가. `.env`/dotenv/vercel env pull/secret read 없음.
- **guard self-regression 추가**: 두 guard가 서로+자신 소스에서 실제 spread 문법(`[{[(,]` 뒤 `...process.env`)만 검사(주석/문자열/정규식 리터럴 오탐 회피) → parent env 전체 spread 0; present-probe가 `buildSanitizedProbeEnv`/`SAFE_CHILD_OS_ENV_KEYS` 사용; runner env 접근이 `Boolean(process.env[...])` presence-only; 수집한 credential-preflight stdout에 secret-shaped value 0.
- **checks/results**: [1] rg → `...process.env`/옛 custom token은 주석·negative-check·historical 문맥에만(active 요구 0), [2][3] node --check 2개 PASS, [4] orch guard **405 PASS/0 FAIL**, [5] owner guard **222 PASS/0 FAIL**, [6] regression content-unit-final-readiness **64 PASS/0 FAIL**, [7][8] git diff/status — 이번 task 3파일만 수정.
- **env/secret handling 확인**: `.env`/`.env.local` read 0(dotenv/vercel env pull 미사용). guard의 `{ ...process.env }` broad spread 0(sanitized helper로 교체, self-regression이 실행 코드에서 재검증). present-probe dummy 값 출력 0(런타임 검증 통과). 승인 runner 패턴은 `Boolean(process.env[keyName])` presence-only.
- **side effects 0**: Instagram/YouTube API 0, OAuth 0, Blob SDK/mutation/HEAD 0, ffmpeg/ffprobe 0, 새 미디어 0, deploy 0, dependency/lockfile 0, commit/push 0. present-probe는 sanitized child env로 execFileSync(부모 env/`.env` 미변경).
- **deviations/risks**: 범위 이탈 없음(허용 수정 대상 3파일 내). owner guard의 취약한 exit-5 정규식 검증은 채택하지 않고, 대신 현재 token 강제 + 옛 소문자 token 부재 + 옛 상수는 historical-only 검증으로 대체(유지보수성·정확성 우선). 남은 위험 없음.
- checkpoint recommendation: 이번 review-fix 3파일 + 누적 credential-preflight diff. Codex review finding 2건 모두 해소·재검증 완료 → checkpoint commit 준비 완료. Codex 검토 후 commit 권장.

## dual-platform-credential-resolution-wiring-no-execute-v1 (2026-07-08)

- **목적**: owner no-log wrapper가 child env로 주입한 승인 6 key를 dual-platform orchestrator gate 5에서 in-memory explicit credential 객체로 조립(no-execute). actual API(gate 6)는 계속 fail-closed. Claude 실제 `.env.local` 미read, dummy env만 사용.
- **changed files**(7): `scripts/run-dual-platform-final-publish-orchestrator.mjs`(resolver + gate 5 wiring + 새 상수/상태), `scripts/check-dual-platform-final-publish-orchestrator-static.mjs`(계약 검증 갱신 + dummy-env ready-probe), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(status/runbook 검증 갱신), `scripts/check-owner-local-env-no-log-wrapper-static.mjs`(preflight 필드 검증 갱신), `scripts/fixtures/dual_platform_final_publish_orchestrator.v1.json`(계약 갱신), `docs/dual-platform-final-publish-orchestrator.md` + `docs/owner-daily-automation-runbook.md`(새 계약 문서화), `scripts/run-owner-daily-automation-entrypoint.mjs`(preflight 요약에 새 필드 surface).
- **credential resolution behavior**: `resolveExplicitCredentialsFromRuntimeEnv()`가 승인 6 key(`INSTAGRAM_BUSINESS_ACCOUNT_ID`/`INSTAGRAM_ACCESS_TOKEN`/`YOUTUBE_CLIENT_ID`/`YOUTUBE_CLIENT_SECRET`/`YOUTUBE_REFRESH_TOKEN`/`BLOB_READ_WRITE_TOKEN`)만 `process.env`에서 직접 read → in-memory Instagram/YouTube/Blob 객체 조립. 값은 로컬 `inMemory` 객체에만 존재(반환 summary/gate trace/출력/error에 미노출; 길이/prefix/suffix/hash/masked/token type 미계산). gate 5 도달 시 executeArmedLiveRun이 `inMemory=null`로 참조 정리. 반환은 boolean(`credentialResolutionWiredThisSlice`/`credentialValuesAccessed`/`credentialValuesResolved`) + `missingCredentialKeyNames`(이름만) + platform `allPresent`.
- **default duplicate-block behavior**: default `t1_lifestyle_inflation/v3_2` --live/--arm은 gate 4 duplicate guard가 `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`(exit 3)로 credential resolution 이전에 차단(gate 5/6 미도달, credentialValuesAccessed:false, counters 0). 핵심 안전 순서(gate 4 → gate 5) 불변 — 정적 가드가 소스 순서 검증.
- **custom ready-probe with dummy env**: dummy 6 key 주입 시 gate 1~4 통과 → gate 5 resolved(credentialResolutionReached:true, credentialValuesAccessed:true, credentialValuesResolved:true, missingCredentialKeyNames:[]) → gate 6 비활성이라 `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`(exit 4). DUMMY 값 출력 미노출, actualApiCallReached:false, API/upload/blob/ledger/video counter 0(credential access/resolve만 1).
- **missing credential fail-closed**: env 없이 실행 시 gate 5 도달 → resolved:false → `CREDENTIAL_KEYS_MISSING_THIS_SLICE`(exit 4), 출력에 `missingCredentialKeyNames`(승인 6 key 이름) 배열만, actualApiCallReached:false, 값 미노출.
- **checks/results**: [1] git status-sb OK, [2] node --check runner OK, [3] node --check guard OK, [4] fixture JSON parse OK, [5] orchestrator guard **433 PASS/0 FAIL**, [6a] owner entrypoint guard **225 PASS/0 FAIL**, [6b] wrapper guard **49 PASS/0 FAIL**, [6c] content-unit readiness guard **64 PASS/0 FAIL**. dummy-env 3시나리오(A resolved/B missing/C duplicate) 계약 전부 확인.
- **side effects confirmation**: Instagram/YouTube/Blob API 0, OAuth 0, upload/HEAD/mutation 0, googleapis/fetch 실행 0, ffmpeg/ffprobe 0, 새 media/TTS/image/browser 0, deploy/DNS 0, dependency/lockfile 0, commit/push 0. lib live publish 함수 import 0(문자열 참조만). actual API call gate 6 도달 0.
- **env/secret handling confirmation**: Claude 실제 `.env.local` 미read(구현·테스트 전부 dummy env). runner의 process.env 접근은 (a) Boolean(process.env[...]) presence + (b) resolver의 승인 6 key 직접 read만(guard가 임의 key read/동적 인덱스 read 부재를 자동 검증). credential 값은 in-memory only, 출력/문서/fixture/report/error 미노출. dummy 값 출력 미노출을 guard가 검증. `--credential-preflight` 모드는 여전히 값 미접근(credentialValuesAccessedInThisMode:false), publish 비활성(actualApiExecutionEnabledThisSlice:false) 명시로 wiring:true를 publish 활성화로 오인 방지.
- **deviations/risks**: 범위 이탈 없음. entrypoint runner는 HANDOFF Allowed files 조건(owner-facing summary가 changed status names로 깨짐)에 정확히 해당하여 preflight 요약에 2개 필드만 추가(로직 변경 없음). 임시 검증 스크립트는 scratchpad에 생성 후 정리(repo 잔존 0). 남은 위험: 없음 — actual API 실행(gate 6)은 여전히 별도 승인 slice 필요.
- checkpoint recommendation: 7파일 credential-adjacent 변경(값 런타임 처리 첫 wiring). no-execute·값 미노출·gate 순서·fail-closed 다층 검증 완료. Codex 검토 후 checkpoint commit 권장.

## dual-platform-credential-resolution-owner-status-stale-text-fix-v1 (2026-07-08)

- **stale 문구 수정**: `run-owner-daily-automation-entrypoint.mjs`의 `runStatus()` → `futureNewVideoContentUnit.note`에 남아있던 옛 계약 문구("fail-closes before credential resolution / API call with CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE")를 최신 credential-resolution no-execute 계약으로 교체: gate 1~4는 여전히 credential 이전 fail-closed, gate 1~4 통과 시 gate 5 credential resolution 도달(no-execute) → 6 key 전부 present면 `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`, 일부 누락이면 `CREDENTIAL_KEYS_MISSING_THIS_SLICE`, gate 6(actual API call) 미도달 명시.
- **guard 보정**: `check-owner-daily-automation-entrypoint-static.mjs`에 5개 검증 추가 — entrypoint 소스에 stale token(`CUSTOM_CONTENT_LIVE_NOT_ENABLED_THIS_SLICE`) 부재 + 새 상태 이름 2종(`ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`/`CREDENTIAL_KEYS_MISSING_THIS_SLICE`) 명시 확인(소스 기준) + `--status` 실행 결과(`statusOut`, runtime) 기준으로 stale token 부재 + 최신 계약(gate 5/credential resolution + 새 상태 2종) 설명 확인(active output 기준). 회귀 시 owner guard가 fail하도록 고정.
- **changed files**(2): `scripts/run-owner-daily-automation-entrypoint.mjs`(note 문구 1곳 교체), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(검증 5개 추가).
- **checks/results**: `node --check` entrypoint OK, `node --check` guard OK, `node scripts/check-owner-daily-automation-entrypoint-static.mjs` **230 PASS/0 FAIL**(이전 225 → +5 신규 검증), `node scripts/check-dual-platform-final-publish-orchestrator-static.mjs` regression **433 PASS/0 FAIL**. `rg`로 stale token 재확인: entrypoint/guard 실행 코드·runbook active 문맥에 stale token 없음(guard 소스 내 매치는 검증 로직 자체, runbook 유일 언급은 기존 historical-only 허용 문맥).
- **side effects confirmation**: `.env`/`.env.local`/secret 파일 접근 0, env/secret 값 출력 0, Instagram/YouTube API·OAuth·upload 0, Vercel Blob mutation/HEAD 0, fetch/googleapis 실행 0, ffmpeg/ffprobe 0, 새 영상 생성 0, dependency/lockfile 0, commit/push 0. docs/runbook 무수정(active 오류 위치가 아니므로 범위 밖).
- **deviations/risks**: 범위 이탈 없음(허용 2파일만 수정). 남은 위험 없음 — 이번 수정은 owner-facing 문구/guard만 대상이며 orchestrator 로직·gate 계약은 무변경(regression 433/0으로 확인).
- checkpoint recommendation: 이전 credential-resolution-wiring 9파일 diff에 이번 2파일이 추가됨(총 11파일 누적, 아직 미커밋). Codex 검토 후 checkpoint commit 권장.

## dual-platform-actual-api-call-wiring-no-execute-v1 (2026-07-08)

- **목적**: dual-platform orchestrator gate 6 `actual_api_call`을 no-execute call plan 구조로 wiring. credential resolve(gate 5) 후 실제 Instagram Graph publish / YouTube direct upload / Vercel Blob upload 호출을 value-free call spec으로 명문화하되, 실제 API/upload/OAuth/Blob mutation은 계속 fail-closed. dummy env only, 실제 `.env.local` 미read.
- **changed files**(5): `scripts/run-dual-platform-final-publish-orchestrator.mjs`(buildActualApiCallPlanNoExecute 추가 + gate 6 실행경로 wiring), `scripts/check-dual-platform-final-publish-orchestrator-static.mjs`(gate 6 plan 검증 13개 추가), `scripts/fixtures/dual_platform_final_publish_orchestrator.v1.json`(gate 6 계약 갱신), `docs/dual-platform-final-publish-orchestrator.md` + `docs/owner-daily-automation-runbook.md`(gate 6 no-execute plan 계약 문서화).
- **gate 6 no-execute wiring**: `buildActualApiCallPlanNoExecute(unit, igJob, ytJob, livenessGate, credSummary)`가 value-free call plan을 구성한다 — 3개 call spec(vercel_blob_upload / instagram_graph_publish / youtube_direct_upload), 각 `functionRef`(LIVE_PUBLISH_FUNCTION_REFS 문자열) + `inputReadiness`(sourceFileReady/blobPublicUrlPresent/metadataOptimizationGateOk/credentialsPresent **presence boolean** 등) + `executionEnabled:false` + `executionDisabledReason`. **credential 값을 인자로 받지 않고**(gate 5 presence summary만 사용 — 값이 plan에 담길 원천 제거) process.env 미접근, live lib import/호출 0, fetch/googleapis/youtube.videos.insert/Blob put·list·head·del·copy/OAuth 0. plan에 `requiredApprovalTokensToEnableExecution` + `actualApiCallPerformedThisRun:false` 명시.
- **default duplicate-block behavior**: default `t1_lifestyle_inflation/v3_2` --live는 gate 4 duplicate guard가 `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`(exit 3)로 차단 — credential/gate6 미도달(credentialValuesAccessed:false, actualApiCallReached:false, actualApiCallPlan 없음), counters 0. gate 4→5→6 순서 정적 가드 강제.
- **custom ready-probe with dummy env**: gate 1~4 통과 → gate 5 resolved → **gate 6 no-execute plan 도달**(actualApiCallReached:true, actualApiCallExecutionEnabledThisSlice:false, actualApiCallPerformed:false). actualApiCallPlan에 3 calls(전부 executionEnabled:false, actualCallPerformed:false, functionRef 문자열), credentialsPresent presence boolean, gate6 gateTrace evaluated:true/reached:true/blockedBy:actual_api_call_not_enabled_this_slice. exit 4, `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`. DUMMY 값 plan 포함 전체 출력에 미노출. API/upload/blob/ledger/video counter 0.
- **missing credential fail-closed**: env 없이 실행 시 gate 5에서 `CREDENTIAL_KEYS_MISSING_THIS_SLICE`(exit 4), gate 6 **미도달**(actualApiCallReached:false, actualApiCallPlan 없음), missingCredentialKeyNames(승인 6 이름)만, 값 미노출.
- **checks/results**: [1] git status-sb OK, [2] node --check runner OK, [3] node --check guard OK, [4] fixture JSON parse OK, [5] orchestrator guard **446 PASS/0 FAIL**(이전 433 → +13 gate6 검증), [6a] owner entrypoint guard **230 PASS/0 FAIL**, [6b] wrapper guard **49 PASS/0 FAIL**, [6c] content-unit readiness **64 PASS/0 FAIL**. dummy-env 3시나리오(A gate6 plan/B missing/C duplicate) 런타임 계약 전부 확인.
- **side effects confirmation**: Instagram/YouTube/Blob API 0, OAuth 0, upload/HEAD/mutation 0, googleapis/fetch/youtube.videos.insert 실행 0, ffmpeg/ffprobe 0, 새 media/TTS/image/browser 0, deploy/DNS 0, dependency/lockfile 0, commit/push 0. gate 6 plan builder는 live lib import 0(함수 참조 문자열만). actual API call **실행** 0(plan 구조까지만 wiring).
- **env/secret handling confirmation**: Claude 실제 `.env.local` 미read(구현·테스트 전부 dummy env). gate 6 plan builder는 credential 값 객체(resolution.inMemory)를 인자로 받지 않고 process.env 미접근 — 값이 plan에 유입될 경로 원천 차단. plan 반환에 credential 값 필드(accessToken/refreshToken 등) 0, 값 파생(length/hash/prefix/suffix/masked/token type) 0. DUMMY 값 plan 포함 전체 출력 미노출을 guard/런타임 양쪽에서 검증. runner process.env 접근은 여전히 (a) Boolean presence + (b) gate 5 resolver 승인 6 key 직접 read만.
- **deviations/risks**: 범위 이탈 없음(허용 파일 5개만 수정, owner entrypoint/wrapper guard는 회귀 없어 무수정). 설계 결정: gate 6 plan builder에 in-memory credential 값 객체 대신 gate 5 presence summary만 전달 — HANDOFF의 "in-memory 객체를 로컬 스코프로만" 요구보다 더 엄격한 값-미유입 구조를 선택(fail-closed 강화). 임시 검증 스크립트 scratchpad 생성 후 정리(repo 잔존 0). 남은 위험: 없음 — actual API call **실행**(gate 6 call 수행)은 여전히 별도 승인 slice 필요.
- checkpoint recommendation: 5파일(credential/external-API-boundary 인접, gate 6 첫 wiring). no-execute·값 미노출·gate 순서·plan 값-미유입·fail-closed 다층 검증 완료(orchestrator guard 446/0 + 런타임 3시나리오). 누적 diff 큼 → Codex 검토 후 checkpoint commit 권장.

## dual-platform-actual-api-call-no-execute-stale-contract-fix-v1 (2026-07-08)

- **stale contract text 보정 요약**: gate 6 no-execute wiring 직후 남아있던 이전 계약 문구를 최신 gate 6 계약으로 정합. (1) runner 파일 헤더 mode 설명(`--live/--arm` 항목) — custom content가 `CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE`로 멈춘다는 stale 서술을 "gate 5 resolve 후 gate 6 no-execute plan 도달 → 전부 present 시 `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`, 누락 시 gate 5에서 `CREDENTIAL_KEYS_MISSING_THIS_SLICE`(gate 6 미도달)"로 교체. "이 슬라이스에서 절대 하지 않는 것" 목록도 credential 값 read(gate 5에서 허용됨)와 값 노출/실행(금지)을 명확히 구분. (2) deprecated `CUSTOM_CONTENT_LIVE_NOT_ENABLED_ERROR` 상수 주석 — "credential resolution이 wiring되지 않았기 때문" stale 문구를 "gate 6이 no-execute plan까지만 wiring, 실제 실행 미활성" historical 서술로 교체. (3) `executeArmedLiveRun` 내부 preflight `contentUnit.note` 인접 주석 — gate 6 plan 도달(`actualApiCallReached:true`)과 실행 비활성(`actualApiCallExecutionEnabledThisSlice:false`/`actualApiCallPerformed:false`)을 구분해 명시. (4) `main()`의 `--live/--arm` 진입 주석 — Codex finding 정확 위치, stale `credential resolution stub`/`CREDENTIAL_RESOLUTION_NOT_WIRED_THIS_SLICE` 서술을 gate 5→6 최신 계약으로 전면 교체. (5) `docs/dual-platform-final-publish-orchestrator.md` mode table `live` 행 — "어느 경로든 actual API call 미도달" 삭제, custom ready + credential 전부 present는 `actualApiCallReached:true`(단 `actualApiCallExecutionEnabledThisSlice:false`/`actualApiCallPerformed:false`로 실행 0)로 구분 서술. `owner-daily-automation-runbook.md`는 재확인 결과 stale 문구 없어 무수정.
- **guard 보정 요약**: `check-dual-platform-final-publish-orchestrator-static.mjs`에 7개 회귀 검증 추가 — (a) 파일 헤더 JSDoc(첫 `/** */` 블록)에 stale stub 상태 active 서술 부재 + gate 6/최신 halt 상태 2종 명시 확인, (b) `executeArmedLiveRun` 함수 본문(~main() 진입 주석 직전까지, `armExecSrc`)에 stale stub 상태 active 서술 부재 + `buildActualApiCallPlanNoExecute` 호출·`ACTUAL_API_CALL_NOT_ENABLED_ERROR` 존재 확인, (c) docs mode table live 행에 "어느 경로든 actual API call 미도달" 부재 + `actualApiCallReached:true`/`actualApiCallExecutionEnabledThisSlice:false`/`actualApiCallPerformed:false` 3필드 구분 서술 + 옛 stub 상태 active 언급 부재 확인. 하위호환 상수 정의·historical 주석·guard negative-check 문구 자체는 매치 허용(active 계약 서술만 차단).
- **changed files**(3): `scripts/run-dual-platform-final-publish-orchestrator.mjs`(주석/문서 4곳 stale 텍스트 교체, 로직 무변경), `scripts/check-dual-platform-final-publish-orchestrator-static.mjs`(회귀 검증 7개 추가), `docs/dual-platform-final-publish-orchestrator.md`(mode table live 행 갱신).
- **checks/results**: git status-sb OK, node --check runner OK, node --check guard OK, fixture JSON parse OK, orchestrator guard **453 PASS/0 FAIL**(446→+7), owner entrypoint guard **230 PASS/0 FAIL**, wrapper guard **49 PASS/0 FAIL**, content-unit readiness **64 PASS/0 FAIL**. `rg` 최종 확인: active 계약 위치(runner 헤더/main 진입 주석/docs mode table)에 stale token 0 — 남은 매치는 하위호환 상수 정의 1건, 정확한 missing-credential 주석 1건(stale 아님), guard 자체의 negative-check 문구뿐.
- **side effects/env-secret confirmation**: `.env`/`.env.local`/secret 파일 접근 0, 값/길이/prefix/suffix/hash/masked/token type 출력 0, Instagram/YouTube API·OAuth·upload 0, Vercel Blob mutation/HEAD 0, fetch/googleapis 실행 0, ffmpeg/ffprobe 0, 새 영상 생성 0, dependency/lockfile 0, commit/push 0. 이번 변경은 주석/문서/guard 텍스트만이며 gate 동작·로직은 무변경(모든 regression guard 0 FAIL로 확인).
- **deviations/risks**: 범위 이탈 없음(허용 3파일만 수정, owner-daily-automation-runbook.md는 실제 stale 없어 무수정 — 범위 축소이지 이탈 아님). 남은 위험 없음 — 로직 변경 없이 텍스트/guard만 보정.
- checkpoint recommendation: 이전 미커밋 gate 6 wiring 5파일 diff에 이번 3파일(2개는 겹침: runner/guard 추가 수정, docs 1개 신규)이 누적. 텍스트 보정이라 위험은 낮으나 총 diff가 커지고 있어 Codex 검토 후 checkpoint commit 권장.

## dual-platform-actual-api-executor-wiring-no-run-v1 (2026-07-08)

- **목적**: gate 6 value-free `actualApiCallPlan`을 소비해 실제 publish 실행 순서(Blob upload → Instagram publish → YouTube upload → ledger record)를 no-run executor 구조로 명문화. 실행 플래그는 전부 false 유지, 실제 API/upload/OAuth/Blob/ledger mutation 0. dummy env only, 실제 `.env.local` 미read.
- **no-run executor wiring**: `buildActualApiExecutorNoRun(plan)` 추가 — plan(값 없음)만 인자로 받아 4-step ordered executor 반환. steps: ①`instagram_blob_upload`(dependsOn: []) ②`instagram_publish_reel`(dependsOn: [instagram_blob_upload]) ③`youtube_direct_upload`(dependsOn: [], 독립) ④`publish_ledger_record`(dependsOn: [instagram_publish_reel, youtube_direct_upload]). 각 step: `functionRef`/`executorRef` 문자열, `dependsOn`, inputReadiness(source/URL/metadata/credential **presence boolean**), `requiredApprovalTokens`, `executionEnabled:false`/`willRun:false`/`performed:false`/disabledReason. executor 최상위: `executionEnabledThisSlice:false`/`executorWillRun:false`/`executorPerformed:false`/`actualExecutorRunThisRun:false`/`orderedStepIds`. ledger step은 `PUBLISH_LEDGER_RECORD_FUNCTION_REF`("lib/publish-ledger.ts#recordDualPlatformPublish") string-ref only + `ledgerMutationThisSlice:false`(no-mutation). executor는 credential 값 객체 미수신(plan presence boolean만), process.env 미접근, live lib/ledger import·호출·network·mutation 0. 실행 경로(executeArmedLiveRun)에서 gate 6 plan 직후 `buildActualApiExecutorNoRun(actualApiCallPlan)` 호출 → result에 `actualApiExecutor` + top-level 플래그(`actualApiExecutorReached:true`, `...ExecutionEnabledThisSlice:false`, `...Performed:false`) + gate 6 gateTrace에 executor 플래그 추가.
- **default duplicate-block behavior**: default `t1_lifestyle_inflation/v3_2` --live는 gate 4에서 `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`(exit 3) — credential/gate6 plan/executor 미도달(actualApiCallReached:false, actualApiExecutor 없음, actualApiExecutorReached undefined), counters 0. gate 4→5→6(plan→executor) 순서 정적 가드 강제.
- **custom ready-probe with dummy env**: gate 1~5 통과 → gate 6 plan 도달 → **no-run executor 구성**(actualApiExecutorReached:true, executor 4-step 정확한 순서/dependsOn, 모든 step executionEnabled/willRun/performed:false, ledger no-mutation, executorWillRun/executorPerformed:false). exit 4 `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`. DUMMY 값 executor 포함 전체 출력 미노출, value-derived 필드 0, API/upload/blob/ledger counter 0.
- **missing credential fail-closed**: env 없이 실행 시 gate 5 `CREDENTIAL_KEYS_MISSING_THIS_SLICE`(exit 4), gate 6 plan/executor **미도달**(actualApiExecutor 없음, actualApiExecutorReached !== true), missingCredentialKeyNames(6 이름)만, 값 미노출.
- **checks/results**: [1] git status-sb OK / [2] node --check runner OK / [3] node --check guard OK / [4] fixture JSON parse OK / [5] orchestrator guard **477 PASS/0 FAIL**(453→+24 executor 검증) / [6a] owner entrypoint **230/0** / [6b] wrapper **49/0** / [6c] readiness **64/0**. dummy-env 3시나리오(A executor 구성/B missing/C duplicate) 런타임 계약 전부 확인.
- **side effects confirmation**: Instagram/YouTube/Blob API 0, OAuth 0, upload/HEAD/mutation 0, googleapis/fetch/youtube.videos.insert 0, ledger mutation(writeFileSync/appendFileSync/insert/upsert) 0, ffmpeg/ffprobe 0, 새 media 0, deploy 0, dependency/lockfile 0, commit/push 0. executor는 live lib/ledger import 0(함수 참조 문자열만). actual executor **실행** 0(구조까지만 wiring).
- **env/secret handling confirmation**: 실제 `.env.local` 미read(전부 dummy env). executor는 plan(값 없음)만 받고 credential 값 객체/process.env 미접근 — 값 유입 경로 원천 차단(plan builder와 이중 차단). executor 반환에 credential 값 필드 0, 값 파생(length/hash/prefix/suffix/masked/tokenType) 0. DUMMY 값 executor 포함 미노출을 guard+런타임 양쪽 검증. step credentialsPresent는 presence boolean만.
- **deviations/risks**: 범위 이탈 없음(허용 5파일만 수정, owner entrypoint/wrapper guard 회귀 없어 무수정). 설계 결정 — executor에 plan(값 없음)만 전달하고 credential 값/summary 미수신(HANDOFF의 "value-bearing credential 객체를 local scope로만"보다 엄격한 값-미유입 구조 선택, plan→executor 이중 fail-closed). ledger lib은 아직 미구현이나 string-ref only로 계약만 표현(no-run/no-mutation). 임시 검증 스크립트 scratchpad 생성 후 정리(잔존 0). 남은 위험 없음 — executor step **실행**은 별도 승인 slice 필요.
- checkpoint recommendation: 5파일(external-side-effect 경계, gate 6 executor 첫 wiring, ledger step 신규). no-run·값 미노출·gate 순서·plan→executor 값 미유입·dependsOn·모든 step 실행 비활성 다층 검증 완료(orchestrator guard 477/0 + 런타임 3시나리오). 이전 checkpoint(0550a85) 이후 새 diff → Codex 검토 후 checkpoint commit 권장.

## dual-platform-executor-execution-wiring-no-run-to-arm-ready-v1 (2026-07-08)

- **목적**: gate 6 value-free `actualApiExecutor`를 소비해 실제 실행 경계(execution boundary)를 arm-ready **no-run dispatcher** 구조로 명문화. dispatch는 이 slice에서 비활성 유지(dispatchEnabled/willDispatch/dispatched/performed 전부 false), 실제 API/upload/OAuth/Blob/ledger mutation·dispatch 0. dummy env only, 실제 `.env.local` 미read.
- **arm-ready dispatcher wiring**: `buildActualApiDispatcherNoRun(executor)` 추가 — executor(값 없음)만 인자로 받아 4-step arm-ready dispatcher 반환. `EXPECTED_DISPATCH_STEP_ORDER` 상수(정확한 4-step 순서) 추가. dispatchSteps: ①`instagram_blob_upload`(dependsOn: []) ②`instagram_publish_reel`(dependsOn: [instagram_blob_upload]) ③`youtube_direct_upload`(dependsOn: [], 독립) ④`publish_ledger_record`(dependsOn: [instagram_publish_reel, youtube_direct_upload]). 각 step: `adapterTarget` **문자열 메타데이터**(executor functionRef/executorRef 복사, 실제 import/참조 아님), `dependsOn`, `dependencySatisfiedInStructure`(구조 존재성 boolean), `inputReadiness`(executor→plan에서 복사만 한 presence/readiness boolean), `requiredApprovalTokens`, `dispatchEnabled:false`/`willDispatch:false`/`dispatched:false`/`performed:false`/dispatchDisabledReason. dispatcher 최상위: `dispatchEnabledThisSlice:false`/`dispatcherWillRun:false`/`dispatcherPerformed:false`/`dispatcherDisabledReason`/`executorAccepted`(executor가 기대 4-step disabled 구조일 때만 true)/`orderedStepIds`/`actualDispatcherRunThisRun:false`. dispatcher는 credential 값 객체/summary/env 미수신(executor만), process.env 미접근, live lib/ledger import·호출·network·mutation 0. 실행 경로(executeArmedLiveRun)에서 executor 직후 `buildActualApiDispatcherNoRun(actualApiExecutor)` 호출 → result에 `actualApiDispatcher` + top-level 플래그(`actualApiDispatcherReached:true`, `...EnabledThisSlice:false`, `...Performed:false`) + gate 6 gateTrace에 dispatcher 플래그(dispatchEnabledThisSlice/dispatcherWillRun/dispatcherPerformed:false) + contentUnit에 reachedActualApiDispatcher 추가.
- **default duplicate-block behavior**: default `t1_lifestyle_inflation/v3_2` --live는 gate 4에서 `BLOCKED_DUPLICATE_ALREADY_PUBLISHED`(exit 3) — credential/plan/executor/dispatcher 미도달(actualApiDispatcher 없음, actualApiDispatcherReached undefined), counters 0. gate 4→5→6(plan→executor→dispatcher) 순서 정적 가드 강제.
- **custom ready-probe with dummy env**: gate 1~5 통과 → gate 6 plan/executor 도달 → **arm-ready dispatcher 구성**(actualApiDispatcherReached:true, executorAccepted:true, dispatchSteps 4-step 정확한 순서/dependsOn, 모든 step dispatchEnabled/willDispatch/dispatched/performed:false, ledger adapterTarget string-ref, dispatcherWillRun/dispatcherPerformed:false). exit 4 `ACTUAL_API_CALL_NOT_ENABLED_THIS_SLICE`. DUMMY 값 dispatcher 포함 전체 출력 미노출, API/upload/blob/ledger counter 0.
- **missing credential fail-closed**: env 없이 실행 시 gate 5 `CREDENTIAL_KEYS_MISSING_THIS_SLICE`(exit 4), gate 6 plan/executor/dispatcher **미도달**(actualApiDispatcher 없음, actualApiDispatcherReached !== true), missingCredentialKeyNames(6 이름)만, 값 미노출.
- **checks/results**: [1] git status-sb OK(clean baseline) / [2] node --check runner OK / [3] node --check guard OK / [4] fixture JSON parse OK / [5] orchestrator guard **502 PASS/0 FAIL**(477→+25 dispatcher 검증) / [6a] owner entrypoint **230/0** / [6b] wrapper **49/0** / [6c] readiness **64/0**. dummy-env 3시나리오(A dispatcher 구성/B missing/C duplicate) 런타임 계약 전부 확인.
- **side effects confirmation**: Instagram/YouTube/Blob API 0, OAuth 0, upload/HEAD/mutation 0, googleapis/fetch/youtube.videos.insert 0, ledger mutation(writeFileSync/appendFileSync/insert/upsert) 0, dispatch 0, ffmpeg/ffprobe 0, 새 media 0, deploy 0, dependency/lockfile 0, commit/push 0. dispatcher는 live lib/ledger import 0(adapterTarget 문자열만). actual dispatcher **실행** 0(구조까지만 wiring).
- **env/secret handling confirmation**: 실제 `.env.local` 미read(전부 dummy env). dispatcher는 executor(값 없음)만 받고 credential 값 객체/summary/process.env 미접근 — plan→executor→dispatcher 삼중 값-미유입 차단. dispatcher 반환에 credential 값 필드 0, 값 파생(length/hash/prefix/suffix/masked/tokenType) 0. DUMMY 값 dispatcher 포함 미노출을 guard+런타임 양쪽 검증. step inputReadiness는 executor에서 복사만 한 presence/readiness boolean만.
- **deviations/risks**: 범위 이탈 없음(허용 5파일만 수정, owner entrypoint/wrapper guard 회귀 없어 무수정). 설계 결정 — dispatcher에 executor(값 없음)만 전달하고 credential 값/summary/env 미수신(plan→executor→dispatcher 삼중 fail-closed). `executorAccepted` 수용 게이트 추가(executor가 기대 4-step disabled 구조일 때만 true)로 오염된 executor를 dispatcher가 거부하는 방어선 명문화. ledger lib은 아직 미구현이나 adapterTarget string-ref only로 계약만 표현(no-run/no-mutation). guard의 executor fnBody 경계를 `EXPECTED_DISPATCH_STEP_ORDER`로 조정(dispatcher 삽입으로 executor 소스 검증이 dispatcher까지 포함하지 않도록). 임시 검증 스크립트 scratchpad 생성 후 정리(잔존 0, debug-spread.mjs도 정리). 남은 위험 없음 — dispatcher step **실행(dispatch)**은 별도 승인 slice 필요.
- checkpoint recommendation: 5파일(external-side-effect 경계, gate 6 dispatcher 첫 wiring — 실제 실행 직전 마지막 구조 레이어). no-run·값 미노출·gate 순서·executor→dispatcher 값 미유입·executorAccepted·dependsOn·모든 dispatch step 비활성 다층 검증 완료(orchestrator guard 502/0 + 런타임 3시나리오). 이전 checkpoint(269067d) 이후 새 diff → Codex 검토 후 checkpoint commit 권장.

## publish-ledger-implementation-no-live-v1 (2026-07-08)

- **목적**: 실제 dual-platform publish를 열기 전 필요한 durable duplicate ledger를 `lib/publish-ledger.ts`로 구현. gate 6 dispatcher `publish_ledger_record` step이 참조하는 string-ref(`lib/publish-ledger.ts#recordDualPlatformPublish`)를 실제 함수로 채움. no-live/no-external — 로컬 JSON read/write만, 외부 API/OAuth/Blob/upload/deploy/media 0, explicit path only.
- **changed files**: `lib/publish-ledger.ts`(신규), `scripts/check-publish-ledger-static.mjs`(신규 dependency-free guard), `scripts/fixtures/publish_ledger.sample.v1.json`(신규 sample), `docs/dual-platform-final-publish-orchestrator.md`(ledger 계약 섹션), `docs/owner-daily-automation-runbook.md`(ledger 문단), `_ai/CLAUDE_REPORT.md`. **orchestrator/fixture는 무수정**(선택 옵션이었으나 dispatcher string-ref이 이미 정합 — 회귀 없음).
- **publish ledger API/contract**: exports — `PUBLISH_LEDGER_SCHEMA_VERSION`("publish_ledger_v1"), `buildPublishLedgerKey({contentId,platform,version})`→`${contentId}/${platform}/${version}`, `createEmptyPublishLedger()`, `readPublishLedger(ledgerPath)`, `writePublishLedger(ledgerPath,ledger)`, `checkPublishLedgerDuplicate(ledger,{...})`, `recordPublishLedgerEntry(ledger,entry)`, `recordDualPlatformPublish(ledger,input)`. record shape: key/contentId/platform/version/variantId?/publishedId/publishedUrl?/status/publishedAtIso/metadata?(non-secret only). platform 허용값 instagram_reels|youtube_shorts. import 시점 파일/network/env 부작용 0.
- **read fail-closed 계약**: 파일없음→빈 ledger ok:true/existed:false(throw 없음). invalid JSON→ok:false/invalid_json. non-object→not_object. wrong schemaVersion→wrong_schema_version. non-array records→records_not_array. invalid record shape→record_shape_invalid. 중복 key→duplicate_key_in_records. 모든 fail은 빈 ledger 반환 + 쓰기 없음.
- **write 계약**: ledgerPath 미제공 시 missing_ledger_path 거부(default/production 경로 없음). 부모 dir만 필요 시 생성. records를 key 정렬해 deterministic JSON write(+\n). append-only partial write 없음(전체 원자적 재작성). import 시점 write 0.
- **duplicate behavior**: `checkPublishLedgerDuplicate`→{alreadyPublished, key, record(non-secret) | null}. `recordPublishLedgerEntry`는 동일 key면 duplicateBlocked:true로 삽입 거부(입력 ledger unmutate). `recordDualPlatformPublish`는 IG+YT 동시 기록, 어느 하나라도 기존 key면 anyDuplicateBlocked:true. sample key 정확히 `t1_lifestyle_inflation/{instagram_reels,youtube_shorts}/v3_2`, public id만(IG media_id 17916511431199303 / YT videoId r9jhckdpC9w).
- **temp read/write smoke result**: 실제 함수 트랜스파일→import 후 OS temp에서 **17 behavior PASS/0 FAIL** — buildKey shape, missing→empty ok, write→read round-trip(2 records), write refuses missing path, invalid JSON/wrong schema/non-array/duplicate key 전부 fail-closed, checkDuplicate 정합, record insert 중복 차단 + no-mutation, dual publish ok/blocked, deterministic sort. temp 디렉토리 생성 후 정리(잔존 0).
- **checks/results**: [1] git status-sb OK(clean baseline) / [2] node --check ledger guard OK / [3] ledger guard **44 PASS/0 FAIL** / [4] tsc --noEmit full project **exit 0**(타입 에러 0) / [5a] orchestrator guard **502/0**(회귀 없음) / [5b] owner entrypoint guard **230/0**(docs 변경 후 회귀 없음). 실제 함수 behavior 17/0(별도 트랜스파일 검증).
- **side effects confirmation**: Instagram/YouTube/Blob API 0, OAuth 0, upload/HEAD/list/copy/delete 0, googleapis/fetch/youtube.videos.insert 0, deploy 0, ffmpeg/ffprobe 0, 새 media/TTS/image 0, dependency/lockfile/pnpm/DB schema 0, commit/push 0. ledger는 caller가 넘긴 explicit path에만 local JSON write(default/production write 0). guard/behavior smoke는 OS temp만 사용 후 정리.
- **env/secret handling confirmation**: 실제 `.env.local` 미read(코드/guard/behavior smoke 어디서도). ledger 모듈은 process.env 미접근, dotenv/vercel env pull 0, secret 필드명(accessToken/refreshToken/clientSecret/apiKey/blobToken/authorization) 0, 값 파생(length/prefix/suffix/hash/masked/tokenType/crypto) 0. record/fixture는 public media/video id만. fixture secret-shaped 값(EAA/ya29/vercel_blob_rw) 0을 guard로 검증.
- **deviations/risks**: 범위 이탈 없음(권장 6파일 중 5파일 신규/수정, orchestrator/fixture/orchestrator-guard는 dispatcher string-ref 이미 정합이라 무수정 — 범위 축소이지 이탈 아님). 설계 결정 — read가 record shape의 key === {contentId}/{platform}/{version} 정합까지 검증(위조 key fail-closed), recordPublishLedgerEntry가 입력 ledger를 mutate하지 않고 새 ledger 반환(순수). TS 타입 가드(isValidRecordShape) false 분기에서 entry가 never로 좁혀지는 tsc 에러를 unknown 캐스팅으로 해결. `.ts`는 Node 직접 실행 불가라 guard는 소스 텍스트 계약 lock + fixture 검증 + JSON round-trip smoke로 구성, 실제 함수 behavior는 typescript transpileModule로 별도 end-to-end 검증(17/0). 임시 검증 스크립트 scratchpad 생성 후 정리(잔존 0). 남은 위험 없음 — 실제 orchestrator wiring(dry-run/preflight/live/arm에서 ledger read/write)은 dispatch 활성화와 함께 별도 승인 slice 필요.
- checkpoint recommendation: 3개 신규 파일(lib 모듈 + guard + fixture) + docs 2개 + report. 실제 게시 전 중복 방지 안전장치라 데이터 무결성 중요 — read fail-closed·write explicit-path·no-secret·duplicate 차단 다층 검증 완료(ledger guard 44/0 + tsc 0 + behavior 17/0 + orchestrator/owner 회귀 0). 이전 checkpoint(63fa756) 이후 새 diff → Codex 검토 후 checkpoint commit 권장.

## publish-ledger-implementation-review-fix-v1 (2026-07-08)

- **목적**: Codex review finding 3건 보정. 범위 확장 없음. 실제 게시 전 중복 방지 데이터 오염 방지가 핵심.
- **changed files**: `lib/publish-ledger.ts`, `scripts/check-publish-ledger-static.mjs`, `docs/dual-platform-final-publish-orchestrator.md`, `_ai/CLAUDE_REPORT.md`. (fixture/runbook 무수정 — 계약 변경 없음).
- **Finding 1 (all-or-nothing dual publish)**: `recordDualPlatformPublish` 재작성. IG/YT key를 먼저 둘 다 계산해 원본 `base`에서 `checkPublishLedgerDuplicate`로 중복 판정 → 하나라도 이미 있으면 **어떤 record도 추가 않고 원본 ledger 그대로 반환**(ok:false, anyDuplicateBlocked:true, 각 platform result가 자기 duplicate 상태 정확 표시). 둘 다 신규일 때만 `[...base.records, igEntry, ytEntry]`로 함께 추가한 새 ledger 반환(입력 unmutate). 이전 partial 위험(igResult.ledger 위에 YT 체이닝 → YT dup 시 IG만 남음) 제거.
- **Finding 2 (YouTube metadata)**: `ytEntry`에 `metadata: input.youtube.metadata` 반영(누락 수정). IG와 대칭.
- **Finding 3 (atomic write)**: `writePublishLedger`를 direct writeFileSync → **atomic replace**로 변경. 같은 디렉토리에 temp file(`.{basename}.tmp-{pid}`) write 후 `renameSync`로 원자적 교체(같은 filesystem rename은 원자적). 실패 시 `rmSync`로 temp 정리(leftover 방지). 주석/docs를 atomic replace로 정확히 정합(이전 "원자적으로 다시 쓴다" 표현↔direct write 불일치 해소).
- **all-or-nothing dual publish behavior result**: behavior smoke — both new→ok:true 2 records / YT dup only→ok:false, IG partial 없음(records 1), 원본 reference 그대로, ig result ok:false·dup:false, yt result dup:true / IG dup only→ok:false 대칭 / both dup→ok:false 둘 다 dup:true. 입력 ledger 전 케이스 unmutate 확인.
- **metadata preservation result**: behavior smoke — YouTube record.metadata.yt === "m2", Instagram record.metadata.ig === "m1" 모두 보존 확인.
- **checks/results**: [1] git status-sb OK / [2] node --check ledger guard OK / [3] ledger guard **57 PASS/0 FAIL**(44→+13 review-fix 검증: all-or-nothing 소스 lock + partial 체이닝 회귀 방지 + YT/IG metadata + atomic write temp+rename+leftover 없음) / [4] tsc --noEmit full project **exit 0** / [5] orchestrator guard **502/0**(회귀 없음). 실제 함수 behavior **20 PASS/0 FAIL**(트랜스파일 end-to-end: all-or-nothing 4케이스 + metadata + atomic write/overwrite/leftover-free + read fail-closed 회귀).
- **side effects confirmation**: Instagram/YouTube/Blob API 0, OAuth 0, upload/HEAD/list/copy/delete 0, googleapis/fetch 0, deploy 0, ffmpeg/ffprobe 0, 새 media 0, dependency/lockfile 0, commit/push 0. ledger는 caller explicit path에만 local JSON atomic write(default/production write 0). guard/behavior smoke는 OS temp만 사용 후 정리(temp leftover 0 검증 포함).
- **env/secret handling confirmation**: 실제 `.env.local` 미read. 모듈 process.env 미접근, secret 필드명/값 파생 0. metadata는 non-secret identifier/fingerprint만(behavior smoke도 "m1"/"m2" 같은 non-secret 값). fixture secret-shaped 0 유지.
- **deviations/risks**: 범위 이탈 없음(finding 3건만, 허용 4파일). 설계 결정 — Finding 1에서 shape 무효(igShapeValid/ytShapeValid)도 all-or-nothing 조건에 포함(무효 entry면 삽입 안 함). atomic write는 같은 디렉토리 temp(같은 filesystem 보장)로 cross-device rename 실패 회피. guard는 `.ts` 직접 실행 불가라 all-or-nothing/metadata/atomic을 소스 텍스트 패턴 lock으로 검증 + guard 내부 temp에서 rename 재현 smoke로 계약 확인, 실제 함수는 transpile behavior 20/0으로 end-to-end 검증. 임시 스크립트 정리(잔존 0). 남은 위험 없음.
- checkpoint recommendation: 2 코드 파일(lib+guard) + docs 1 + report. review-fix라 데이터 무결성 개선(partial 기록 방지·atomic write). all-or-nothing·metadata·atomic 다층 검증 완료(ledger guard 57/0 + tsc 0 + behavior 20/0 + orchestrator 회귀 0). 이전 checkpoint(63fa756) 이후 ledger 신규+fix 누적 → Codex 검토 후 checkpoint commit 권장.

## publish-ledger-runtime-readonly-orchestrator-bridge-no-live-v1 (2026-07-08)

- **목적**: durable publish ledger를 dual-platform orchestrator gate 4 duplicate guard에 **read-only fail-closed additive 입력**으로 연결. 실제 publish/upload/API/OAuth/Blob/ledger write는 계속 비활성. no-live/no-external/no-write.
- **changed files**: `lib/publish-ledger-runtime.mjs`(신규 dependency-free read-only adapter), `scripts/run-dual-platform-final-publish-orchestrator.mjs`(--publish-ledger 옵션 + gate 4 additive 연결 + BLOCKED_PUBLISH_LEDGER_READ_FAILED), `scripts/check-dual-platform-final-publish-orchestrator-static.mjs`(ledger bridge smoke+정적), `scripts/run-owner-daily-automation-entrypoint.mjs`(--publish-ledger passthrough), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(passthrough smoke), `docs/dual-platform-final-publish-orchestrator.md` + `docs/owner-daily-automation-runbook.md`(bridge 계약), `_ai/CLAUDE_REPORT.md`.
- **runtime adapter 이유**: `lib/publish-ledger.ts`는 TS 계약이고 orchestrator는 .mjs라 런타임 `.ts` 직접 import(트랜스파일 의존) 불안정. dependency-free `lib/publish-ledger-runtime.mjs`가 read 계약(schemaVersion/platform 허용값/fail-closed reason 코드)을 정확히 미러링. `existsSync`/`readFileSync`만 import, write API(writeFileSync/renameSync/mkdirSync) import조차 안 함. exports: readPublishLedgerReadOnly / checkLedgerDuplicate / buildPublishLedgerKey / evaluateLedgerDuplicateForUnit.
- **ledger bridge behavior**: `--publish-ledger <path>`(옵션, 없으면 bridge 비활성=기존 동작 완전 보존, default/production 경로 없음). gate 4(credential gate 5 이전): 파일없음→빈 ledger ok 계속 / read 실패(invalid_json/not_object/wrong_schema_version/records_not_array/record_shape_invalid/duplicate_key_in_records)→distinct BLOCKED_PUBLISH_LEDGER_READ_FAILED(exit 3, credential 이전) / 매칭 record→additive(reference OR ledger)로 BLOCKED_DUPLICATE_ALREADY_PUBLISHED(exit 3), duplicateSources.{reference,ledger}×{IG,YT} boolean 명시 / 매칭 없음→기존 gate 계속. 출력 publishLedgerBridge에 readOnly:true/writePerformed:false/pathProvided/readOk/existed/readFailReason/recordCount/key/alreadyPublished boolean만(secret 없음).
- **owner entrypoint passthrough**: `--preflight`/`--duplicate-guard-check`가 `--publish-ledger`를 read-only로 orchestrator에 passthrough(경로만 전달, entrypoint는 값 미read). default+sample ledger로 `--duplicate-guard-check` 돌려도 여전히 안전 block(exit 0, BLOCKED_DUPLICATE_ALREADY_PUBLISHED).
- **checks/results**: [1] git status-sb OK / [2~5] node --check(orchestrator/orch guard/owner/owner guard/runtime helper) 전부 exit 0 / [6] publish-ledger guard **57/0**(회귀 없음) / [7] orchestrator guard **539/0**(502→+37: ledger bridge 8 smoke 시나리오 + runtime adapter 정적 + additive/read-fail-closed 소스 lock) / [8] owner-entry guard **239/0**(230→+9: passthrough 소스+operator smoke) / [9] tsc --noEmit full project **exit 0**(TS6053 미발생). 런타임 smoke(guard 내 execFileSync): default+sample→additive dup / no-ledger→기존 동작 / missing→empty ok / empty→gate5/6 / matching→gate4 block(credential 이전) / invalid·wrong-schema·dup-key→BLOCKED_PUBLISH_LEDGER_READ_FAILED. 모든 case side-effect counter 0.
- **side effects confirmation**: Instagram/YouTube/Blob API 0, OAuth 0, upload/HEAD/list/copy/delete 0, googleapis/fetch 0, deploy 0, ffmpeg/ffprobe 0, 새 media 0, dependency/lockfile 0, commit/push 0. **ledger write/mutation 0**(runtime adapter는 read-only, write API import 안 함; orchestrator/entrypoint에 writePublishLedger/recordPublishLedgerEntry/recordDualPlatformPublish/appendFileSync/writeFileSync ledger 접근 0 — guard로 강제). smoke는 OS temp에만 probe ledger write 후 정리(레포 밖, orchestrator 자신은 write 0).
- **env/secret handling confirmation**: 실제 `.env.local` 미read(코드/guard/smoke 어디서도). runtime adapter는 process.env 미접근, dotenv/vercel env pull 0, secret 필드명(accessToken/refreshToken/clientSecret/apiKey/blobToken/authorization) 0, 값 파생(length/prefix/suffix/hash/masked/tokenType/crypto) 0. publishLedgerBridge 출력은 non-secret boolean/count/public key만. dummy-env probe의 DUMMY 값이 전 ledger probe 출력에 미노출 확인.
- **deviations/risks**: 범위 이탈 없음(HANDOFF Approved Scope 내 — likely editable 파일만). 설계 결정 — ① runtime adapter를 `lib/`에 신규(HANDOFF optional 허용, .ts 직접 import 회피) ② ledger read 실패는 duplicate 판정 불가라 BLOCKED_DUPLICATE와 별도 distinct status로 credential 이전 fail-closed(안전 방향) ③ additive는 read ok일 때만 ledger 판정 반영(read 실패는 위 distinct 경로가 선점) ④ owner --duplicate-guard-check는 default content가 reference로 이미 block이라 ledger 유무와 무관하게 안전 block 유지. guard는 기존 dummy-env probe 인프라 재사용(OS temp ledger write 후 정리). 임시 스크립트 없음(전부 guard 내장 smoke). 남은 위험 없음 — 실제 publish wiring(dispatch 활성화)은 별도 승인 slice.
- checkpoint recommendation: adapter 신규 + orchestrator/owner runner 2개 + guard 2개 + docs 2개 + report(약 8파일, +대규모 라인). publish 직전 안전 경계(gate 4 duplicate)에 durable ledger 연결이라 fail-closed 순서·additive·no-write 불변식 중요 — 다층 검증 완료(3 guard 57/539/239 + tsc 0 + 8 런타임 smoke 시나리오 + side-effect 0). 이전 checkpoint(63fa756) 이후 ledger 신규→fix→bridge 누적 → Codex 검토 후 checkpoint commit 권장.

## publish-ledger-runtime-readonly-preflight-gate-fix-v1 (2026-07-08)

- **목적**: Codex review finding 보정 — `--preflight --publish-ledger <invalid>`에서 `publishLedgerBridge.readOk:false`인데 `preflight.preflightOk:true`였던 문제. ledger read 실패 시 preflight도 not-ready(preflightOk:false)로 반영해 operator 오도 방지. review-fix scope만, no-live/no-write.
- **changed files**: `scripts/run-dual-platform-final-publish-orchestrator.mjs`(buildPreflight가 ledgerEvidence 수신 + preflightOk에 반영 + non-secret readiness flag), `scripts/check-dual-platform-final-publish-orchestrator-static.mjs`(preflight readiness smoke C9 + 소스 정적), `scripts/run-owner-daily-automation-entrypoint.mjs`(owner summary에 readiness flag surface + 콘솔 표시), `scripts/check-owner-daily-automation-entrypoint-static.mjs`(owner preflight readiness smoke), `docs/dual-platform-final-publish-orchestrator.md` + `docs/owner-daily-automation-runbook.md`, `_ai/CLAUDE_REPORT.md`.
- **fix summary**: `buildPreflight(unit, ledgerEvidence)`로 시그니처 확장. preflight 핸들러가 `evaluateLedgerDuplicateForUnit`을 `buildPreflight`보다 먼저 평가해 전달. preflightOk 계산에 `!publishLedgerReadFailureBlocksReadiness` 조건 추가(`publishLedgerReadFailureBlocksReadiness = pathProvided && readOk !== true`). preflight 반환에 non-secret flag 4종(publishLedgerPathProvided/publishLedgerReadOk/publishLedgerReadFailureBlocksReadiness/publishLedgerReadFailReason). owner summary도 동일 flag surface + 콘솔에 "publish ledger read ok: false (read FAILED: <reason> — blocks readiness)" 표시.
- **exact preflight invalid-ledger result after fix**: `--preflight --content-unit <ready> --publish-ledger <invalid_json>` → `preflight.preflightOk:false`, `preflight.publishLedgerReadOk:false`, `preflight.publishLedgerReadFailureBlocksReadiness:true`, `preflight.publishLedgerReadFailReason:"invalid_json"`, `publishLedgerBridge.readOk:false`, `publishLedgerBridge.readFailReason:"invalid_json"`, `publishLedgerBridge.writePerformed:false`. exit 0(preflight는 진단 모드라 exit 0; readiness는 boolean으로 표현). owner `--preflight`는 preflightOk:false라 exit 1(not-ready).
- **regression 4케이스 결과**: [1] preflight+invalid→preflightOk false+flag/reason(위) / [2] preflight+missing→readOk true, existed false, readiness 안 막음, preflightOk 유지 / [3] --arm+invalid→BLOCKED_PUBLISH_LEDGER_READ_FAILED exit 3, credential 미도달, side-effect 0(기존 유지) / [4] preflight+no-path→pathProvided false, readiness 안 막음(기존 동작 보존). wrong-schema/duplicate-key도 preflightOk false로 잠금 확인.
- **checks/results**: node --check(orchestrator/orch guard/owner/owner guard) 전부 exit 0 / publish-ledger guard **57/0**(회귀 없음) / orchestrator guard **553/0**(539→+14: preflight readiness smoke C9a~f + 소스 정적 6종) / owner-entry guard **243/0**(239→+4: owner preflight invalid-ledger smoke + flag surface 정적) / tsc --noEmit full project **exit 0**. 4 regression 케이스 수동 실행 전부 기대대로.
- **side effects confirmation**: Instagram/YouTube/Blob API 0, OAuth 0, upload/HEAD/list/copy/delete 0, deploy 0, ffmpeg 0, 새 media 0, dependency/lockfile 0, commit/push 0. **ledger write/mutation 0**(read-only 유지, orchestrator/entrypoint에 write 접근 0). guard smoke는 OS temp에만 probe ledger write 후 정리(레포 밖).
- **env/secret handling confirmation**: `.env.local` 미read. preflight readiness flag는 non-secret(boolean + reason 코드)만. dummy-env/DUMMY 값이 전 preflight/owner probe 출력에 미노출 확인. 값 파생(length/prefix/suffix/hash/masked) 0.
- **deviations/risks**: 범위 이탈 없음(review-fix scope 내, likely editable 파일 + optional owner surface). 설계 결정 — ① readiness block 조건은 `pathProvided && readOk !== true`로 좁혀 missing 파일(readOk:true)/no-path(pathProvided:false)는 기존 동작 보존 ② preflight는 진단 모드라 exit 0 유지하되 preflightOk boolean으로 not-ready 표현(--live만 exit 3 fail-closed) ③ owner summary flag surface는 HANDOFF optional 허용(preflightOk가 이미 exit 1 유발하나 명확성 위해 flag 추가). 임시 스크립트 없음(guard 내장 smoke). 남은 위험 없음.
- checkpoint recommendation: runner 2개 + guard 2개 + docs 2개 + report. review-fix로 preflight readiness 정확성 개선(operator 오도 방지) — 다층 검증 완료(3 guard 57/553/243 + tsc 0 + 4 regression + preflight readiness smoke 12종). 이전 checkpoint(63fa756) 이후 ledger 신규→fix→bridge→preflight-gate-fix 누적 → Codex 검토 후 checkpoint commit 권장.

## final-e2e-ready-content-unit-and-publish-one-v1 (2026-07-08) — ✅ 실제 E2E 게시 성공

- **결과**: **PUBLISHED_DUAL_PLATFORM_OK** — 새 non-default content unit `t2_salary_3days/v1`을 Blob→Instagram→YouTube→ledger 순서로 실제 1회 E2E 게시 완료(exit 0).
- **게시 결과(공개 identifier, non-secret)**:
  - Vercel Blob: `https://7iq7vppwlaha2vuo.public.blob.vercel-storage.com/instagram/reels/t2_salary_3days/instagram_reels_full_frame_1080x1920/v1/00e5ab3fa1af.mp4` (HEAD 200 video/mp4, put ×1 allowOverwrite:false)
  - Instagram Reels: mediaId **18103818062321982** (container 18145942084479092, REELS, poll 9회 후 FINISHED)
  - YouTube Shorts: videoId **T4Kgxhq4cpE** → https://www.youtube.com/shorts/T4Kgxhq4cpE (videos.insert ×1, public)
  - publish ledger: `C:\tmp\money-shorts-os\final-e2e-ready-content-unit-and-publish-one-v1\publish-ledger.json` — 정확히 2 record(`t2_salary_3days/instagram_reels/v1`, `t2_salary_3days/youtube_shorts/v1`), 양쪽 성공 후 all-or-nothing 1회 write.
- **소스 준비**: ffprobe로 `golden_sample_t2_salary_3days_tts_mux.mp4` 검증(1080x1920 h264 30fps + aac, 29.17s, 7,486,733B) → IG source 재사용. YouTube letterbox는 기존 RENDER_PROFILE(864x1536 content box + 1080x1920 black pad, h264/faststart/aac) 미러로 ffmpeg 정확히 1회 렌더 → `golden_sample_t2_salary_3days_youtube_letterbox_v1.mp4`(2,689,521B, 1080x1920, Δduration 0.0007s, audio 보존; pix_fmt yuvj420p=full-range 표기로 플랫폼 호환). content unit manifest는 blueprint(`월급이 3일 만에 사라지는 이유`) 기반 metadata로 작성 → orchestrator `--preflight` **preflightOk:true** (metadata gate/소스/duplicate/blob evidence 전부 통과).
- **changed files**: `scripts/run-final-e2e-dual-platform-publish-once.mjs`(신규 one-shot E2E 러너: 9-gate fail-closed + Blob put/HEAD + IG container→poll→publish + YT insert + all-or-nothing ledger write + secret-sanitized error + one-shot result 차단 + t1/금지 evidence 하드 차단), `scripts/run-owner-command-with-local-env-no-log.mjs`(final-e2e-publish command 추가 — script+baseArgs+passthrough 구조, wrapper 자체는 여전히 API 코드 0), `lib/publish-ledger-runtime-write.mjs`(직전 slice에서 준비된 all-or-nothing writer — E2E 러너가 소비, untracked였던 것을 이 task에서 정식 사용), repo 밖 산출물(letterbox mp4, manifest, ledger, result JSON). `_ai/CLAUDE_REPORT.md`.
- **checks/results**: node --check 3개(e2e runner/wrapper/ledger writer) exit 0 / wrapper guard **49/0** / orchestrator guard **553/0** / publish-ledger guard **57/0** / social live-client guard **23/0** / owner-entry guard **243/0** / tsc --noEmit **exit 0**. 사전 검증: credential 없는 세션에서 gate 8 fail-closed(exit 2) 확인, fake-env wrapper 경유 PREFLIGHT_ONLY_OK(exit 0, 외부 호출 0) + dummy 값 출력/JSON 미노출 확인.
- **실행 후 검증**: result JSON parse OK(PUBLISHED_DUAL_PLATFORM_OK) / ledger 정확히 2 record(t2 key만) / 금지 evidence 비교(IG 18103818062321982≠17916511431199303, YT T4Kgxhq4cpE≠r9jhckdpC9w, t1 재게시 0) / side-effect counters: blobPut 1, blobHead 1, igContainer 1, igPoll 9, igPublish 1, ytInsert 1, ledgerWrite 1, envSecretValuePrint 0 — 의도한 단일 경로와 정확히 일치 / result JSON에 secret-shaped 문자열 0.
- **env/secret handling**: `.env.local` 직접 read 0(모델 컨텍스트 미진입). credential은 approved no-log wrapper가 child env로만 주입, 러너는 process.env→로컬 변수→API 인자로만 사용. 출력/result JSON은 presence boolean만. 에러 텍스트 sanitize(access_token/EAA/ya29/vercel_blob_rw → REDACTED).
- **deviations/risks**: 범위 이탈 없음(HANDOFF 허용 목록 내: ffprobe, letterbox 렌더 1회, manifest 생성, Blob put 1, IG publish 1, YT upload 1, ledger write 1). letterbox once runner는 t1 하드코딩(APPROVED_SOURCE_PATH)이라 미수정하고 동일 RENDER_PROFILE로 직접 ffmpeg 1회 실행(기존 one-shot 계약 보존). 남은 위험: 게시물 품질/노출은 Owner 확인 필요(콘텐츠 자체는 golden-sample 산출물).
- checkpoint recommendation: 직전 미커밋 누적(preflight-gate-fix docs/report) + 이번 신규 2 + wrapper 1 + report. **프로젝트 최종 마일스톤(실제 E2E 게시 성공)** — Codex 검토 후 checkpoint commit 강력 권장.

## owner-web-operator-ui-local-control-v1 (2026-07-08) — ✅ 웹 운영 콘솔 v1 완료

- **목표**: 터미널 명령이 아니라 `/money-shorts` 웹 화면에서 **버튼을 눌러** 준비 상태를 확인하는 운영 콘솔. 실제 게시는 이번 slice에서 잠금.
- **changed files**: `app/api/money-shorts/operator/route.ts`(신규, GET/POST + action enum), `lib/owner-web-operator.ts`(신규, safe local control helper), `components/OperatorPanel.tsx`(신규, client 운영 패널), `app/money-shorts/page.tsx`(큰 CTA + 패널 삽입, 영어 주석 정리), `scripts/check-owner-web-operator-ui-static.mjs`(신규 guard 85 checks), `docs/simple-execution-manual.md`(웹 중심 재작성), `_ai/CLAUDE_REPORT.md`.
- **웹 버튼 5개**: ① 상태 확인(orchestrator `--preflight`) ② 업로드 키 확인(entrypoint `--credential-preflight`, presence boolean만) ③ 재업로드 차단 확인(duplicatePublishReference 해석) ④ 게시 전 점검(final E2E runner, **`--arm` 없이**) ⑤ 실제 업로드 = **disabled/aria-disabled/cursor-not-allowed 잠금**.
- **보안 경계**: 실행 가능한 명령은 helper의 하드코딩 3개 스크립트뿐(임의 명령/셸 문자열 불가). `spawnSync(process.execPath, argsArray, {shell:false, timeout:120s, cwd:repoRoot})`. `FORBIDDEN_ARG_TOKENS=["--arm","--live"]`를 실행 직전 재검사. child env는 승인된 6개 key만 개별 복사(broad spread 없음). `.env.local` 직접 read 0. 출력은 `sanitizeOutput`으로 EAA/ya29/vercel_blob_rw_/GOCSPX/client_secret 치환. finalE2ePreflight의 3개 경로는 절대경로+dash-prefix+null-byte 검증.
- **local vs production**: `isLocalDevRuntime()`로 분기. Vercel/production에서는 스크립트 실행 action을 `LOCAL_AUTOMATION_REQUIRES_LOCAL_DEV`로 차단하고 credential presence만 반환. UI 상단에 「실제 영상 생성과 게시 준비는 Owner PC에서 pnpm dev로 연 로컬 화면에서 실행합니다. 배포 사이트는 상태 확인과 화면 검토용입니다.」 상시 노출.
- **구현 중 발견/수정한 버그**: final E2E runner는 result를 stdout이 아니라 **out-dir 파일**(`final-e2e-publish-preflight.json`)에 쓰고 stdout엔 경로만 낸다. 초기 구현은 stdout JSON 파싱에 의존해 `exitCode:0`(성공)인데도 UI가 "막힘"으로 표시됐다 → `readFinalE2ePreflightResult(validatedOutDir)`로 결과 파일을 읽도록 수정. 또한 이른 gate에서 `bail()`하면 stderr에 `BLOCKED: <CODE>`만 남으므로 정규식으로 blockerCode를 복구하고 `describeE2eBlocker()`로 한국어 설명 매핑 추가.
- **실브라우저 E2E 검증**(dev 서버 + preview 도구, 실제 클릭): ① 상태 확인 → 「시스템 준비 상태가 정상입니다」 ② 업로드 키 확인 → 「업로드 키 6개가 모두 준비되어 있습니다」(raw JSON에 40자+ 문자열은 schemaVersion 식별자 1건뿐, **secret 값 0**) ③ 재업로드 차단 확인 → 「이미 올린 샘플이라 재업로드가 안전하게 차단됐습니다」 ④-a 실제 t2 manifest → `DUPLICATE_ALREADY_PUBLISHED` → 「이미 올린 콘텐츠라 재업로드가 안전하게 차단됐습니다」(재게시 구조적 차단 실증) ④-b 미게시 probe manifest → **`PREFLIGHT_ONLY_OK`, `armed:false`, side-effect counters 전부 0**(blobPut/blobHead/igContainer/igPublish/ytInsert/ledgerWrite = 0), `envSecretValuesPrinted:false`, `dotEnvLocalDirectRead:false` ⑤ 실제 업로드 버튼 클릭 시도 → `disabled:true`, DOM 무변화. **네트워크 요청 전수 확인: 전부 localhost:3000, 외부 API(graph.facebook/googleapis/blob.vercel-storage) 호출 0건.** 콘솔 에러 0.
- **checks/results**: `node --check` guard/entrypoint/wrapper 3개 exit 0 / **신규 guard 85 PASS, 0 FAIL** / `tsc --noEmit` **exit 0**(신규 오류 0) / 기존 guard 회귀 없음: entrypoint 243/0, wrapper 49/0, orchestrator 553/0, publish-ledger 57/0, live-client 23/0.
- **side effects confirmation**: Instagram publish 0, YouTube upload/OAuth 0, Vercel Blob put/del/list/head/copy 0, ledger write 0, `--arm` 실행 0, ffmpeg/새 영상 생성 0, OpenAI/ElevenLabs/Pexels/Supabase 호출 0, dependency/lockfile 0, deploy 0, commit 0, push 0. 검증용 probe 산출물(`C:\tmp\...\web-operator-preflight-check-v1`)은 사후 삭제. **실제 게시 ledger(records 2개)와 산출물 5개는 무변경 보존 확인.**
- **env/secret confirmation**: `.env`/`.env.local` 직접 Read/Edit 0(모델 컨텍스트 미진입). `pnpm dev`가 로드한 env는 route가 presence boolean으로만 소비. 값/길이/prefix/suffix/hash/masked/token type 출력 0. guard가 `maskSecret|valuePrefix|valueHash|tokenType` 식별자 부재와 secret-shaped 리터럴 부재를 강제.
...
## owner-web-auto-topic-refresh-and-upload-button-v1 (2026-07-08) — ✅ 새 주제 추천 + 웹 업로드 버튼 완료

- **목표**: ① `주제 추천받기`가 고정 7개 fixture 반복이 아니라 클릭마다 새 주제 묶음을 내놓고, ② 새 주제가 대본→음성→영상→미리보기→게시 전 점검까지 실제 연결되고, ③ 웹에 확인 게이트형 실제 업로드 버튼을 추가(테스트 중 실업로드 0).
- **changed files**: `lib/owner-web-operator.ts`(8카테고리×12개=96개 로컬 topic bank + `generateWizardTopicBatch`(클릭마다 무작위 9개, 레포 밖 카탈로그 누적) + 생성 주제용 규칙 기반 대본 빌더 + voiceSample 선택 대본 기반 전환 + `buildWizardContentUnitForTopic`(dual_platform_content_unit_v1) + `wizardPreflight`/`actualUpload` 명령 + `--arm`은 ARM_ARG_TOKEN 단일 지점·runOperatorScript `allowArm` 게이트), `app/api/money-shorts/operator/route.ts`(카테고리 enum 검증, 서버측 확인 게이트(체크2+"업로드"), allowArm 단일 부여, runner blocker 한국어 해석, 300s 업로드 timeout), `components/VideoCreationWizard.tsx`(8카테고리 전체 활성, "다른 주제 보기", 주제/카테고리 변경 시 downstream 리셋, 8단계 업로드 UI: 경고+체크2+"업로드" 입력+버튼), `components/OperatorPanel.tsx`(5번을 위저드 안내 카드로), guard 2종 갱신, `docs/simple-execution-manual.md`, 본 report.
- **새 주제 추천 실측**: 재테크팁 2회 클릭 → 9개씩 서로 다른 묶음, 금지 주제(t1/t2/base-rate) 0. 귀여운동물 전환 → 동물 주제 9개(카테고리 반영), 카테고리 전환 시 목록/downstream 리셋 확인.
- **downstream 실측(생성 주제 `gen-animal-summer-paw-care` "여름 산책, 발바닥부터 챙기세요")**: 대본(훅 점수 표시)→음성(mock)→영상 5단계 통과→**실제 mp4 자막이 생성 주제 대본 그대로**("한낮 바닥은 생각보다 뜨겁습니다" 등 6줄)→미리보기 재생(video 태그 로드)→`wizardPreflight`(final E2E runner --arm 없음) **PREFLIGHT_ONLY_OK: 업로드 키 6/6, canonical ledger(t2 기록 2건) 중복 0, 외부 호출 0**.
- **업로드 버튼 안전 실측**: 서버 게이트 4종 fail-closed — 확인 미완료→`UPLOAD_CONFIRMATION_REQUIRED`, 문구 오타→동일 차단, 영상 없음→`video_not_created_yet`, 점검 evidence 없음→`preflight_evidence_missing` (전부 spawn 전 차단). UI 게이트 — 초기/미리보기 후/점검 통과 후/체크만으로는 전부 disabled, 체크2+"업로드" 입력 완료 시에만 enabled(활성화 확인만 하고 **클릭 안 함**), 입력 지우면 다시 disabled, 경고 문구 표시.
- **live upload 0 물증**: canonical ledger LastWriteTime 07-08 11:28(기존 t2 게시 시각) 그대로·records 2건 그대로, publish 폴더에 preflight JSON만 있고 result JSON 없음(armed 실행 0), 네트워크 전수 localhost:3000(외부 도메인 0), 콘솔 에러 0.
- **checks**: wizard guard **102/0**(topic bank/새 묶음/업로드 게이트/allowArm 단일 지점 회귀 체크 포함), web guard **88/0**(--arm 리터럴 1곳·actualUpload 전용·allowArm 실행 게이트 계약), pipeline guard 61/0, `tsc --noEmit` exit 0, `node --check` 2종 exit 0.
- **env/secret**: `.env*` read/edit 0, 값·파생값 출력 0. child env는 승인 6키만 개별 전달(broad spread 없음, 기존 helper 경로 그대로). 결과 표시는 public id/URL/status만.
- **deviations/risks**: 범위 이탈 없음. 알려진 특성 — runner의 one-shot 계약상 armed 실행이 한 번 기록되면(성공/실패 불문) 같은 주제 재-arm은 차단되며 한국어로 안내함(재실행은 Codex/Owner 수동 판단). 충격뉴스/셀럽엔터 등은 실존 인물/미검증 사실 주장을 피한 evergreen 주제로 구성. 실제 첫 웹 업로드는 Owner가 직접 확인 절차를 거쳐 실행하는 것을 권장(테스트에서는 의도적으로 미실행).
- checkpoint recommendation: 수정 8(허용 목록 내) + guard 2종 갱신. 새 추천 엔진 + live 업로드 게이트가 걸린 release-critical 슬라이스 — Codex 검토 후 checkpoint commit 권장.

## owner-web-upload-status-contract-review-fix-v1 (2026-07-08) — ✅ 커밋 전 리뷰 보정 완료

- **목표**: Codex review의 2개 stale/semantic issue 보정. 실제 업로드 경계의 응답 의미와 사용자-facing 잠금 문구를 현재 계약(확인 게이트형 업로드)으로 정확히 맞춤.
- **changed files**: `app/api/money-shorts/operator/route.ts`, `lib/owner-web-operator.ts`, `docs/simple-execution-manual.md`, `scripts/check-owner-one-click-video-creation-ui-static.mjs`, `scripts/check-owner-web-operator-ui-static.mjs`, `_ai/CLAUDE_REPORT.md`.
- **Finding 1 (noLive 의미) 보정**: `OperatorResponse.noLive` 타입을 `true` 리터럴 → `boolean`으로 변경하고, `liveRunnerInvoked?: boolean` 필드 추가. actualUpload에서 `runOperatorScript(..., allowArm:true)`로 arm runner를 **실제 spawn한 이후**의 응답(성공/timeout/부분게시/실패)은 `noLive:false` + `liveRunnerInvoked:true`로 표시. arm 이전 차단(확인 미완료/build 실패/`SCRIPT_NOT_RUN`=spawn 시작 안 됨)은 `noLive:true` 유지. secret 값은 응답 어디에도 없음(public id/url/status만).
- **actualUpload 응답 계약 요약**: ① 확인 게이트 미통과 → `noLive:true`(UPLOAD_CONFIRMATION_REQUIRED) ② build 실패(영상/점검 evidence 없음 등) → `noLive:true` ③ spawn 실패(SCRIPT_NOT_RUN) → `noLive:true` ④ arm 실행 후 성공 → `noLive:false, liveRunnerInvoked:true` ⑤ arm 실행 후 timeout → `noLive:false, liveRunnerInvoked:true` ⑥ arm 실행 후 부분게시/실패 → `noLive:false, liveRunnerInvoked:true`. 경계선은 `allowArm:true` spawn 호출 지점이며 `runUp.ran===false`까지는 no-live, 그 이후는 live.
- **Finding 2 (stale 문구) 보정**: (a) route status raw의 `actualUploadEnabled:false` → `actualUploadAvailableOnLocalDev: localDev`, note는 localDev 여부에 따라 "확인 게이트 후 실행" / "로컬 화면에서만 가능"으로 동적화. (b) route finalE2ePreflight interpret detail "실제 업로드는 별도 승인 후 활성화됩니다" → 확인 게이트(체크 2 + "업로드" 입력) 문구로 교체. (c) `lib/owner-web-operator.ts` 상단 주석 "`--arm`/`--live`/`--approval` 어떤 경로로도 안 붙는다" → actualUpload 전용 `--arm`(ARM_ARG_TOKEN 단일 지점 + allowArm 게이트) 계약으로 정정. (d) `docs/simple-execution-manual.md` §4 "실제 업로드는 아직 잠겨 있습니다" → "실제 업로드 (확인 게이트)" 4조건 + 로컬 전용/중복 차단 안내로 재작성. (e) web guard 헤더 주석 "실제 업로드 버튼 disabled/잠김" → "점검 패널은 실행 안 하고 위저드 확인 게이트로 안내" 정정.
- **회귀 방지 guard 추가**(wizard guard): `noLive:boolean` 타입 강제, `liveRunnerInvoked` 필드 존재, arm 이후 응답에 `noLive:false` ≥3회, status raw에 `actualUploadEnabled:false` 부재, `actualUploadNote`에 "별도 승인 후 활성화" 부재.
- **checks**: wizard guard **107/0**(신규 noLive/stale 체크 5개 포함), web guard **88/0**, `tsc --noEmit --pretty false` **exit 0**(Codex 환경의 TS6053은 이 세션에서 재현 안 됨 — 환경성 이슈로 판단), `node --check` 2종 exit 0. active stale 문구 전수 재조사(app/components/lib/docs) → 허용 파일 내 0(runbook의 별개 CLI 러너 계약 문맥만 잔존, 정당·미접촉).
- **side effects / env·secret**: 실제 upload/API/OAuth/Blob/ledger write 0(문서·주석·타입·문구만 수정, 로직 동작 불변). `.env*` read/edit 0, 값/파생값 출력 0. deploy/push/commit 0. 기존 게시 evidence 재시도 0.
...
## owner-web-real-script-voice-visual-generation-pipeline-v1 (2026-07-09) — ✅ 웹 위저드 실제 제작 파이프라인 연결 완료

- **목표**: /money-shorts 위저드를 실제 제작 흐름으로 연결 — 로컬 후보 선별 → Claude 1회 대본 보정 → 실제 ElevenLabs TTS → ChatGPT+Playwright 장면 이미지 6장 → 최종 mp4 합성 → media quality gate 통과 시에만 업로드. "no-run 검증 + Owner-run 가능 구조".
- **changed files**: `lib/owner-web-operator.ts`(+~700줄: Claude polish 엔진/script-final 캐시/실제 파이프라인 입력·상태/media gate/신규 action·command), `app/api/money-shorts/operator/route.ts`(신규 action 4종 핸들러 + scriptPreview 보정 + actualUpload 게이트 1.5 + GET audio=real/video=final), `components/VideoCreationWizard.tsx`(9단계 재구성 + 엔진 배지 + audio/video player + 게이팅), `scripts/check-owner-one-click-video-creation-ui-static.mjs`(+47 신규 검사), `scripts/check-owner-web-operator-ui-static.mjs`(helper fetch 규칙 정밀화 +3), `docs/simple-execution-manual.md`(9단계 실제 흐름), 신규 `scripts/run-owner-real-scene-images-from-wizard-script-once.mjs`, 신규 `scripts/run-owner-real-video-from-wizard-assets-once.mjs`.
- **A — Claude 대본 보정**: `polishWizardScriptWithClaude`(raw Node fetch, `fetchImpl` 주입, 고정 `ANTHROPIC_API_URL`, SDK 0) — 로컬 judge 최고 후보 1개만 1회 보정. JSON 계약(title/oneSentencePoint/hookLine/fullVoiceover/captionLines 6/scenes 6{label,caption,visualCue,narration}/uploadCaptionDraft/rewriteNotes) 검증: 자막 22자·6개, 설명체(니다)·제목 약패턴 금지, 로컬 judge 점수 회귀(-5 초과) 시 거부. key 없음/HTTP/파싱/검증 실패 → 로컬 대본 그대로(reasonCode 기록). `script-final.json`(fingerprint 캐시)로 재클릭 시 재호출 0. 기본 모델 `claude-sonnet-4-6`(Owner 비용 정책 메모리: 기본 Sonnet, Opus는 승인 필요 — `ANTHROPIC_POLISH_MODEL` env로 오버라이드 가능). 킬스위치: `WIZARD_DISABLE_CLAUDE_POLISH=1` 또는 `C:\tmp\...\DISABLE_LIVE_CLAUDE_POLISH.marker`(검증용, UI에 사유 표시, 검증 후 제거 완료).
- **B — 실제 TTS**: 검증된 기존 `build-elevenlabs-scene-paced-tts-from-script.mjs`를 그대로 스폰 재사용. 핵심 개선: `tts-script.real.json`의 scene durationSec을 내레이션 글자수(≈6자/초, 3~8s clamp)로 산정해 scene-paced의 하드트림(단어 잘림)을 원천 차단 — 이 duration이 최종 영상 scene 경계와 동일해 A/V 싱크 결정적. env는 `MEDIA_ENV_KEY_NAMES`(ELEVENLABS 4키)만 child 전달(값 출력 0). no-key → "실제 음성 키가 없어 생성하지 못했습니다. 테스트 소리는 업로드할 수 없습니다." fail-closed. 웹 audio player(`?audio=real`) 재생.
- **C — 장면 이미지**: 신규 once 스크립트가 `_chatgpt-image-core.mjs`(CDP Chrome AI-GPT-1) 재사용 — fresh-v3 실증 로직(25s passive→1.8s poll→stable×3 저장→intercept 회수→180s TIMEOUT). `ALLOW_CHATGPT_IMAGE=1` 게이트(route가 이 action에만 하드코딩 extraEnv, playwright import는 게이트 뒤), 씬당 1회 하드캡 6, 재실행 시 모자란 장면만, 세로형(w≥900,h≥1200,h≥1.2w) 검증, no-login/rate → BLOCKED fail-closed.
- **D — 최종 mp4**: 신규 once 스크립트 — 씬별 zoompan slow-zoom 세그먼트(1080x1920 cover) → concat → ASS 자막(Malgun Gothic, 기존 renderer 스타일) burn + 실제 mp3 mux → ffprobe 검증(1080x1920/15~60s/audio+video stream/size>0/scene 6) → `real-video-summary.json`. mock audio/placeholder 이미지 입력은 스크립트 자체가 거부. 출력 전부 `C:\tmp\money-shorts-os\<slug>\real\` 하위.
- **E — media quality gate**: `readWizardRealMediaState` — TTS(provider=elevenlabs+liveApiCallPerformed=true)·이미지(6/6 SAVED_OK)·영상(RENDER_MUX_OK+검증값) 전부 ready여야 ok. `buildWizardContentUnitForTopic`이 real 최종 mp4 필수로 재작성(reason: real_tts_required/real_scene_images_required/final_mp4_required/media_quality_gate_not_ready) → wizardPreflight·actualUpload 모두 시안 불가. route actualUpload에 [게이트 1.5] 서버 재검증 + 문구 "아직 실제 음성/실제 장면 이미지가 들어간 최종 영상이 아닙니다. 업로드를 막았습니다." blocker: REAL_TTS_REQUIRED/REAL_SCENE_IMAGES_REQUIRED/FINAL_MP4_REQUIRED/MEDIA_QUALITY_GATE_NOT_READY.
- **F — UI**: 9단계(카테고리/주제/대본/실제 목소리/장면 이미지/최종 영상/미리보기/게시 전 점검/업로드). 상태 칩: 실제 음성 준비·음성 키 필요·장면 이미지 준비 6/6·이미지 생성 필요 N/6·최종 영상 준비·시안 영상(업로드 불가). 대본 카드에 "대본 생성 방식: 로컬 후보 선별 → Claude 1회 보정" + 적용/미적용 배지 + 고친 부분 details. 테스트 소리/시안 영상은 details로 강등(업로드 불가 명시). 미리보기는 최종 영상(`?video=final`) 우선. 주제 선택 시 realMediaStatus로 진행 상태 자동 복원.
- **checks**: tsc exit 0 · wizard guard **261 PASS/0 FAIL**(신규 47: polish 구조/1회 캐시/SDK 0/killswitch/TTS 재사용·duration 산정/이미지 게이트·하드캡/영상 검증/media gate/--arm 단일 게이트 유지/UI 라벨) · web guard **91 PASS/0 FAIL**(helper 네트워크 = fetchImpl→ANTHROPIC_API_URL 1곳만으로 정밀화) · `node --check` 4종 exit 0 · package.json/pnpm-lock **무변경**. fake-fetch 하네스 **28/28 PASS**(적용/no-key/파싱 실패/계약 위반/HTTP 500/펜스/캐시/duration/media gate — 실호출 0, 검증 후 삭제). 브라우저+HTTP E2E **13/13 + UI 12항목 PASS**(marker 킬스위치 상태에서 대본 만들기 → "Claude 보정 미적용" 표시, 신규 스폰 경로 전부 스폰 전 차단 확인, actualUpload 서버 media gate 차단 + noLive:true, 콘솔 에러 0). **검증 중 Anthropic/ElevenLabs/ChatGPT/업로드 실호출 0.**
- **env/secret**: `.env*` Read/Edit/출력 0. key는 서버 런타임 process.env→(TTS만) sanitized child env로만 흐르고 로그/응답/summary에 값·길이·prefix·hash 없음. ANTHROPIC 키는 child에 전달 안 됨(in-process fetch 전용).
- **deviations/risks**: ① Claude 보정 기본 모델을 Owner 비용 정책(메모리)에 따라 Sonnet으로 함 — Codex가 Opus 원하면 `ANTHROPIC_POLISH_MODEL=claude-opus-4-8` env 한 줄. ② spawnSync 구조 유지로 이미지 생성(최대 21분) 동안 dev 서버가 블로킹됨(Owner 단독 로컬 전제, UI에 안내). ③ 이미지/TTS/영상 실제 생성 경로는 no-run 원칙상 Owner 첫 실행에서 최초 검증됨(스크립트는 기존 실증 로직 재사용 + fail-closed). ④ ChatGPT 이미지가 세로 기준(h≥1.2w) 미달이면 SAVED_LOW_QUALITY로 gate 차단 — 재클릭으로 해당 씬만 재생성. ⑤ web guard의 "helper no fetch" 검사를 승인된 아키텍처(anthropic 전용 단일 fetchImpl)로 재정의 — 완화가 아니라 정밀화(직접 fetch 금지는 유지).
- checkpoint recommendation: 수정 6 + 신규 2 파일, +대략 1,900줄 — release-critical 통합 슬라이스로 diff 큼. Codex checkpoint review 후 commit 권장. Owner 첫 실행 스모크(대본→실제 목소리→이미지→최종 영상 1회)가 다음 자연 단계.

## owner-web-real-pipeline-env-and-output-guard-review-fix-v1 (2026-07-09) — ✅ Codex review 보정 2건 완료

- **목표**: 직전 통합 task의 checkpoint 전 보안/운영 경계 보정 — (1) ELEVENLABS env를 realTtsCreate child에만 전달, (2) 신규 이미지/영상 스크립트 입출력을 `C:\tmp\money-shorts-os\` 하위로 강제.
- **changed files**: `lib/owner-web-operator.ts`(buildSanitizedChildEnv opt-in + runOperatorScript includeMediaEnv), `app/api/money-shorts/operator/route.ts`(realTtsCreate만 includeMediaEnv:true), `scripts/run-owner-real-scene-images-from-wizard-script-once.mjs`(경로 강제), `scripts/run-owner-real-video-from-wizard-assets-once.mjs`(5개 인자 경로 강제), `scripts/check-owner-one-click-video-creation-ui-static.mjs`(신규 검사 10), `_ai/CLAUDE_REPORT.md` append.
- **Finding 1 fix**: `buildSanitizedChildEnv(opts?: { includeMediaEnv?: boolean })` — MEDIA_ENV_KEY_NAMES(ELEVENLABS 4키) 복사는 `includeMediaEnv === true`일 때만. `runOperatorScript`에 `includeMediaEnv?: boolean` 추가, 기본 false. route는 `realTtsCreate`에서만 `includeMediaEnv: true`(정확히 1회, grep 확인). realSceneImagesCreate/finalVideoCreate/wizardPreflight/actualUpload 및 그 외 모든 runner는 ELEVENLABS 키 미전달. 주석도 "realTtsCreate child에만 전달"로 정정.
- **Finding 2 fix**: 이미지 스크립트 — `MEDIA_ROOT_RE=/^C:[\\/]+tmp[\\/]+money-shorts-os[\\/]+/i`로 `--out-dir`(디렉토리) + `--script`(.json)를 강제, ALLOW_CHATGPT_IMAGE 게이트·playwright import **이전**에 검사(어긋나면 exit 2, 외부 접근 0). 영상 스크립트 — `--script/--tts-script/--audio-summary/--images-dir/--out-dir` 5개 전부 PATH_INPUTS 루프로 강제, ffmpeg/ffprobe **이전**에 검사(exit 2). 기존 repo-밖/.money-shorts-local/ffmpeg 동작 무변경. helper의 real 경로는 전부 `C:\tmp\money-shorts-os\` 하위라 정상 흐름은 통과.
- **checks**: `node --check` 3종 exit 0 · wizard guard **271 PASS/0 FAIL**(신규 10: media env opt-in/includeMediaEnv 옵션·threading/route 단독 true·4개 action 미설정/이미지·영상 경로 강제) · web guard **91/0** · tsc exit 0 · package.json/pnpm-lock **무변경**. no-run fail-closed 실증: C:\tmp 밖 out-dir/script/audio-summary → exit 2(playwright/ffmpeg 전 차단), 정상 C:\tmp 경로 → 경로검사 통과 후 입력검증(exit 3) 도달(기존 ffmpeg 흐름 유지).
- **side effects 0 / env·secret untouched**: 외부 API/upload/Blob/ledger/`--arm`/`--live` 0, 검증 중 Anthropic/ElevenLabs/ChatGPT/Playwright 실호출 0, `.env*` Read/Edit/출력 0, secret 값·길이·prefix·hash 출력 0, dependency/lockfile/pnpm-workspace 미변경(pnpm-workspace.yaml은 세션 시작 전부터 dirty인 보호 파일 — 미터치).
