...
Next atomic slice: define a session-level interrupted-claim recovery contract before connecting lifecycle state to the executor. It must compare the active claim with the current artifact-derived coordinator/queue evidence and allow only one evidence-backed terminal outcome: acknowledge an already-advanced safe action, or clear an unchanged zero-progress claim for a later separate Owner-started attempt. Ambiguous drift must stay locked. The recovery slice must execute no action, create no execution receipt, retry nothing, and perform no paid/external generation, render, upload, or publication.

Later, after interrupted-claim recovery is independently reviewed, a separate slice may connect one coordinator claim to one executor call for an Owner-started local Node host. Do not activate or reuse the legacy n8n workflow.
