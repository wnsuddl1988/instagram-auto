...
Next atomic slice: expose a read-only safe-session recovery view and one exact Owner-confirmed direct recovery action through the local operator API/UI. The route must reconstruct the active claim's current artifact plan, historical execution guard, queue preview, and execution-recovery evidence; send the displayed recovery fingerprint and allowed decision back on the Owner click; then recompute everything inside the session mutation lock. Pending chain decisions must direct the Owner to the existing execution-receipt recovery first and must not mutate the session. Responses must prove zero executed actions, zero retry, and no paid/external generation, render, upload, or publication.

Later, after the recovery UI/action is independently reviewed, a separate slice may connect one coordinator claim to one executor call for an Owner-started local Node host. Do not activate or reuse the legacy n8n workflow.
