...
Contract slice 3 is complete: the local operator API and wizard expose `safeSessionStatus`, `safeSessionStart`, and `safeSessionStop`. The UI labels it `의도 기록 전용`; start/stop responses prove `actionCount:0`, `automaticRetryCount:0`, and `chainedActionCount:0`. No route branch calls the queue executor, receipt store, worker, timer, render, paid generation, upload, or publication service. Store 14/14, planner 15/15, queue 35/35, combined 79/79, operator UI 91/91, one-click UI 389/389, TypeScript, build, and diff checks pass.

Next atomic slice: add a standalone **session coordinator dry-run** module. It will read the persisted session intent and existing deterministic queue preview, then return at most one content-addressed local-safe claim or a halt reason. It must not claim/lock a job, write a receipt, dispatch an action, start a process/timer, retry, or access any external service. This makes the future local host boundary testable before any executor is permitted.

Later, after that dry-run coordinator is independently reviewed, a separate slice may extract the current queue reconstruction and bounded executor out of the Next route for an Owner-started local Node host. Do not activate or reuse the legacy n8n workflow.
