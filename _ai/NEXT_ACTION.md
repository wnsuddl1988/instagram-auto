...
Next atomic slice: extract the current bounded one-action dispatcher and its exact begin/finish receipt lifecycle from the Next route into a shared local-only executor service. This is still an architecture-preserving refactor: do not add a session loop, timer, background process, automatic retry, new allowlisted action, paid/external generation, Owner QA decision, upload, or publication authority, and do not execute a real action during validation. The existing route must consume the new service with response fields and fail-closed behavior unchanged.

Later, after the shared executor service is independently reviewed, a separate slice may connect one coordinator claim to one executor call for an Owner-started local Node host. Do not activate or reuse the legacy n8n workflow.
