...
Next atomic slice: extract the existing artifact-derived queue read model (`readMoneyShortsAutomationQueueView`) out of the Next route into a shared local-only module, without changing its returned queue preview or adding a worker. The route and a future Owner-started Node host must then consume the same read model. This is an architecture-preserving refactor only: no session start/stop action, lock/receipt creation, queue dispatch, timer, retry, paid/external generation, render, upload, or publication authority may be added.

Later, after the shared read model is independently reviewed, a separate slice may connect the pure coordinator to that read model for an Owner-started local Node host. Do not activate or reuse the legacy n8n workflow.
