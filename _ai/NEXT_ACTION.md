...
Next atomic slice: extend the durable safe-session store with atomic action lifecycle transitions before connecting it to the executor. `begin` must bind the exact coordinator/queue claim fingerprint and set `actionInFlight:true`; `finish` must accept only the matching claim, increment `completedActionCount` exactly once, clear in-flight state, record a bounded terminal result, and halt at stop request or cap. Interrupted or mismatched evidence must remain fail-closed for Owner recovery. This slice must still invoke no queue action, executor, timer, retry, paid/external generation, render, upload, or publication.

Later, after session lifecycle transitions are independently reviewed, a separate slice may connect one coordinator claim to one executor call for an Owner-started local Node host. Do not activate or reuse the legacy n8n workflow.
