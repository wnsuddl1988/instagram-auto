...
Contract slice 3 is complete: the local operator API and wizard expose `safeSessionStatus`, `safeSessionStart`, and `safeSessionStop`. The UI labels it `의도 기록 전용`; start/stop responses prove `actionCount:0`, `automaticRetryCount:0`, and `chainedActionCount:0`. No route branch calls the queue executor, receipt store, worker, timer, render, paid generation, upload, or publication service.
...
Next atomic slice: add the atomic safe-session recovery mutation, still without connecting the session to the executor. It must accept only the currently active claim plus a freshly recomputed matching recovery fingerprint. Proven unstarted or execution-cleared claims must be cleared while forcing the session into a halted stop state with zero count increment; a matching terminal receipt must reuse the exact terminal result and increment once; repeated identical resolution must be idempotent. Chain decisions that still require execution-receipt recovery must not mutate the session until that separate Owner-confirmed receipt recovery is complete. Mismatch or drift must keep the claim locked.

Later, after the recovery mutation and its read-only Owner UI are independently reviewed, a separate slice may connect one coordinator claim to one executor call for an Owner-started local Node host. Do not activate or reuse the legacy n8n workflow.
