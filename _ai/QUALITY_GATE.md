...
## 6. Episode Pre-Production Rules (2026-06-16 추가)

upload_002_copier Owner QA FAIL 교훈 반영. 에피소드 기획 단계에서 걸러내야 할 구조적 실패 패턴.

### 생성 금지 조건

에피소드 concept/script/scene contract는 Owner 승인 전 생성 금지.  
아래 항목 중 하나라도 해당하면 기획안 자체를 FAIL 처리한다.

| 조건 | 판정 |
|---|---|
| 대사가 화면 사건보다 먼저 나오는 구조 | FAIL |
| 컷 전환 중 핵심 대사가 걸리는 구조 | FAIL |
| 대사로 화면을 억지 설명하는 구조 ("이게 바로 X입니다" 형) | FAIL |
| Boss/제3자 등장에 화면상 진입 근거 없음 | FAIL |
| 영상 없이 대본만 읽어도 의미가 통하는 구조 | FAIL |
| 기존 영상에 대사 위치만 조정하는 수습 방식 | FAIL |
| 씬 구조가 없고 대사 배치만 있는 경우 | FAIL |

### 통과 조건

- 각 씬의 화면 사건(visible action)이 대사보다 먼저 정의됨
- 대사는 사건의 반응/결과로만 배치
- 무대사 구간에 SFX/BGM 또는 강한 시각 사건이 있음
- 펀치라인을 대본 설명 없이 화면만으로 이해 가능
